...
- Find critical code paths without tests (jCodemunch `get_untested_symbols` is a good starting point)
...
2. **Codebase Search**: Find code without corresponding tests (`get_untested_symbols`, `find_dead_code`)
...
- Uses the `mythosmud-test-writing` skill for test conventions and layout
- Supports `make test` and `make test-comprehensive` workflows
- References project test coverage requirements (70% minimum, 90%+ for critical code)
...
- **Critical Code**: 90%+ minimum (security, core features, user-facing code)
...
```
Goal: Ensure critical authentication code has 90%+ coverage
Process:
...
- Ported from `.cursor/agents/test-analyzer.md`
