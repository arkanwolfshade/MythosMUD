---
description: Execute the modular multiplayer E2E Playwright test suite (21 scenarios) exactly as written, no deviation.
argument-hint: [all | scenario-NN-name.md | scenario range]
---
...
- Requires the `playwright` MCP server (registered via `claude mcp add playwright -- npx -y @playwright/mcp@latest`)

**SUBAGENT INTEGRATION:**

This command can leverage the **Codebase Explorer** agent for test discovery and analysis:
- **Agent**: `.claude/agents/codebase-explorer.md`
- **Purpose**: Deep codebase exploration for finding test files and understanding test structure
- **When Used**: When test discovery or architecture analysis is needed
...
All scenarios use the Playwright MCP server for multi-tab coordination
...
*Ported from `.cursor/commands/multiplayer-playwright-testing.md`.*
