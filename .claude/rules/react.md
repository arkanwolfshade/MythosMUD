...
### MythosMUD client (canonical layout)

The MythosMUD client uses **`components/`**, **`pages/`**, **`hooks/`**, **`contexts/`**, and
**`stores/`** as the primary structure under `src/` (see `client/README.md`). Prefer
`src/features/<domain>/` for **new** work when it clearly spans multiple layers; do not
require a wholesale migration of existing code.

### Component typing: prefer plain functions over `React.FC`

For **new** components, prefer `function ComponentName(props: Props)` (or `const` with an
explicit `Props` type) instead of `React.FC` / `FC<Props>`, for clearer `children` typing and
default props behavior. A repo-wide removal of `React.FC` is optional backlog; do not block
reviews on migrating legacy files.
