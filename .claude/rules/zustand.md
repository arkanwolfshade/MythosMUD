...
Prefer **several focused stores** (e.g. connection, game, commands) over one oversized store. The
slice-composition pattern below is optional: use it when a single store module grows hard to
navigate; otherwise multiple `create()` modules remain valid.
...
const UserProfile = () => {
  ...
  const { name, email } = useAppStore((state) => ({ name: state.user?.name, email: state.user?.email }), shallow);
  // ...
};
