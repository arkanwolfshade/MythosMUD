...
const getFontSizeClass = (): string => 'text-sm';
...
export const ChatPanel: React.FC<ChatPanelProps> = ({
  ...
  const filteredMessages = useMemo(
    () =>
      messages.filter(message => {
        if (message.channel === 'game-log' || message.messageType === 'system' || message.messageType === 'combat') {
          return false;
        }
        ...
        if (messageChannel === 'whisper') return normalizedSelectedChannel === 'whisper';
        if (messageChannel === 'party') return normalizedSelectedChannel === 'party';
        return messageChannel === normalizedSelectedChannel;
      }),
  ...
  const searchFilteredMessages = useMemo(() => {
    ...
    if (searchQuery.trim()) {
      ...
      baseMessages = baseMessages.filter(message => (message.rawText ?? message.text).toLowerCase().includes(query));
    }
    ...
    if (searchFilterType !== 'all') {
      baseMessages = baseMessages.filter(message => message.messageType === searchFilterType);
    }
    return baseMessages;
  ...
  const chatStats = { currentChannelMessages: filteredMessages.length, totalMessages: historyEligibleMessages.length };

  const unreadCounts = useMemo(() => {
    if (isAllChannelSelected) return {};
  ...
  const getMessageClass = (message: ChatMessage): string => {
    if (message.tags?.includes('hallucination')) return 'text-fuchsia-300 italic';
    if (message.tags?.includes('command-misfire')) return 'text-mythos-terminal-warning font-semibold';
    if (message.tags?.includes('rescue')) return 'text-mythos-terminal-primary font-semibold';
  ...
  const exportToText = (messagesToExport: ChatMessage[]): string =>
    messagesToExport.map(msg => `[${formatTimestamp(msg.timestamp)}] ${msg.rawText ?? msg.text}`).join('\n');
  const exportToHTML = (messagesToExport: ChatMessage[]): string => {
    const htmlMessages = messagesToExport
      .map(msg => {
        ...
        return `<div class="message" style="margin-bottom: 1em;"><div style="font-size: 0.8em; color: #888;">[${formatTimestamp(msg.timestamp)}]</div><div class="${getMessageClass(msg)}">${text}</div></div>`;
      })
      .join('\n');
  ...
  const exportToCSV = (messagesToExport: ChatMessage[]): string => {
    ...
    const rows = messagesToExport.map(msg => {
      ...
      return `"${formatTimestamp(msg.timestamp)}","${msg.channel || ''}","${msg.messageType || ''}","${text}"`;
    });
        ...
        {visibleMessages.length === 0 ? (
          ...
          <div className="space-y-3">
            {visibleMessages.map((message, index) => {
              const shouldAnimateMessage = !isHistoryVisible && !searchQuery && index >= visibleMessages.length - 10;
              const animationClass = shouldAnimateMessage ? 'animate-fade-in' : '';
              const animationDelay = shouldAnimateMessage ? `${(index - (visibleMessages.length - 10)) * 40}ms` : '0ms';
              return (
                <div
                  key={index}
                  className={
