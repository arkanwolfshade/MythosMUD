...
describe('sanityEventUtils', () => {
  ...
  it('should return fallback when sanitizeTier receives non-string value', () => {
    ...
    // We can't directly test sanitizeTier as it's not exported, but we can test it through buildSanityStatus
    const { status } = buildSanityStatus(
      null,
      { tier: 123, current_san: 50 }, // tier is not a string
      '2025-11-13T12:00:00Z'
    );
  ...
  it('should parse number from string in parseNumber', () => {
    ...
    // We can't directly test parseNumber, but we can test it through buildSanityStatus
    const { status } = buildSanityStatus(
      null,
      { current_san: '75', max_san: '100' }, // numbers as strings
      '2025-11-13T12:00:00Z'
    );
  ...
  it('should return fallback when tier is not in valid tiers list', () => {
    ...
    const { status } = buildSanityStatus(null, { tier: 'invalid_tier', current_san: 50 }, '2025-11-13T12:00:00Z');
  ...
  it('should handle parseNumber with non-finite parsed value', () => {
    ...
    const { status } = buildSanityStatus(
      null,
      { current_san: 'not-a-number', max_san: 'also-not-a-number' },
      '2025-11-13T12:00:00Z'
    );

    // Assert - should use fallback values when parsing fails
    expect(status.current).toBe(0); // fallback for null previous
    expect(status.max).toBe(100); // DEFAULT_MAX_SAN
  });

  it('should use currentSan when current_san is not provided', () => {
    // Arrange - Test line 35: currentSan fallback branch
    const { status } = buildSanityStatus(null, { currentSan: 75, max_san: 100 }, '2025-11-13T12:00:00Z');

    // Assert - should use currentSan
    expect(status.current).toBe(75);
  });

  it('should use currentSan when current_san is null', () => {
    // Arrange - Test line 35: currentSan fallback when current_san is null
    const { status } = buildSanityStatus(
      null,
      { current_san: null, currentSan: 80, max_san: 100 },
      '2025-11-13T12:00:00Z'
    );

    // Assert - should use currentSan when current_san is null
    expect(status.current).toBe(80);
  });

  it('should use currentSan when current_san is undefined', () => {
    // Arrange - Test line 35: currentSan fallback when current_san is undefined
    const { status } = buildSanityStatus(
      null,
      { current_san: undefined, currentSan: 85, max_san: 100 },
      '2025-11-13T12:00:00Z'
    );

    // Assert - should use currentSan when current_san is undefined
    expect(status.current).toBe(85);
  });

  it('should calculate current from previous + delta when neither current_san nor currentSan provided', () => {
  ...
  it('should use previous.current when previous exists and both current_san and currentSan are null', () => {
    // Arrange - Test line 35: previous?.current branch when previous exists
    const { status } = buildSanityStatus(
      { current: 90, max: 100, tier: 'lucid', liabilities: [] },
      { current_san: null, currentSan: null, delta: 5 },
      '2025-11-13T12:00:00Z'
    );
  ...
  it('should use 0 when previous is null and both current_san and currentSan are null', () => {
    // Arrange - Test line 35: (previous?.current ?? 0) branch when previous is null
    const { status } = buildSanityStatus(
      null,
      { current_san: null, currentSan: null, delta: 10 },
      '2025-11-13T12:00:00Z'
    );
  ...
  it('should use 0 + delta when both current_san and currentSan are null and no previous', () => {
  ...
  it('should use 0 + delta when both current_san and currentSan are undefined and no previous', () => {
    // Arrange - Test line 35: fall through to (previous?.current ?? 0) + delta when previous is null
    const { status } = buildSanityStatus(
      null,
      { current_san: undefined, currentSan: undefined, delta: -3 },
      '2025-11-13T12:00:00Z'
    );
  ...
  it('should use current_san when it is 0 (falsy but not null/undefined)', () => {
    // Arrange - Test line 35: current_san ?? branch - 0 is falsy but ?? only triggers for null/undefined
    const { status } = buildSanityStatus(null, { current_san: 0, max_san: 100 }, '2025-11-13T12:00:00Z');

    // Assert - ?? operator only triggers for null/undefined, so 0 should be used
    expect(status.current).toBe(0);
  });

  it('should use maxSan when max_san is not provided', () => {
    // Arrange - Test line 38: maxSan fallback branch
    const { status } = buildSanityStatus(null, { current_san: 50, maxSan: 120 }, '2025-11-13T12:00:00Z');

    // Assert - should use maxSan
    expect(status.max).toBe(120);
  });

  it('should use DEFAULT_MAX_SAN when max is 0 or negative', () => {
    // Arrange - Test line 47: max > 0 check
    const { status } = buildSanityStatus(null, { current_san: 50, max_san: 0 }, '2025-11-13T12:00:00Z');

    // Assert - should use DEFAULT_MAX_SAN (100) when max is 0
    expect(status.max).toBe(100);
  });
  ...
  it('should filter out empty liability entries', () => {
    ...
    const { status } = buildSanityStatus(
      null,
      { current_san: 50, liabilities: ['valid', '', 'also-valid'] },
      '2025-11-13T12:00:00Z'
    );
  ...
  it('should build sanity change message without reason or source', () => {
    // Arrange - Test lines 73-78: buildSanityChangeMessage branches
    const { status, delta } = buildSanityStatus(null, { current_san: 75, delta: -5 }, '2025-11-13T12:00:00Z');
    const message = buildSanityChangeMessage(status, delta, {});
  ...
  it('should build sanity change message with reason but no source', () => {
    // Arrange
    const { status, delta } = buildSanityStatus(null, { current_san: 75, delta: -5 }, '2025-11-13T12:00:00Z');
    const message = buildSanityChangeMessage(status, delta, { reason: 'disturbing_encounter' });
  ...
  it('should build sanity change message with source but no reason', () => {
    // Arrange
    const { status, delta } = buildSanityStatus(null, { current_san: 75, delta: -5 }, '2025-11-13T12:00:00Z');
    const message = buildSanityChangeMessage(status, delta, { source: 'Byakhee' });
