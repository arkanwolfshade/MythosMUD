...
**Fix**: Created `authenticatedTest` and `adminTest` fixtures that login once per test worker and share the session
...
**Fix**: Removed cleanup calls from authenticated test suites (session is shared, so cleanup isn't needed between
...
4. **Session Management**: If tests need true isolation, consider using separate browser contexts instead of shared
