...
--
\ restrict 9wgDAUNpT1eOAIH0i0DMbSjMLNdwKmkdYM53riAlkOJhGZGapfJkElb4oTIl4Ef -- Dumped from database version 18.1
...
ALTER TABLE IF EXISTS public.sanity_exposure_state
ALTER COLUMN id DROP DEFAULT;
ALTER TABLE IF EXISTS public.sanity_cooldowns
ALTER COLUMN id DROP DEFAULT;
ALTER TABLE IF EXISTS public.sanity_adjustment_log
ALTER COLUMN id DROP DEFAULT;
ALTER TABLE IF EXISTS public.professions
ALTER COLUMN id DROP DEFAULT;
ALTER TABLE IF EXISTS public.npc_spawn_rules
ALTER COLUMN id DROP DEFAULT;
ALTER TABLE IF EXISTS public.npc_relationships
ALTER COLUMN id DROP DEFAULT;
ALTER TABLE IF EXISTS public.npc_definitions
ALTER COLUMN id DROP DEFAULT;
ALTER TABLE IF EXISTS public.item_component_states
ALTER COLUMN id DROP DEFAULT;
...
CONSTRAINT calendar_holidays_day_check CHECK (
    (
        (day >= 1)
        AND (day <= 31)
    )
),
CONSTRAINT calendar_holidays_duration_hours_check CHECK (
    (
        (duration_hours >= 1)
        AND (duration_hours <= 168)
    )
),
CONSTRAINT calendar_holidays_month_check CHECK (
    (
        (month >= 1)
        AND (month <= 12)
    )
...
notes text,
CONSTRAINT calendar_npc_schedules_end_hour_check CHECK (
    (
        (end_hour >= 0)
        AND (end_hour <= 24)
    )
),
CONSTRAINT calendar_npc_schedules_start_hour_check CHECK (
    (
        (start_hour >= 0)
        AND (start_hour <= 24)
    )
...
--
CREATE SEQUENCE public.item_component_states_id_seq AS integer START WITH 1 INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1;
--
...
created_at timestamp with time zone DEFAULT now() NOT NULL,
updated_at timestamp with time zone DEFAULT now() NOT NULL,
CONSTRAINT npc_definitions_npc_type_check CHECK (
    (
        (npc_type)::text = ANY (
            (
                ARRAY ['shopkeeper'::character varying, 'quest_giver'::character varying, 'passive_mob'::character varying, 'aggressive_mob'::character varying]
            )::text []
        )
    )
)
...
--
CREATE SEQUENCE public.npc_definitions_id_seq AS integer START WITH 1 INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1;
--
...
CONSTRAINT npc_relationships_relationship_type_check CHECK (
    (
        (relationship_type)::text = ANY (
            (
                ARRAY ['ally'::character varying, 'enemy'::character varying, 'neutral'::character varying, 'follower'::character varying]
            )::text []
        )
    )
)
...
--
CREATE SEQUENCE public.npc_relationships_id_seq AS integer START WITH 1 INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1;
--
...
--
CREATE SEQUENCE public.npc_spawn_rules_id_seq AS integer START WITH 1 INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1;
--
...
CONSTRAINT player_sanity_current_san_check CHECK (
    (
        (current_san >= '-100'::integer)
        AND (current_san <= 100)
    )
),
CONSTRAINT player_sanity_current_tier_check CHECK (
    (
        (current_tier)::text = ANY (
            (
                ARRAY ['lucid'::character varying, 'uneasy'::character varying, 'fractured'::character varying, 'deranged'::character varying, 'catatonic'::character varying]
            )::text []
        )
    )
)
...
--
CREATE SEQUENCE public.professions_id_seq AS integer START WITH 1 INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1;
--
...
--
CREATE SEQUENCE public.sanity_adjustment_log_id_seq AS integer START WITH 1 INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1;
--
...
--
CREATE SEQUENCE public.sanity_cooldowns_id_seq AS integer START WITH 1 INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1;
--
...
--
CREATE SEQUENCE public.sanity_exposure_state_id_seq AS integer START WITH 1 INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1;
--
...
CONSTRAINT chk_subzones_environment CHECK (
    (
        (environment IS NULL)
        OR (
            environment = ANY (
                ARRAY ['indoors'::text, 'outdoors'::text, 'underwater'::text, 'void'::text]
            )
        )
    )
)
...
special_rules jsonb DEFAULT '{}'::jsonb,
CONSTRAINT chk_zones_environment CHECK (
    (
        (environment IS NULL)
        OR (
            environment = ANY (
...
CONSTRAINT chk_zones_zone_type CHECK (
    (
        (zone_type IS NULL)
        OR (
            zone_type = ANY (
                ARRAY ['city'::text, 'countryside'::text, 'mountains'::text, 'swamp'::text, 'tundra'::text, 'desert'::text, 'death'::text]
            )
        )
    )
)
...
--
CREATE UNIQUE INDEX uq_muting_active_per_target ON public.muting_rules USING btree (target_type, target_id)
WHERE is_active;
--
...
--
ALTER TABLE ONLY public.invites
ADD CONSTRAINT invites_created_by_user_fkey FOREIGN KEY (created_by_user_id) REFERENCES public.users(id) ON DELETE
SET NULL;
--
-- Name: invites invites_used_by_user_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
ALTER TABLE ONLY public.invites
ADD CONSTRAINT invites_used_by_user_id_fkey FOREIGN KEY (used_by_user_id) REFERENCES public.users(id) ON DELETE
SET NULL;
--
...
--
ALTER TABLE ONLY public.muting_rules
ADD CONSTRAINT muting_rules_created_by_user_fkey FOREIGN KEY (created_by_user) REFERENCES public.users(id) ON DELETE
SET NULL;
--
