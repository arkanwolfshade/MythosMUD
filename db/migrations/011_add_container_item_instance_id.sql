...
BEGIN
    IF NOT EXISTS (
        ...
        ALTER TABLE containers ADD COLUMN container_item_instance_id VARCHAR(64);
        ...
        IF NOT EXISTS (
            ...
            RAISE EXCEPTION 'Failed to create container_item_instance_id column';
        END IF;
    ELSE
        RAISE NOTICE 'container_item_instance_id column already exists in containers table';
    END IF;
END $$;

-- Step 2: Verify item_instances table exists before creating foreign key
DO $$
BEGIN
    IF NOT EXISTS (
        ...
        RAISE EXCEPTION 'item_instances table does not exist. Migration must run after authoritative_schema.sql';
    END IF;
END $$;
...
BEGIN
    ...
    ELSE
        RAISE NOTICE 'Foreign key constraint fk_containers_container_item_instance already exists';
    END IF;
END $$;
...
-- Step 5: Create container_contents table if it doesn't exist
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.tables
        ...
        IF NOT EXISTS (
            ...
            RAISE EXCEPTION 'Failed to create container_contents table';
        END IF;
    ELSE
        RAISE NOTICE 'container_contents table already exists';
    END IF;
END $$;

-- Step 6: Create indexes for container_contents if they don't exist
...
-- Step 7: Create stored procedures for container operations
...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        RAISE EXCEPTION 'Migration failed: container_item_instance_id column does not exist after migration';
    END IF;
    ...
    IF NOT EXISTS (
        ...
        RAISE EXCEPTION 'Migration failed: container_contents table does not exist after migration';
    END IF;
    ...
    IF NOT EXISTS (
        ...
        RAISE EXCEPTION 'Migration failed: add_item_to_container function does not exist after migration';
    END IF;
