...
-- Description: Enables multiple characters per user, case-insensitive
-- uniqueness, and soft deletion
-- Date: 2025-01-XX
--
-- This migration:
-- 1. Removes unique constraint on players.user_id to allow multiple
--    characters per user
-- 2. Adds is_deleted and deleted_at columns for soft deletion
-- 3. Replaces unique constraints with case-insensitive unique indexes
--    for usernames and character names
-- 4. Creates indexes for efficient queries

-- Step 1: Drop unique constraint on players.user_id
-- Handle both possible constraint names: players_user_id_key and
-- players_new_user_id_key
DO $$
BEGIN
    IF EXISTS (
...
-- Step 2: Drop unique constraint on users.username (will be replaced
-- with case-insensitive index)
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM pg_constraint
...
-- This enforces uniqueness case-insensitively (e.g., "ithaqua" and
-- "Ithaqua" are mutually exclusive)
...
-- This enforces uniqueness case-insensitively only for active
-- (non-deleted) characters. Deleted character names can be reused
...
COMMENT ON COLUMN players.is_deleted IS
'Soft delete flag - deleted characters are hidden but not removed'
' from database';
COMMENT ON COLUMN players.deleted_at IS
'Timestamp when character was soft-deleted (NULL for active'
' characters)';
