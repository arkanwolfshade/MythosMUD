...
2. **Responsive Design:** Layout uses absolute pixels. For responsive design, consider viewport-relative positioning or

   media queries.

3. **Panel Ordering:** Z-index values are critical. When adding new panels, assign appropriate z-index values based on

   desired layering.

4. **Header Height:** If header height changes, update panel `defaultPosition.y` values accordingly (currently `y: 80`).

5. **Content Overflow:** Content areas use `overflow-auto` for scrolling. Ensure content doesn't overflow panel
