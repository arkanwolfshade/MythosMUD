...
| Surface | Excluded because | Risk if wrong | Status | Closed by |
|---|---|---|---|---|
| Systematic security review (auth, authz, injection, secrets handling) | filed separately — open-ended corpus, distinct from the bounded defect-class sweep below | unknown — the one incidental finding was Critical | SWEPT, 8 findings, all fixed inline | `#734` |
...
| `server_authority_remediation` | unverifiable on a stale index; needs a dedicated client-side pass | unknown | RESOLVED — agent-rule half adjudicated intentional (tooling config, same category §7 already excludes); client-side pass complete, 10 violations fixed, 3 server gaps closed | `FRD_PLAN_VERIFICATION_REGISTER_2026-08.md` §4, `CLIENT_SERVER_AUTHORITY_REGISTER_2026-09.md`, `#752` (closed) |
