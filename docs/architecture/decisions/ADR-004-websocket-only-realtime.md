...
> `ConnectionManager.player_websockets` maps a player to a *list* of connections and a session may hold
> several (grace reconnect, multiple tabs). WebSocket-only transport is unaffected — only connection
...
| 1.1.0 | 2026-08-19 | Cross-reference ADR-018: connection multiplicity amended |
