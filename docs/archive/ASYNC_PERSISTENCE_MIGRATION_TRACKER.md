...
This document tracks the migration from synchronous `PersistenceLayer` methods to asynchronous `AsyncPersistenceLayer`
or `asyncio.to_thread()` wrapped methods to eliminate event loop blocking.
...
1. user_manager.py (5 instances)
2. player_position_service.py (2 instances)
3. player_death_service.py (1 instance)
4. player_combat_service.py (1 instance)
5. health_service.py (1 instance)

### Phase 4: Startup Services (Low Priority)

1. npc_startup_service.py (3 instances) - Only runs at startup
2. environmental_container_loader.py (2 instances) - Only runs at startup
3. corpse_lifecycle_service.py (4 instances) - Infrequent operations
