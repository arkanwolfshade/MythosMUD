...
| WebSocket/HTTP correlation | `correlation_middleware.py` | `CorrelationMiddleware` (HTTP) and `WebSocketCorrelationMiddleware` (WS) — attach a request-scoped correlation ID for cross-log tracing. Only `CorrelationMiddleware` is wired into the app (`server/main.py`); `WebSocketCorrelationMiddleware` and its `create_websocket_correlation_middleware` factory have zero production references — filed as [`#754`](https://github.com/arkanwolfshade/MythosMUD/issues/754). |
