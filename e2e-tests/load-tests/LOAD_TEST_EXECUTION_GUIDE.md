...
2. **Verify ports are free**: Check ports 54768 and 5173
...
await mcp_playwright_browser_fill_form({
  fields: [
    ...
    { name: "Password", type: "textbox", ref: "password-input", value: "<ephemeral-per-player-credential>" },
    // Generate a unique value per player (see generateLoadTestCredential in load_test_10_players.spec.ts).
