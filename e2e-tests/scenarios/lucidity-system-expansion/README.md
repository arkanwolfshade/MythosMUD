...
These scenarios extend the multiplayer test suite with coverage for the new lucidity workflows described in `2025-11-13-lucidity-system-expansion`. Follow the master rules in `@MULTIPLAYER_TEST_RULES.md` before executing any scenario.
...
- Whenever a scenario requires modifying the lucidity tables, run the SQL commands exactly as written and verify the row count before proceeding.
- After finishing each scenario, restore the altered lucidity rows to their original values to avoid cross-scenario contamination.
