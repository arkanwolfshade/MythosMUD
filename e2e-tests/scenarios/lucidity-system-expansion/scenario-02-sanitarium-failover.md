# Scenario LCD-02: Sanitarium Failover Escalation **[REQUIRES MULTI-PLAYER]**
...
### Step 3 – Monitor Passive Flux to −100 LCD
...
When LCD reaches −100, the catatonia observer triggers the sanitarium transfer.
...
- Target LCD meter jumps to `1 / 100`, tier `Deranged`.
...
```javascript
const luciditySummary = await mcp_playwright_browser_evaluate({
  function: "() => document.querySelector('[data-testid=\"lucidity-meter\"]')?.textContent || ''"
});
console.log("LCD meter snapshot:", luciditySummary);
...
Reset the victim’s lucidity and room placement to pre-test defaults.
