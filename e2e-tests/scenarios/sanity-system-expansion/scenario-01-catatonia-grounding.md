# Scenario SAN-01: Catatonia Grounding Ritual **[REQUIRES MULTI-PLAYER]**

## Overview

Validates the end-to-end rescue flow when an investigator becomes catatonic: the target is locked out of commands, UI banners surface the rescue status, the rescuer channels `ground`, and both players receive synchronized feedback when sanity stabilizes.
...
**Purpose**: Confirm SAN stabilizes and UI clears the emergency banner.

```javascript
await mcp_playwright_browser_wait_for({ text: "Rescue ritual succeeds", time: 20 });
```

- Target SAN meter updates to `1 / 100` with tier `Deranged`.
