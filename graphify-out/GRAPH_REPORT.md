# Graph Report - MythosMUD  (2026-09-10)

## Corpus Check
- 3308 files · ~3,478,937 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 55386 nodes · 104156 edges · 2224 communities (1721 shown, 503 thin omitted)
- Extraction: 94% EXTRACTED · 6% INFERRED · 0% AMBIGUOUS · INFERRED: 6060 edges (avg confidence: 0.93)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `9ec102a1`
...
- GameClientV2AuxiliaryPanels.tsx
...
- test_connection_manager_methods.py
...
- TestGetNPCLifecycleManager
- TestGetCatatoniaRegistry
- TestGetPassiveLucidityFluxService
- TestGetMythosTimeConsumer
- TestGetChatService
- TestGetNatsMessageHandler
...
- test_record_integrity_check_no_violation
- test_validate_room_integrity_duplicate_players
- test_get_metrics_with_data
- test_get_metrics_integrity_rate
- test_get_alerts_no_alerts
- test_get_alerts_high_failure_rate
- test_get_alerts_slow_movement_time
- test_reset_metrics
- test_get_movement_monitor_returns_singleton
- test_log_performance_summary
- test_record_movement_attempt_success
- test_record_movement_attempt_string_player_id
- test_record_movement_attempt_multiple_players
...
1. `get_logger()` - 538 edges
2. `LoggedHTTPException` - 370 edges
3. `ValidationError` - 318 edges
4. `User` - 307 edges
5. `AliasStorage` - 269 edges
6. `DatabaseError` - 248 edges
7. `Player` - 240 edges
8. `EventBus` - 224 edges
9. `CombatParticipant` - 217 edges
10. `CombatInstance` - 190 edges
...
## Communities (2224 total, 503 thin omitted)

### Community 0 - "get_logger"
Cohesion: 0.00
Nodes (769): AliasRecord, CombatMessages, get_alerts(), health(), get, Health check endpoint, Get recent alerts (for testing), PasswordHasher (+761 more)

### Community 1 - "ErrorType"
Cohesion: 0.02
Nodes (121): JSONResponse, ErrorResponseDetailsInput, post, Request, Receive and log alert webhooks, webhook(), Error handlers package for MythosMUD. This package provides specialized error…, convert_pydantic_error() (+113 more)

### Community 2 - "User"
Cohesion: 0.01
Nodes (478): AuthenticationBackend, BaseUserManager, CharacterInfo, DependsParam, get_container, get_current_active_user, ID, IntegrityError (+470 more)

### Community 3 - "container_endpoints_basic.py"
Cohesion: 0.01
Nodes (376): get_connection_manager, _apply_inventory_stack_defaults(), _as_inventory_dicts(), _as_str_list(), _as_str_object_dict(), _as_str_object_mapping(), _build_container_data_from_dict(), _build_open_container_response() (+368 more)

### Community 4 - "Player"
Cohesion: 0.01
Nodes (631): _apply_grounding_adjustment(), _complete_ground_command(), _get_ground_services(), handle_ground_command(), handle_rescue_command(), _normalize_player_ids(), Any, UUID (+623 more)

### Community 5 - "command.py"
Cohesion: 0.02
Nodes (122): AliasesCommand, Alias command models for MythosMUD. This module provides command models for…, Command for listing all aliases., CommandType, StrEnum, Valid command types for MythosMUD., ChannelCommand, Channel management command models for MythosMUD. This module provides command… (+114 more)

### Community 6 - "server/dependencies.py"
Cohesion: 0.04
Nodes (90): get_async_persistence(), get_catatonia_registry(), get_chat_service(), get_combat_service(), get_connection_manager(), get_container(), get_exploration_service(), get_level_service() (+82 more)

### Community 7 - "NPCCombatIntegrationService"
Cohesion: 0.01
Nodes (263): Check if participant is alive enough to be in combat. For players: alive if DP…, CombatResultCtx, NPCCombatHandlers, Any, NPC Combat Event Handlers. This module handles combat result processing and NPC…, Handle NPC death when combat ends, with defensive exception handling. Args:…, Handle NPC death and related effects., Check if a string is a valid UUID. (+255 more)
...
### Community 9 - "inventory_equip_command.py"
Cohesion: 0.03
Nodes (122): AbstractContextManager, Equip command: move an item from inventory to an equipment slot., _ensure_shared_services_initialized(), get_shared_services(), Shared service initialization for inventory commands., Clear lazy singletons so each test gets a fresh init path. For unit tests only;…, Resolve async_persistence from the request and construct shared singletons., Get shared service instances, initializing them lazily if needed. This ensures… (+114 more)

### Community 10 - "NPCBase"
Cohesion: 0.01
Nodes (242): NPCAttacked, NPCListened, NPCTookDamage, Event fired when an NPC attacks a target. This event is triggered when an NPC…, Event fired when an NPC takes damage. This event is triggered when an NPC…, Event fired when an NPC receives a message. This event is triggered when an NPC…, AggressiveMobNPC, Protocol (+234 more)

### Community 11 - "NATSService"
Cohesion: 0.01
Nodes (263): AppConfig, BaseSettings, model_validator, Composite application configuration model., Composite application configuration. This is the main configuration class that…, Set environment variables for legacy code that reads them directly., ChatConfig, BaseSettings (+255 more)

### Community 12 - "TargetMatch"
Cohesion: 0.02
Nodes (214): Resolve a typed target match for the given name in the current context., NpcIntegrationStringIdPort, NpcLifecycleManagerPort, NpcSpellDamageTarget, PlayerPersistenceSpellPort, PlayerServiceHealPort, Protocol, UUID (+206 more)

### Community 13 - "event_types.py"
Cohesion: 0.01
Nodes (335): NPC bundle: lifecycle manager, spawning service, population controller. Depends…, Event bus for MythosMUD. This module provides the EventBus class that…, _default_timestamp(), NPCDied, NPCEnteredRoom, NPCLeftRoom, ObjectAddedToRoom, ObjectRemovedFromRoom (+327 more)

### Community 14 - "test_security_validator.py"
Cohesion: 0.01
Nodes (208): Validate target player name format using centralized validation., Validate pose description for security using centralized validation., field_validator, Validate character name format., Unit tests for security validation utilities. Tests the security validator…, Test that comprehensive sanitization removes null bytes., Test that comprehensive sanitization removes control characters., Test that comprehensive sanitization normalizes newlines to spaces. (+200 more)

### Community 15 - "useMythosAppState.ts"
Cohesion: 0.06
Nodes (53): CharacterNameScreenProps, MechanicalEffect, Profession, ProfessionCard(), ProfessionCardProps, StatRequirement, ProfessionSelectionContentProps, ProfessionSelectionScreen() (+45 more)

### Community 16 - "CombatParticipant"
Cohesion: 0.02
Nodes (164): CombatAction, CombatParticipant, Check if participant is mortally wounded (players only). For players: mortally…, Check if participant can perform voluntary combat actions. Unconscious (DP <=…, Apply damage to this participant and determine resulting death states.…, Represents a combat action., Represents a participant in combat., Check if participant is dead. For players: dead if DP <= -10 For NPCs: dead if… (+156 more)

### Community 17 - "test_message_handlers.py"
Cohesion: 0.03
Nodes (148): ChatMessageHandler, ClientErrorReportMessageHandler, CommandMessageHandler, FollowResponseMessageHandler, MessageHandler, MessageHandlerFactory, PartyInviteResponseMessageHandler, PingMessageHandler (+140 more)

### Community 18 - "JsonMap"
Cohesion: 0.05
Nodes (25): Msg, _as_json_map(), _NatsListenerClient, NatsMessageCallback, _NatsSubscribeFn, _NatsSubscription, JsonMap, Protocol (+17 more)

### Community 19 - "test_npc_definitions_api.py"
Cohesion: 0.05
Nodes (91): create_npc_definition(), delete_npc_definition(), get_npc_definition(), get_npc_definitions(), AsyncSession, delete, get, post (+83 more)

### Community 20 - "NPCDefinition"
Cohesion: 0.01
Nodes (268): _JSONDict, Random, Base, _loads_json_dict(), NPCDefinition, NPCRelationship, NPCSpawnRule, DeclarativeBase (+260 more)

### Community 21 - "ContainerComponent"
Cohesion: 0.01
Nodes (425): handle_close_container_exceptions(), handle_loot_all_exceptions(), handle_open_container_exceptions(), handle_transfer_items_exceptions(), Any, Exception, Request, UUID (+417 more)
...
### Community 24 - "PlayerRespawnService"
Cohesion: 0.05
Nodes (46): decode_liabilities(), encode_liabilities(), LiabilityStackEntry, Decode liability JSON into structured list., Serialize liability structures into JSON string., Reduce or remove a liability stack., Select the first liability not already applied, falling back to the first…, _PlayerCombatClearing (+38 more)

### Community 25 - "api/character_creation.py"
Cohesion: 0.02
Nodes (153): get_current_user(), Get current user with enhanced logging., _apply_rate_limiting_for_stats_roll(), _apply_stat_modifiers(), _as_float(), _as_int(), _check_shutdown_status(), _convert_stat_summary_to_stat_summary_model() (+145 more)

### Community 26 - "test_combat_event_publisher.py"
Cohesion: 0.02
Nodes (137): CombatEndedEvent, CombatStartedEvent, CombatTargetSwitchEvent, NPCAttackedEvent, NPCTookDamageEvent, PlayerAttackedEvent, Combat-specific events for the MUD. This module defines combat-related events…, Event fired when combat begins. (+129 more)

### Community 27 - "time.py"
Cohesion: 0.02
Nodes (144): _check_holiday_coverage(), _get_calendar_paths(), _load_and_validate_holidays(), load_document_ids(), main(), parse_args(), _print_errors(), _print_success_message() (+136 more)

### Community 28 - "CombatService"
Cohesion: 0.02
Nodes (123): Initialize the spell targeting service. Args: target_resolution_service:…, CombatService, DataProviderProtocol, _fallback_find_combat_id_for_npc(), find_participant_uuid_by_string_id(), get_combat_by_participant(), get_combat_id_for_npc(), get_combat_id_for_npc_via_mapping() (+115 more)

### Community 29 - "MemoryMonitor"
Cohesion: 0.03
Nodes (93): Return the live singleton without constructing one., get_engine(), Get the database engine from DatabaseManager. Returns: AsyncEngine: The…, peek_performance_monitor(), Return the live singleton, or None if it was never constructed. Idle sampling…, AllocSiteSample, _append_sample_jsonl(), _as_int() (+85 more)

### Community 30 - "PlayerService"
Cohesion: 0.01
Nodes (663): EffectHandler, create_dialogue_definition(), delete_dialogue_definition(), get_dialogue_definition(), list_dialogue_definitions(), delete, get, post (+655 more)

### Community 31 - "EventBus"
Cohesion: 0.01
Nodes (274): ModuleType, DistributedEventBus, Any, Distributed EventBus that uses NATS for cross-instance event distribution.…, EventBus that distributes domain events via NATS for horizontal scaling. When…, Initialize distributed EventBus. Args: nats_service: NATS service for…, Set NATS service and start the bridge (call after NATS connects)., Publish event locally and to NATS when bridge is active. (+266 more)

### Community 32 - "test_npc_admin_commands.py"
Cohesion: 0.03
Nodes (160): handle_npc_behavior_command(), handle_npc_react_command(), handle_npc_stop_command(), Any, NPC behavior control commands (behavior, react, stop)., Handle NPC behavior control command., Handle NPC reaction trigger command., Handle NPC behavior stop command. (+152 more)

### Community 33 - "async_load_zone_configurations"
Cohesion: 0.12
...
Cohesion: 0.03
Nodes (75): _AsyncPlayerGuardState, InventoryMutationGuard, _PlayerGuardState, Acquire sync mutation guard., Acquire async mutation guard., Get or create per-player guard state for sync contexts. Uses thread-safe…, Get or create per-player guard state for async contexts. Uses async lock to…, Clean up per-player guard state when no longer needed (sync context). Removes… (+67 more)

### Community 35 - "InventoryCommandFactory"
Cohesion: 0.03
Nodes (134): Unit tests for inventory command factory helper functions. Tests the helper…, Test create_equip_command() with item name and inferred slot., Test create_unequip_command() with slot., Test create_unequip_command() with item name., Test create_inventory_command() creates InventoryCommand., Test create_inventory_command() raises error with args., Test create_pickup_command() with numeric index., Test create_pickup_command() with quantity. (+126 more)
...
Cohesion: 0.03
Nodes (127): DatabaseManager, Thread-safe singleton for database management. Manages database engine, session…, Initialize the database manager., Get the singleton instance., Reset singleton for testing., Get the database URL, initializing if necessary. Returns: str: The database URL…, Any, Initialize the exploration service. Args: database_manager: Database manager… (+119 more)

### Community 38 - "ConnectionManager"
Cohesion: 0.03
Nodes (56): ConnectionManager, WebSocket, Check if a WebSocket is open., Check if a WebSocket ID is in the closed set., Mark a WebSocket ID as closed., Get the count of closed WebSocket IDs being tracked., Safely close a WebSocket connection., Resolve a room id to the canonical Room.id value (public method). (+48 more)
...
### Community 40 - "get_session_maker"
Cohesion: 0.02
Nodes (187): get_10_active_invites(), main(), Get 10 active invite codes from the database., get_session_maker(), async_sessionmaker, AsyncSession, Get the async session maker, initializing if necessary. Returns:…, Get the async session maker from DatabaseManager. Returns: async_sessionmaker:… (+179 more)

### Community 41 - "test_communication_commands_flows.py"
Cohesion: 0.06
Nodes (68): _chat_send_with_room_bundle(), _deliver_reply_to_last_whisper(), _deliver_whisper_message(), flow_global_command(), flow_local_command(), flow_reply_command(), flow_say_command(), flow_system_command() (+60 more)

### Community 42 - "ConnectionManager"
Cohesion: 0.06
Nodes (33): check_connection_health_impl(), ConnectionManager, get_connection_count_impl(), get_message_delivery_stats_impl(), get_pending_messages_impl(), get_player_session_impl(), get_player_websocket_connection_id_impl(), get_rate_limit_info_impl() (+25 more)

### Community 43 - "AsyncPersistenceLayer"
Cohesion: 0.02
Nodes (124): AsyncPersistenceLayer, datetime, Player, Profession, Room, Unpack, UUID, Close and cleanup resources. Note: SQLAlchemy async sessions are managed by the… (+116 more)

### Community 44 - "lifespan_magic.py"
Cohesion: 0.06
Nodes (37): _initialize_magic_service(), initialize_magic_services(), _initialize_mp_regeneration_service(), _initialize_spell_effects(), _initialize_spell_learning_service(), _initialize_spell_registry(), _initialize_spell_repositories(), _initialize_spell_targeting_service() (+29 more)

### Community 45 - "eventHandlers/types.ts"
Cohesion: 0.06
Nodes (65): handleCombatDeath(), handleCombatEnded(), handleCombatStarted(), handleCombatTargetSwitch(), handleNpcAttacked(), handleNpcDied(), handlePlayerAttacked(), eventHandlers (+57 more)

### Community 46 - "lifespan_startup.py"
Cohesion: 0.04
...
### Community 47 - "TargetResolutionService"
Cohesion: 0.02
Nodes (135): BaseModel, Target metadata schema for MythosMUD. This module defines Pydantic models for…, Metadata about a target in target resolution. This model represents additional…, TargetMetadata, StrEnum, Target resolution schemas for MythosMUD. This module defines Pydantic models…, Enumeration of possible target types., Result of target resolution operation. This model contains the complete result… (+127 more)

### Community 48 - "GameClientV2AuxiliaryPanels.tsx"
Cohesion: 0.05
Nodes (62): formatDelta(), HealthMeter, TIER_METADATA, TierMetadata, computeLucidityBar(), formatChange(), LucidityChangeFooter(), LucidityMeter (+54 more)

### Community 49 - "UtilityCommandFactory"
Cohesion: 0.03
...
### Community 50 - "ApplicationContainer"
Cohesion: 0.02
Nodes (197): ChatBundle, Chat bundle: chat service. Depends on Core (config, persistence), Game…, Initialize chat service., CombatBundle, Combat bundle: player combat, death, respawn, combat service, catatonia,…, Raise if prerequisites for NATS combat are missing., Create CombatService with NATS and register it. Assumes NATS is connected., Start NATS message handler if available. Logs and swallows errors. (+189 more)
...
Cohesion: 0.03
Nodes (114): handle_cast_command(), handle_learn_command(), handle_spell_command(), handle_spells_command(), handle_stop_command(), MagicCommandHandler, Any, Exception (+106 more)

### Community 53 - "test_command_parser.py"
Cohesion: 0.02
Nodes (115): Smoke test for command parser., Test basic command parsing., Test command parsing with arguments., Test command parsing with pipes., test_parse_command_basic(), test_parse_command_with_args(), test_parse_command_with_pipes(), command_parser() (+107 more)

### Community 54 - "test_websocket_helpers.py"
Cohesion: 0.04
Nodes (68): check_shutdown_and_reject(), get_npc_name_from_instance(), is_client_disconnected_exception(), is_websocket_disconnect_message(), load_player_mute_data(), BaseException, WebSocket, Load player mute data when they connect. AI: Uses async version to avoid… (+60 more)

### Community 55 - "BaseCommand"
Cohesion: 0.02
Nodes (77): BaseCommand, BaseModel, Base class for all MythosMUD commands. Provides common validation and security…, CommandFactory, Create StandCommand from arguments., Create LieCommand from arguments., Create GroundCommand from arguments., Create FollowCommand from arguments. (+69 more)

### Community 56 - "ChatHistoryPanel.tsx"
Cohesion: 0.04
Nodes (68): EldritchEffectsDemo(), EldritchEffectsDemoProps, mockAlert, ALWAYS_ACTIVE_EFFECTS, effectClass(), EffectOption, ELDRITCH_EFFECT_OPTIONS, hasEffect() (+60 more)

### Community 57 - "test_admin_auth_service.py"
Cohesion: 0.02
Nodes (114): AdminAuthService, AdminRole, AdminSession, Any, Request, Represents an admin session., Service for admin authentication and authorization., Initialize the admin auth service. (+106 more)

### Community 58 - "look_container.py"
Cohesion: 0.03
Nodes (104): _as_map(), _as_map_list(), _as_uuid(), _container_name(), _ContainerPersistence, _extract_container_metadata(), _fetch_container(), _find_container_in_room() (+96 more)
...
Cohesion: 0.05
Nodes (79): NPCCombatIntegration, Publish NPC attack event to event bus., Integrates NPCs with the existing combat and game mechanics systems. Extends…, Handle NPC death and related effects. Args: npc_id: ID of the dead NPC room_id:…, Get NPC stats or use defaults., integration(), asyncio, fixture (+71 more)
...
### Community 62 - "test_command_validator.py"
Cohesion: 0.03
Nodes (88): Unit tests for command validator., Test validate_command_length returns True for valid length., Test validate_command_length returns False for too long command., Test validate_command_length with custom max_length., Test validate_command_format returns True for valid command., Test validate_command_format returns False for empty command., Test validate_command_format returns False for suspicious command., Test validate_command_format returns False for too long command. (+80 more)

### Community 63 - "MythosTickScheduler"
Cohesion: 0.03
Nodes (92): HolidayResolver, Decayed corpse cleanup for the game tick loop., isolated_chronicle(), asyncio, fixture, Unit tests for TimeBundle container wiring., Calendar components and daypart helpers., Real/Mythos datetime conversion round-trips approximately. (+84 more)

### Community 64 - "test_inventory_helpers.py"
Cohesion: 0.03
...
### Community 67 - "ui-v2/types.ts"
Cohesion: 0.04
Nodes (88): PanelManager(), PanelManagerProps, minimapBackdropLayout(), MinimapPanelBackdrop(), MinimapPanelSection(), MinimapPanelSectionProps, ExpandedPanelBody(), ExpandedPanelBodyProps (+80 more)

### Community 68 - "CombatParticipantData"
Cohesion: 0.05
...
### Community 69 - "GameBundle"
Cohesion: 0.03
Nodes (62): LevelUpHook, GameBundle, Any, Exception, Game bundle: player, room, movement, exploration, user_manager,…, Wire user_manager into follow_service and nats_message_handler when present., Set item prototype registry on player service when both are available., Create room and profession cache services; set to None on RuntimeError. (+54 more)
...
### Community 71 - "magic_service.py"
Cohesion: 0.05
Nodes (54): CastingStateManager, Any, UUID, Casting state manager for tracking active spell castings. This module manages…, Check if a player is currently casting. Args: player_id: Player ID to check…, Get the casting state for a player. Args: player_id: Player ID Returns:…, Complete and remove a casting state. Args: player_id: Player ID Returns:…, Interrupt and remove a casting state. Args: player_id: Player ID Returns:… (+46 more)
...
Cohesion: 0.02
Nodes (155): CombatInstance, UUID, Represents an active combat instance., Get the participant whose turn it is., Advance to the next round - all participants act each round. In round-based…, Check if combat should end. CRITICAL: Combat should NOT end when a player is…, Get all participants that are not dead (includes mortally wounded players at 0…, Update the last activity tick and datetime. (+147 more)

### Community 74 - "test_player_death_service.py"
Cohesion: 0.03
...
Cohesion: 0.04
Nodes (44): Path, Generate room ID from parsed filename and location data. Args: parsed_filename:…, Recursively scan directory for all room JSON files. Args: base_path: Optional…, Validate basic room structure., Extract plane, zone, sub_zone from file path., Handles discovery and loading of room definition files. As noted in the…, Validate or update room ID based on filename and location., Validate required fields are present. (+36 more)

### Community 76 - "test_websocket_initial_state.py"
Cohesion: 0.03
Nodes (118): get_container_async_persistence(), Return the container-backed AsyncPersistenceLayer instance. Uses importlib for…, _accumulate_valid_occupant_name(), convert_uuids_to_strings(), get_occupant_names(), Validate that a name is not a UUID string., Parse one occupant row: append display name or log when it looks like a UUID., Extract and validate occupant names from room occupants list. (+110 more)

### Community 77 - "ContainerData"
Cohesion: 0.03
Nodes (89): ContainerData, ContainerDataCore, ContainerDataExtras, Container data class for persistence operations., Identity and placement fields for a container row., Optional payload and timestamps for a container row., Data class for container information., Convert container data to dictionary. Returns dictionary with model field names… (+81 more)
...
Cohesion: 0.04
Nodes (73): handle_admin_command(), _handle_admin_status_command(), _handle_admin_time_command(), Any, Expose current Mythos time metadata, active holidays, and freeze diagnostics., Entry point for general admin commands that expose subcommands like `admin…, Provide contextual status information about the caller's administrative…, handle_unmute_command() (+65 more)

### Community 80 - "real_time.py"
Cohesion: 0.03
Nodes (114): _app_state_from_request(), _app_state_from_websocket(), _ConnectionManagerUtilsModule, _ensure_connection_manager(), _extract_bearer_token(), get_connection_statistics(), get_player_connections(), handle_new_game_session() (+106 more)
...
Cohesion: 0.02
Nodes (127): PlayerRespawnEventHandler, BoundLogger, ConnectionManager, Player, Room, UUID, Player respawn event handlers. This module handles player respawn and delirium…, Get updated player data for respawn event. As documented in "Resurrection and… (+119 more)
...
### Community 84 - "connection_manager.py"
Cohesion: 0.02
Nodes (189): Cleanup method implementations for ConnectionManager. Thin wrappers that…, _async_callable(), cleanup_dead_websocket_impl(), _close_dead_websocket_if_open(), delegate_connection_cleaner(), delegate_connection_cleaner_sync(), delegate_game_state_provider(), delegate_game_state_provider_sync() (+181 more)
...
Cohesion: 0.03
Nodes (107): _filter_other_players(), _format_containers_section(), _format_exits_list(), _format_items_section(), _format_npcs_section(), _format_players_section(), _get_room_description(), _get_room_id() (+99 more)
...
Cohesion: 0.04
Nodes (100): Check if player is resting or in login grace period, interrupt rest if needed.…, Check if player is resting or in login grace period, interrupt rest if needed., If player is resting, cancel rest countdown so they can cast. Swallows errors…, _begin_seated_rest_countdown(), cancel_rest_countdown(), check_player_in_combat(), _check_rest_location(), _delayed_disconnect_player_intentionally() (+92 more)

### Community 89 - "NATSError"
Cohesion: 0.03
Nodes (72): NATSConnectionError, NATSError, NATSHealthCheckError, NATSPublishError, NATSSubscribeError, Exception, Base exception for all NATS-related errors., Raised when NATS connection operations fail. (+64 more)
...
### Community 91 - "player_presence_tracker.py"
Cohesion: 0.03
Nodes (111): _acquire_disconnect_lock(), broadcast_connection_message_impl(), _build_player_info(), _disconnect_during_rest_is_intentional(), _get_instance_manager_from_manager(), _GraceReconnectManager, Any, Player (+103 more)

### Community 92 - "look_helpers.py"
Cohesion: 0.03
Nodes (112): _AppWithState, _async_persistence_from_app(), _ContainerWithPersistence, _EquippedPlayer, _get_corruption_label(), _get_corruption_prose(), _get_health_label(), _get_lucidity_label() (+104 more)

### Community 93 - "NPCOccupantProcessor"
Cohesion: 0.04
Nodes (63): NPCOccupantProcessor, NPC occupant processing utilities. This module handles querying and processing…, Processes NPC occupants for rooms., Initialize NPC occupant processor. Args: connection_manager: ConnectionManager…, Any, Room ID normalization and comparison utilities. This module provides utilities…, Check if NPC room IDs match target room IDs using fallback comparison. Args:…, Check if NPC room matches target room using normalized comparison. Args:… (+55 more)

### Community 94 - "ContainerService"
Cohesion: 0.08
Nodes (73): ContainerService, Service for managing container operations. Orchestrates open/close, transfer…, MutationDecision, Result of attempting to acquire a guarded mutation context., Test get_container_service function., Test get_container_service returns ContainerService instance., TestGetContainerService, _allow_all_mutations() (+65 more)

### Community 95 - "chat_service.py"
Cohesion: 0.05
Nodes (73): ChatEmoteService, ChatLogger, ChatPlayerService, ChatRateLimiter, ChatUserManager, Protocol, Return True if command is a predefined emote alias., Return (self_message, other_message) for a predefined emote command. (+65 more)

### Community 96 - "TestMonitoringEndpoints"
Cohesion: 0.07
...
Cohesion: 0.05
Nodes (68): get_message_validator(), MessageValidationError, Exception, WebSocketInboundMessage, WebSocket message validation for MythosMUD. This module provides comprehensive…, Calculate the maximum nesting depth of a JSON structure. Args: obj: Object to…, Validate that strings in the JSON structure don't exceed length limits. Args:…, Ensure a message carries a recognizable top-level shape. Returns: bool: True if… (+60 more)

### Community 98 - "CombatAttackHandler"
Cohesion: 0.03
Nodes (80): _AppStateWithConnectionManager, _AppWithState, CombatAttackHandler, _CombatAttackService, Protocol, UUID, Apply damage to target and check death states. Delegates domain logic to…, Check if room has no_death attribute (tutorial/safe zones). (+72 more)

### Community 99 - "test_websocket_handler_core.py"
Cohesion: 0.03
...
Cohesion: 0.04
Nodes (91): NPCSpoke, Event fired when an NPC speaks. This event is triggered when an NPC…, _ChatDeliveryService, deliver_fake_npc_whisper(), deliver_npc_room_speech(), deliver_personal_system(), npc_sender_id(), _on_npc_spoke() (+83 more)

### Community 101 - "CorruptionService"
Cohesion: 0.05
Nodes (73): CleanseApp, CleanseAppState, CleanseContainer, CleansePersistence, CleanseRequest, CleanseTargetPlayer, _format_cooldown_message(), handle_cleanse_command() (+65 more)

### Community 102 - "test_movement_service.py"
Cohesion: 0.03
Nodes (104): check_combat_state(), check_player_posture(), extract_player_id(), Any, Room, UUID, Movement validation helpers for MovementService. Cohesive validation and room-…, Validate player is in the from_room, auto-adding if database matches. (+96 more)
...
### Community 104 - "RoomService"
Cohesion: 0.01
Nodes (368): _MapRooms, RoomDictList, MapZoneContext, NamedTuple, Plane, zone, and sub_zone grouped for map/minimap APIs to reduce parameter…, _append_room_with_fallback_coords_if_needed(), _apply_minimap_fallback_coordinates(), _ensure_current_room_in_minimap_rooms() (+360 more)

### Community 105 - "combat_service.py"
Cohesion: 0.01
Nodes (218): NPCDiedEvent, Event fired when an NPC dies., CombatParticipantType, CombatResult, CombatStatus, _get_default_damage(), Enum, Combat system models for in-memory state management. This module defines the… (+210 more)

### Community 106 - "ScheduleEntry"
Cohesion: 0.02
Nodes (99): Initialize the Temporal context: holiday/schedule/tick-scheduler, then the…, get_asyncpg_server_settings_for_database_url(), Build asyncpg ``server_settings`` so unqualified table names resolve like…, MythosHourTickEvent, Event fired when the accelerated Mythos clock rolls over to a new hour., Record the schedule categories currently active for NPC routines., Any, field_validator (+91 more)
...
Cohesion: 0.06
Nodes (65): ChatResult, _append_channel_history(), _authorize_global_sender(), _authorize_system_sender(), ChatPlayerView, ChatSendServices, _load_whisper_participants(), _log_and_store_system_message() (+57 more)

### Community 109 - "NATSSubjectManager"
Cohesion: 0.01
Nodes (253): get_subject_manager_dependency(), Dependency function to inject NATSSubjectManager. Returns: Global…, Initialize combat event publisher. Args: nats_service: NATS service instance…, InvalidPatternError, MissingParameterError, NATSSubjectError, PatternNotFoundError, Exception (+245 more)

### Community 110 - "test_npc_utils.py"
Cohesion: 0.03
Nodes (92): Check if this NPC is required to spawn., Prefer live NPC room attrs, then lifecycle SPAWNED/left event room_id., _resolve_despawn_room_id(), extract_definition_id_from_npc(), extract_npc_metadata(), extract_room_id_from_lifecycle_record(), extract_room_id_from_npc(), get_zone_key_from_room_id() (+84 more)

### Community 111 - "Alias"
Cohesion: 0.04
Nodes (65): Alias, Any, BaseModel, Alias model for command aliases. Stores player command aliases for quick access…, String representation of the alias., Check equality based on name and command., Hash based on name and command for use in sets/dicts., Update the updated_at timestamp to current time. (+57 more)

### Community 112 - "command_result_text"
Cohesion: 0.04
...
### Community 113 - "handle_whisper_command"
Cohesion: 0.06
Nodes (53): handle_reply_command(), handle_whisper_command(), Reply to last whisper sender., Shared mock wiring for communication command unit tests., asyncio, Unit tests for whisper and reply communication command handlers., Test handle_whisper_command successful execution., Test handle_reply_command with no message. (+45 more)

### Community 114 - "is_player_in_login_grace_period"
Cohesion: 0.04
Nodes (89): _as_grace(), cancel_login_grace_period(), _EffectPersistence, get_login_grace_period_remaining(), _grace_period_task(), _GraceApp, _GraceAppState, _GraceContainer (+81 more)
...
Cohesion: 0.04
Nodes (65): AttackCommand, KickCommand, PunchCommand, field_validator, Command for attacking a target., Validate combat target name format using centralized validation., Command for punching a target., Validate combat target name format using centralized validation. (+57 more)
...
### Community 118 - "command_service.py"
Cohesion: 0.07
Nodes (40): Command service for MythosMUD. This module provides the main command processing…, Help command adapter module. The original help command handler lives in…, Command processing system for MythosMUD. This package provides the command…, _get_position_command_services(), handle_lie_command(), _handle_position_change(), handle_sit_command(), handle_stand_command() (+32 more)

### Community 119 - "PlayerPositionService"
Cohesion: 0.04
Nodes (66): PlayerPositionService, PositionChangeResponse, PositionPlayer, TypedDict, Player posture coordination service for MythosMUD. As noted in the Pnakotic…, Validate and normalize position., Get player for position change. Returns: Tuple of (player, response_dict) if…, Copy player identity fields into the position-change response. (+58 more)

### Community 120 - "npc_database.py"
Cohesion: 0.04
...
Cohesion: 0.03
Nodes (140): _apply_room_exit_to_memory(), _apply_room_properties_to_memory(), _build_exit_attributes(), create_room_exit(), _create_room_link_in_db(), delete_room_exit(), _delete_room_link_in_db(), _invalidate_room_cache() (+132 more)

### Community 122 - "test_container_helpers_inventory_find.py"
Cohesion: 0.05
Nodes (91): check_item_matches_target(), _component_metadata(), _container_from_equip_dict(), _container_uuid(), create_wearable_container(), _fallback_create_equipment_container(), find_container_in_room(), find_item_in_inventory() (+83 more)

### Community 123 - "NPCDefinitionCRUDMixin"
Cohesion: 0.06
Nodes (38): NPCDefinitionCRUDMixin, Any, AsyncSession, Execute create_npc_definition stored procedure and return the created…, Validate create_npc_definition parameters. Raises ValueError if invalid., Log successful NPC definition creation., Validate NPC update parameters., Add a simple field to update_data if value is not None. (+30 more)
...
### Community 127 - "test_look_container.py"
Cohesion: 0.03
Nodes (115): ContainerLookArgs, _find_container_in_room_or_equipped(), _find_container_via_inner_container(), _format_container_display(), _get_container_description(), _handle_container_look(), CommandResponse, NamedTuple (+107 more)
...
### Community 129 - "PlayerCombatService"
Cohesion: 0.01
Nodes (208): CombatCommandHandler, CombatCommandHandlerExtras, _NpcWithLife, Any, AppWithState, ConnectionManager, Protocol, Combat command handler class and shared helpers. Extracted from combat.py to… (+200 more)
...
Cohesion: 0.03
Nodes (63): _as_user_dict(), _check_all_command_blocks(), _check_rate_limit(), _ensure_alias_storage(), _handle_special_command_routing(), _prepare_command_for_processing(), _process_alias_expansion(), process_command() (+55 more)
...
### Community 135 - "test_connection_disconnection.py"
Cohesion: 0.04
Nodes (87): _apply_disconnect_side_effects(), _cleanup_connection_tracking(), _cleanup_fully_disconnected_player(), _cleanup_player_data(), _cleanup_room_subscriptions(), cleanup_websocket_disconnect(), _close_and_untrack_websockets(), disconnect_all_websockets_impl() (+79 more)

### Community 136 - "disconnect_grace_period.py"
Cohesion: 0.04
Nodes (80): cancel_grace_period(), _grace_period_seconds(), is_player_in_grace_period(), _PlayerLookupManager, Any, Player, Protocol, UUID (+72 more)

### Community 137 - "test_aggro_threat.py"
Cohesion: 0.04
Nodes (97): add_damage_threat(), add_heal_threat(), _aggression_scale(), apply_stealth_wipe(), apply_taunt(), _corruption_scale(), _get_aggro_config(), get_npc_current_target() (+89 more)
...
Cohesion: 0.05
Nodes (81): _app_from_request(), _as_response(), _connection_manager_from_app(), _container_from_app(), _get_app_and_persistence(), _get_room_drops(), _handle_implicit_target_lookup(), handle_look_command() (+73 more)

### Community 140 - "SkillRepository"
Cohesion: 0.04
Nodes (72): get_skills_catalog(), get, Request, Return the skills catalog (base values, allow_at_creation). Cthulhu Mythos is…, SkillService: skills catalog, set_player_skills, get_player_skills (with…, PlayerSkill, Base, PlayerSkill model: per-character skill values. Links a player to a skill with a… (+64 more)

### Community 141 - "test_population_control.py"
Cohesion: 0.02
Nodes (147): Get population statistics for a given zone. Args: zone_key: Zone key in format…, PopulationStats, Any, Population Statistics Module. This module provides the PopulationStats class…, Statistics for NPC population in a zone or sub-zone., Initialize population statistics. Args: zone_id: The zone identifier…, Add an NPC to the population statistics. Args: npc_type: Type of the NPC…, Remove an NPC from the population statistics. Args: npc_type: Type of the NPC… (+139 more)
...
### Community 145 - "run_flee_effect"
Cohesion: 0.08
Nodes (49): _flee_effect_failure_response(), _flee_effect_invalid_target_response(), _flee_effect_invalid_target_type_response(), _flee_effect_not_in_combat_response(), _flee_effect_room_error_response(), _flee_effect_services_available(), _flee_effect_services_unavailable_response(), _flee_effect_success_response() (+41 more)

### Community 146 - "test_logout_commands.py"
Cohesion: 0.03
Nodes (111): _clear_corrupted_cache_entry(), _coerce_player_uuid(), _disconnect_player_connections(), _force_disconnect_player(), _get_app_services(), _get_player_for_logout(), _get_player_position_from_connection_manager(), handle_logout_command() (+103 more)

### Community 147 - "hallucinations.py"
Cohesion: 0.06
Nodes (46): deliver_forced_hallucination(), Deliver the requested hallucination type via the real handler functions., FakeHallucinationService, FakeNpcTellData, TypedDict, UUID, Generate a room text overlay hallucination. Args: player_id: Player UUID who…, Select which type of fake hallucination to trigger (50/50 chance). Returns:… (+38 more)

### Community 148 - "test_database_helpers.py"
Cohesion: 0.04
Nodes (80): close_db(), ensure_database_directory(), get_async_session(), get_database_path(), get_database_url(), get_engine(), get_session_maker(), init_db() (+72 more)

### Community 149 - "test_connection_helpers_impl.py"
Cohesion: 0.05
Nodes (62): broadcast_global_event_impl(), broadcast_room_event_impl(), handle_new_login_impl(), _optimize_payload(), Any, _queue_message_if_needed(), Helper utilities for connection manager. This module provides utility functions…, Queue message for later delivery if no active connections. Args: player_id: The… (+54 more)
...
### Community 153 - "test_config_init.py"
Cohesion: 0.07
Nodes (33): Any, Initialize configuration and set environment variables for legacy compatibility., Return first set CORS origins env var to reduce CCN in _sanitize., Normalize environment variables so nested configs can parse them reliably., MonkeyPatch, parametrize, Unit tests for config module initialization., A CORS origins env var that already looks like JSON is left untouched. (+25 more)
...
### Community 159 - "LoggedHTTPException"
Cohesion: 0.02
Nodes (221): get_patterns(), get_subject_statistics(), PatternsResponse, BaseModel, get, post, NATS Subject Management API Controller for MythosMUD. This module provides REST…, Dependency to require admin permissions. Args: current_user: Current… (+213 more)

### Community 160 - "test_teleport_helpers.py"
Cohesion: 0.05
Nodes (82): broadcast_teleport_effects(), get_online_player_by_display_name(), notify_player_of_teleport(), Any, Teleport utility functions for admin commands in MythosMUD. This module…, Notify a player that they are being teleported by an admin. Args:…, Get online player information by display name. Args: display_name: Display name…, Broadcast teleport visual effects to players in affected rooms. Args:… (+74 more)

### Community 161 - "FollowService"
Cohesion: 0.04
Nodes (81): FollowService, If follower is sitting or prone, try to stand them so they can move., Move followers when the followed NPC moves., In-memory follow state and movement propagation. Subscribes to…, connection_manager(), event_bus(), follow_service(), movement_service() (+73 more)

### Community 162 - "player.ts"
Cohesion: 0.09
Nodes (38): locationIndicatesDeathVoid(), requiredAliveButDeadMessage(), assertLookVisibleInPanels(), lookAndStand(), prepAwForAdminSet(), prepNonAdminForSetAttempt(), runAdminSetWithRecovery(), assertNpcSpawnVisible() (+30 more)

### Community 163 - "admin_setstat_command.py"
Cohesion: 0.05
Nodes (69): _apply_stat_change_and_build_result(), _execute_admin_set_stat(), _maybe_attach_dp_posture_message(), _mutate_player_stat(), _notify_player_stat_change(), UUID, Admin command to set player statistics. This module provides the handler for…, Apply DP or generic stat mutation; return previous posture when DP changes. (+61 more)

### Community 164 - "ChatModeration"
Cohesion: 0.04
Nodes (43): ChatModeration, normalize_player_id(), PlayerServiceProtocol, Any, datetime, Protocol, UUID, Chat moderation utilities. This module provides moderation functionality… (+35 more)

### Community 165 - "QuestService"
Cohesion: 0.05
Nodes (58): Quest subsystem: service, goal progression, rewards., _build_collect_n_progress(), _call_add_item_to_inventory(), _collect_goal_prototype_id(), _collect_goal_required_count(), _consume_collect_goals_from_player(), _definition_completion_mode_error(), _goal_activity_target() (+50 more)

### Community 166 - "test_combat_schema.py"
Cohesion: 0.08
Nodes (41): Draft7Validator, CombatSchemaValidationError, get_combat_stats_summary(), Exception, Combat system JSON schema validation. This module provides JSON schema…, Raised when combat data fails schema validation., Validate base_stats combat data against schema. Args: data: Base stats…, Validate behavior_config combat data against schema. Args: data: Behavior… (+33 more)

### Community 167 - "deleteCharacterFlow.ts"
Cohesion: 0.11
Nodes (41): getCreateCharacterErrorMessage(), errorMessageFromApiBody(), parseSelectCharacterResult(), pickString(), requestDeleteCharacter(), resolveSelectedId(), CreationRefreshResult, messageFromCreationRefreshHttpError() (+33 more)

### Community 168 - "test_go_command.py"
Cohesion: 0.04
Nodes (79): Exploration commands for MythosMUD. This module contains handlers for…, _cancel_rest_if_moving(), _canonical_room_id_for_go(), _connection_manager_from_go_app(), _execute_movement(), handle_go_command(), _movement_combat_and_event_bus_from_go_app(), _movement_service_for_go_command() (+71 more)

### Community 169 - "RateLimiter"
Cohesion: 0.03
...
### Community 170 - "room_update_event_builder.py"
Cohesion: 0.06
Nodes (43): build_room_update_event(), ConnectionManager, Room, Room-update event construction, shared by the broadcast and per-viewer fan-out…, Build room update event with room data and occupants (players/npcs for…, get_hallucinated_exits(), _hash_string(), mulberry32() (+35 more)
...
Cohesion: 0.03
Nodes (68): Initialize monitoring services. Depends on Core/Realtime/Game for injected deps., MemoryLeakMetricsCollector, Any, Memory leak metrics collector for MythosMUD. This module provides comprehensive…, Collect event metrics from EventBus. Returns: Dictionary with event metrics, Collect cache metrics from CacheManager. Returns: Dictionary with cache metrics, Collect task metrics from TaskRegistry. Returns: Dictionary with task metrics, Collect NATS subscription metrics from NATSService. Returns: Dictionary with… (+60 more)

### Community 174 - "MessageQueue"
Cohesion: 0.03
...
### Community 175 - "test_inventory_display_helpers.py"
Cohesion: 0.07
...
### Community 177 - "test_command_admin.py"
Cohesion: 0.03
...
### Community 178 - "inventory_get_command.py"
Cohesion: 0.13
...
Cohesion: 0.05
Nodes (75): Logger, Queue, QueueHandler, QueueListener, add_handler_to_loggers(), LoggerNameFilter, LogRecord, Filter that only allows logs from loggers matching specified prefixes. This… (+67 more)

### Community 180 - "follow_service.py"
Cohesion: 0.04
Nodes (46): ConnectionManager, FollowTargetValue, UserManager, UUID, Follow service for MythosMUD. In-memory follow state: who is following whom…, Fire-and-forget; close coro if no running event loop (e.g. sync unit tests)., Send a command_response-style message to a single player., Send follow_state event so client can update title panel (who I am following). (+38 more)

### Community 181 - "item_instance_persistence_async.py"
Cohesion: 0.04
Nodes (67): CreateItemInstanceInput, EnsureItemInstanceInput, TypedDict, Constants and shared types for async persistence layer. Extracted to keep…, Optional fields for create_item_instance. owner_type, owner_id, etc. with…, Optional fields for ensure_item_instance., Initialize the async persistence layer. This facade delegates to focused async…, Create a new item instance. Delegates to ItemRepository. (+59 more)

### Community 182 - "catatonia_check.py"
Cohesion: 0.04
Nodes (54): check_catatonia_block(), _check_catatonia_database(), _check_catatonia_registry(), _convert_player_id_to_uuid(), _fetch_lucidity_record(), _is_catatonic(), _load_player_for_catatonia_check(), _PersistenceGetPlayerByName (+46 more)
...
Cohesion: 0.09
Nodes (49): Process termination utilities for graceful server shutdown. This module handles…, Schedule a best-effort graceful process termination after a short delay. This…, schedule_process_termination(), _cancel_background_tasks(), _cleanup_connection_manager(), _despawn_all_npcs(), _disconnect_all_players(), _disconnect_nats_service() (+41 more)

### Community 185 - "Test Player Disconnect Handlers"
Cohesion: 0.04
...
### Community 188 - "Stats"
Cohesion: 0.10
Nodes (16): _ProfessionStatRequirementsSource, Protocol, Stats, Roll character stats using the specified method. Args: method: Rolling method…, Roll Size using formula: (2D6+6)*5 (range 40-90)., Roll stats using 3d6 method (scaled to 15-90 range)., Roll stats using 4d6 drop lowest method (more generous, scaled to 15-90 range)., Generate stats using a point-buy system (balanced, scaled to 1-100 range). (+8 more)
...
Cohesion: 0.02
Nodes (94): correct_api_logging(), correct_basic_logging(), correct_batch_logging(), correct_database_logging(), correct_error_handling(), correct_exception_tracking(), correct_performance_logging(), correct_security_logging() (+86 more)

### Community 191 - "executeCommand"
Cohesion: 0.11
Nodes (37): prepCoLocatedContexts(), expectWhoListingOnPage(), primeBothForCoLocate(), waitForLookReflected(), executeUnmuteAndWaitForAck(), primeBothForCoLocate(), executeUnmuteAndWaitForAck(), nudgeStandBothPlayers() (+29 more)
...
Cohesion: 0.06
Nodes (74): _apply_lucidity_change(), check_admin_permissions(), _execute_lucidity_change(), _extract_command_args(), _get_catatonia_registry_from_app(), get_current_lcd(), get_player_service_from_app(), _handle_admin_set_lucidity_command() (+66 more)

### Community 194 - "OccupantFormatter"
Cohesion: 0.04
...
Cohesion: 0.04
Nodes (98): _format_cooldown_message(), _format_recovery_success_message(), handle_folk_tonic_command(), handle_group_solace_command(), handle_meditate_command(), handle_pray_command(), handle_therapy_command(), _perform_recovery_action() (+90 more)
...
Cohesion: 0.05
Nodes (78): _dispatch_player_event(), _format_liabilities(), LucidityChangeEventExtras, LiabilityStackEntry, UUID, Helpers for broadcasting lucidity-related SSE events., Emit a catatonia state event to the affected player., Send rescue progress/status updates to either participant. (+70 more)

### Community 201 - "PlayerStateCommandFactory"
Cohesion: 0.04
Nodes (63): Unit tests for player state command factories. Tests the…, Test create_cleanse_command() raises error with args., Test create_skills_command() creates SkillsCommand., Test create_skills_command() raises error with args., Test create_journal_command() creates JournalCommand., Test create_journal_command() raises error with args., Test create_quests_command() creates QuestsCommand., Test create_status_command() creates StatusCommand. (+55 more)

### Community 202 - "test_config_models.py"
Cohesion: 0.03
Nodes (89): GameConfig, BaseSettings, field_validator, Game-specific configuration., Validate combat tick interval., Validate combat timeout., Validate combat XP multiplier., Validate combat alert threshold. (+81 more)
...
### Community 204 - "test_look_npc_helpers.py"
Cohesion: 0.04
...
### Community 208 - "PlayerDPUpdated"
Cohesion: 0.03
Nodes (94): PlayerDPUpdated, PlayerXPAwardEvent, Event fired when a player's DP changes. This event is triggered when a player…, Event published when a player receives XP., Handle player DP update events by sending updates to the client. Args: event:…, _attach_dp_updated_posture_fields(), _decay_previous_position_before_lying(), _dispatch_player_dp_decay_payload() (+86 more)
...
Cohesion: 0.06
Nodes (46): LogEntry, aggregate_log_entry(), LogAggregator, LogEntry, LogQueryFilter, _optional_datetime_from_object(), _optional_str_from_object(), datetime (+38 more)

### Community 211 - "admin_hallucinate_command.py"
Cohesion: 0.07
Nodes (45): extract_args(), get_current_lcd_or_error(), HallucinateCommandError, _handle_admin_hallucinate_command(), _log_hallucinate_command(), Exception, Request, UUID (+37 more)

### Community 212 - "LoggingConfig"
Cohesion: 0.07
Nodes (38): LoggingConfig, BaseSettings, field_validator, Security and logging configuration models., Validate admin password strength (production only)., Logging configuration., Validate logging environment., configure_enhanced_structlog() (+30 more)
...
### Community 215 - "test_room_renderer.py"
Cohesion: 0.04
...
Cohesion: 0.02
Nodes (159): _DropResolved, _FloorPickupResolved, Protocol, Narrows room managers for floor drop operations (pickup / get room)., RoomDropManager, add_pickup_to_inventory(), broadcast_room_event(), build_and_broadcast_inventory_event() (+151 more)
...
### Community 218 - "equipment_helpers.py"
Cohesion: 0.09
...
### Community 219 - "CommunicationCommandFactory"
Cohesion: 0.05
...
Cohesion: 0.06
Nodes (43): buildHeaders(), buildMapUrl(), fetchAsciiMap(), FetchAsciiMapParams, fetchAsciiMinimap(), FetchAsciiMinimapParams, formatDetailMessage(), formatMapErrorResponse() (+35 more)
...
### Community 222 - "test_database_extended.py"
Cohesion: 0.05
Nodes (41): get_database_path(), get_database_url(), Path, Get the database file path. DEPRECATED: PostgreSQL does not use file paths.…, Get the database file path (deprecated for PostgreSQL). Returns: Path | None:…, Get the database URL from DatabaseManager. Returns: str | None: The database URL, Extended unit tests for database module. Tests database initialization, error…, Test DatabaseManager.get_engine initializes if not initialized. (+33 more)
...
Cohesion: 0.04
Nodes (69): NPCThreadManager, Stop a specific NPC thread. Args: npc_id: Unique identifier for the NPC…, Internal method to stop an NPC thread., Get list of active NPC thread IDs., Get NPC definition for a specific NPC., Process a message for an NPC., Resolve active NPC instance and definition for a WANDER action., Parse NPC behavior config from instance attribute (dict or JSON string). (+61 more)
...
### Community 228 - "Room"
Cohesion: 0.02
Nodes (147): AsyncPersistenceRoomFacade, _AsyncPersistenceRoomFacadeBase, AsyncSession, Room-cache loading and RoomCacheLoader test hooks for AsyncPersistenceLayer., Delegate to room loader; exposed for unit tests., Delegate to room loader; exposed for unit tests., Delegate to room loader; exposed for unit tests., Attrs provided by AsyncPersistenceLayer when mixed in. (+139 more)
...
### Community 231 - "generate_invites_db.py"
Cohesion: 0.19
...
### Community 233 - "PlayerEventHandler"
Cohesion: 0.03
Nodes (86): OccupantSnap, PlayerEventHandler, ChatLogger, ConnectionManager, JsonMap, UUID, Handle player entering a room with enhanced synchronization. Args: event: The…, Handle player leaving a room with enhanced synchronization. Args: event: The… (+78 more)

### Community 234 - "HealthStatus"
Cohesion: 0.02
Nodes (144): HealthStatus, ConnectionsComponent, DatabaseComponent, HealthComponents, HealthErrorResponse, HealthResponse, HealthStatus, BaseModel (+136 more)
...
### Community 238 - "communication_commands.py"
Cohesion: 0.10
Nodes (40): handle_me_command(), handle_pose_command(), handle_say_command(), Communication commands for MythosMUD. Handlers delegate heavy logic to…, Room-wide say; returns user-facing result dict., Set or clear persistent pose text., Return (request, container) with request.app.state.container wired. Typed…, request_with_app_container() (+32 more)

### Community 239 - "NATSRetryHandler"
Cohesion: 0.01
Nodes (218): DeadLetterMessage, DeadLetterQueue, Any, Path, Dead Letter Queue for failed NATS messages. Stores messages that fail after all…, Add failed message to dead letter queue (async version). Args: message: Dead…, Add failed message to dead letter queue (sync version). Args: message: Dead…, Retrieve and remove oldest message from DLQ (async version). Returns: Message… (+210 more)

### Community 240 - "UserManager"
Cohesion: 0.06
Nodes (38): UUID, Check if a player is globally muted by any other player. Args: player_id:…, Get information about who muted a player. Args: player_id: Player ID to check…, Add a player as an admin. Args: player_id: Player ID player_name: Player name…, Get the mute data file path for a specific player., Convert timestamp strings in mute_info to datetime objects., Convert UUID strings in mute_info to UUID objects., Load player mutes from JSON data into memory. (+30 more)
...
### Community 242 - "_MagicServiceCore"
Cohesion: 0.06
...
Cohesion: 0.03
Nodes (83): AddAdminCommand, AdminCommand, MuteCommand, MuteGlobalCommand, MutesCommand, field_validator, Moderation command models for MythosMUD. This module provides command models…, Command for showing current mute status. (+75 more)
...
### Community 246 - "ModerationCommandFactory"
Cohesion: 0.05
...
Cohesion: 0.08
Nodes (41): GameClientV2Container(), getEmptyOccupantsReportContextOrNull(), isWithinRoomOccupantsSettleGracePeriod(), runEmptyOccupantsReportIfNeeded(), tryGetRoomWithEmptyOccupantsList(), forceLogoutFallback(), performGameClientLogout(), stillShowingGameClient() (+33 more)
...
### Community 249 - "test_nats_retry_handler.py"
Cohesion: 0.04
...
### Community 250 - "test_lucidity_service.py"
Cohesion: 0.07
...
Cohesion: 0.07
Nodes (49): nudgeStandBothPlayers(), nudgeStandBothPlayers(), loginAdminPlayable(), assertCommandChannelReady(), ensurePlayableConnection(), EnsurePlayableConnectionOptions, getLivePageForUsername(), isPageConnected() (+41 more)
...
Cohesion: 0.05
Nodes (55): PerformanceStats, ExceptionStats, Get exception tracking statistics. Returns: Current exception statistics, Statistics for exception tracking., Initialize the exception tracker. Args: max_records: Maximum number of…, __getattr__(), Any, Monitoring package for MythosMUD server. (+47 more)

### Community 254 - "combat_taunt.py"
Cohesion: 0.05
Nodes (54): AppWithState, Protocol, Shared Starlette/FastAPI-shaped protocols for combat command modules. Keeps…, Application object with a ``state`` namespace (dynamic attributes)., _apply_taunt_and_maybe_broadcast(), AppWithState, Protocol, UUID (+46 more)
...
### Community 256 - "systemHandlers.ts"
Cohesion: 0.11
Nodes (31): appendDaypartChange(), appendHolidayChange(), appendHourChime(), handleIntentionalDisconnect(), handleLucidityChange(), handleMythosTimeUpdate(), handleRescueUpdate(), parseMythosHour() (+23 more)

### Community 257 - "test_websocket_handler_helpers_extended.py"
Cohesion: 0.04
Nodes (60): mock_connection_manager(), mock_validator(), mock_websocket(), asyncio, fixture, MockerFixture, Extended unit tests for websocket handler helper functions. Tests additional…, Test _send_error_response() returns True when sent successfully. (+52 more)
...
### Community 264 - "NPCCombatIntegrationBase"
Cohesion: 0.06
Nodes (32): NPCCombatIntegrationBase, ABC, Exception, UUID, ValidationError, Base segment of NPC combat integration (damage, effects, attack orchestration).…, Apply combat effects to a target (player or NPC). Args: target_id: ID of the…, Convert target_id to UUID, accepting either string or UUID input. (+24 more)

### Community 265 - "useMythosAppActions.ts"
Cohesion: 0.11
Nodes (32): postSelectCharacter(), runAfterCharacterCreatedFlow(), isGracePeriodServerUnavailableError(), tryStartLoginGracePeriod(), AppActions, AppState, buildActionViewModel(), buildMythosAppViewModel() (+24 more)
...
### Community 267 - "test_command_helpers.py"
Cohesion: 0.04
...
### Community 270 - "admin_commands.py"
Cohesion: 0.09
Nodes (39): Administrative commands for MythosMUD. This module contains the main admin…, _collect_mute_display_lines(), _extract_mute_target(), _format_mute_line(), handle_add_admin_command(), handle_mute_command(), handle_mute_global_command(), handle_mutes_command() (+31 more)

### Community 271 - "_is_predefined_emote"
Cohesion: 0.11
...
Cohesion: 0.08
Nodes (53): _async_load_lucidity_rate_overrides(), build_override_key(), extract_lucidity_rate(), load_lucidity_rate_overrides(), _LucidityRateLoadResult, _normalize_database_url(), _parse_special_rules_from_raw(), _parse_zone_stable_id() (+45 more)

### Community 273 - "resolve_lazy_attr"
Cohesion: 0.06
Nodes (42): _ConnectionManagerAPI, Protocol, UUID, Send a room event to all players in a specific room. Args: room_id: The room's…, Send a system notification to a player. Args: player_id: The player's ID…, Send a player status update to a player. Args: player_id: The player's ID…, Send room description to a player. Args: player_id: The player's ID room_data:…, Structural type for API helpers; avoids importing ConnectionManager. (+34 more)
...
Cohesion: 0.09
Nodes (45): formatNpcAttackedLine(), formatNpcTookDamageLine(), formatPlayerAttackedLine(), mergePlayerDpFromPlayerAttackedPayload(), messageHandlers, appendPostureGameInfoMessage(), mergePostureMessageIntoState(), mergeRespawnedPlayer() (+37 more)

### Community 276 - "WebSocketRequestContext"
Cohesion: 0.07
...
### Community 277 - "MovementService"
Cohesion: 0.05
Nodes (37): MovementService, Any, Exception, Room, UUID, Validate movement parameters. Returns False if validation fails (same room),…, Resolve player by ID or name and return player object and resolved ID., Get and validate rooms for movement. (+29 more)

### Community 278 - "build_event"
Cohesion: 0.01
Nodes (216): Current FSM state as a single State. Uses python-statemachine 3.x configuration…, build_event(), _get_next_global_sequence(), Protocol, UUID, Event envelope utilities for MythosMUD real-time messages. Provides a single,…, Minimal typing for connection_manager passed to build_event (sequence_counter…, Custom JSON encoder that handles UUID objects. (+208 more)

### Community 279 - "NATSMessageBroadcastMixin"
Cohesion: 0.05
Nodes (31): Any, Extract information from chat event. Args: chat_event: Chat event dictionary…, Determine if mute check should be applied for a channel. Args: channel: Channel…, Initialize message filtering helper. Args: connection_manager:…, Check if a receiving player has muted the sender using a provided UserManager…, Check if a player has muted the sender. Args: user_manager: UserManager…, Filter target players based on room location and mute status. Args: targets:…, NATSMessageBroadcastMixin (+23 more)

### Community 280 - "_weapon_damage_from_equipped_player"
Cohesion: 0.29
Nodes (7): _attacker_stats_dict_from_full_player(), _get_combat_container_services(), PrototypeRegistry, Normalize full_player.get_stats() to a dict for damage math., Resolve rolled damage and type from main-hand weapon, or unarmed fallback., Return (player_service, registry, async_persistence) from app container, or…, _weapon_damage_from_equipped_player()
...
Cohesion: 0.08
Nodes (25): Unit tests for movement monitor. Tests the MovementMonitor class for monitoring…, Test record_integrity_check() records check with violation., Test validate_room_integrity() with valid room data., Test validate_room_integrity() handles empty rooms dict., Test validate_room_integrity() handles rooms without get_players method., Test get_metrics() returns metrics for empty monitor., Test get_alerts() alerts on high concurrent movements., Test MovementMonitor initialization. (+17 more)
...
Cohesion: 0.04
Nodes (50): Get base stats as dictionary., _coerce_xp_mapping_value(), _NPCCombatIntegrationValidationDeps, Protocol, UUID, Validate that player and NPC are in the same room., End any active combat that includes this player when room validation fails., Convert string IDs to UUIDs and set up XP mappings. (+42 more)
...
### Community 290 - "Stats"
Cohesion: 0.03
Nodes (100): get_stats_generator(), Get a StatsGenerator instance via dependency injection. StatsGenerator is…, generate_random_stats(), Stats Generator Service for MythosMUD. This module provides random stat…, Generate Stats with random attribute values. Factory function for creating…, Service for generating random character statistics., Initialize the stats generator., StatsGenerator (+92 more)
...
### Community 292 - "test_communication_commands_support.py"
Cohesion: 0.08
Nodes (35): app_from_request(), AsyncPersistenceForPose, chat_result_map(), get_pose_persistence(), get_services_from_container(), message_id_from_result(), PlayerWithPose, primary_id() (+27 more)
...
### Community 298 - "test_inventory_equip_command.py"
Cohesion: 0.13
...
### Community 305 - "test_command_alias.py"
Cohesion: 0.07
Nodes (35): AliasCommand, field_validator, Command for creating or viewing command aliases., Validate alias name format using centralized validation., Validate command content for security using centralized validation., Command for removing an alias., Validate alias name format using centralized validation., UnaliasCommand (+27 more)
...
Cohesion: 0.07
Nodes (40): buildCreateCharacterPayload(), CharacterNameScreen(), CreateCharacterPayload, OccupationSlotPayload, PersonalInterestPayload, SkillsPayload, CharacterCard(), CharacterCardDeleteState (+32 more)
...
Cohesion: 0.06
Nodes (63): _AppStateForPlayerService, build_basic_player_data(), convert_schema_to_dict(), _ensure_player_in_room_occupancy(), _fetch_room_for_tracked_player(), get_player_and_room(), get_player_service_from_connection_manager(), get_player_stats_data() (+55 more)
...
### Community 313 - "middleware"
Cohesion: 0.23
Nodes (16): CorrelationMiddleware, create_correlation_middleware(), Any, Correlation middleware for request tracing and logging context. This middleware…, Create a correlation middleware factory. Args: correlation_header: HTTP header…, Pure ASGI middleware for adding correlation IDs and request context to all…, asyncio, Unit tests for correlation ID middleware. (+8 more)
...
### Community 315 - "player_event_handlers_respawn_room.py"
Cohesion: 0.09
...
### Community 317 - "PlayerEnteredRoom"
Cohesion: 0.02
Nodes (127): PlayerEnteredRoom, PlayerLeftRoom, Event fired when a player enters a room. This event is triggered when a player…, Event fired when a player leaves a room. This event is triggered when a player…, Move followers when the followed player moves., _entity_id_for_quest_offer(), _make_on_npc_died(), _make_on_player_entered() (+119 more)
...
### Community 320 - "LucidityFluxService"
Cohesion: 0.10
Nodes (18): LucidityFluxService, PlayerFluxCtx, AsyncSession, datetime, Player, Build room cache for all players., Process a single player's passive flux., Evaluate passive LCD flux for the current tick. (+10 more)
...
### Community 322 - "test_login_grace_period.py"
Cohesion: 0.07
Nodes (37): mock_connection_manager(), asyncio, fixture, Unit tests for login grace period functionality. Tests the core login grace…, Test cancelling grace period when not active (should not error)., Test checking if player is in grace period when active., Test checking if player is in grace period when not active., Test checking grace period when manager lacks attribute. (+29 more)

### Community 323 - "InviteManager"
Cohesion: 0.17
Nodes (22): InviteManager, Remove expired invites and return count of removed invites., Manages invite creation, validation, and tracking. Handles the invite-only…, asyncio, Unit tests for InviteManager (server.auth.invites)., use_invite reserves, captures, commits, then re-fetches the row (3 execute()…, A code that isn't reservable (unknown/already used) is rejected before any…, Defense-in-depth: a capture that returns false after a successful reserve still… (+14 more)
...
Cohesion: 0.04
Nodes (57): _AppStateCommandGuards, _CastingStateManagerView, _CastingStateView, check_casting_state(), check_grace_period_block(), _coerce_player_uuid(), _get_casting_block_result(), _get_grace_check_context() (+49 more)
...
Cohesion: 0.10
Nodes (35): add_default_combat_data_to_config(), add_default_combat_data_to_stats(), Any, Add default combat data to base_stats if not present. Args: stats: Base stats…, Add default combat data to behavior_config if not present. Args: config:…, main(), migrate_npc_combat_data(), _migrate_one_npc() (+27 more)

### Community 330 - "test_game_tick_processing_async.py"
Cohesion: 0.07
...
### Community 331 - "DialogueService"
Cohesion: 0.08
Nodes (35): DialogueCursor, DialoguePrompt, DialogueService, UUID, In-memory dialogue session service for classic MUD talk (#583). Loads trees by…, Load and validate a dialogue tree, or clear cursor and return fade text., Return an error string if option_index is out of range for node., Advance from the current cursor by 1-based option index. Returns… (+27 more)
...
Cohesion: 0.11
Nodes (19): ConnectionCleaner, Any, UUID, Identify players whose last_seen timestamp exceeds the max age. Args:…, Remove all data for a stale player. Args: pid: Player ID to remove…, Remove players whose presence is stale beyond the threshold. Args: last_seen:…, Extract player_id from connection metadata if present., Close stale WebSocket and remove from tracking. Handles None websocket… (+11 more)
...
### Community 336 - "test_combat_persistence_handler_persistence.py"
Cohesion: 0.09
...
### Community 339 - "WearableContainerService"
Cohesion: 0.11
...
Cohesion: 0.10
Nodes (22): ContainerCreateParams, Optional fields for creating a container row (beyond source_type)., _container_data_to_dict(), ContainerRepository, Any, ContainerData, datetime, UUID (+14 more)
...
### Community 347 - "test_nats_service_health.py"
Cohesion: 0.08
Nodes (35): nats_config(), asyncio, fixture, NATS health-check, batch flush, and subscription-lifecycle tests., Test _stop_health_monitoring() handles no task., Test publish_batch() adds message to batch., Test publish_batch() flushes when batch is full., Test _flush_batch() successfully flushes batch. (+27 more)

### Community 348 - "_handle_admin_set_stat_command"
Cohesion: 0.07
Nodes (45): _handle_admin_set_stat_command(), Handle admin set <stat_name> <target_player> <value>., _assert_stat_write_path(), asyncio, patch, Test handling when player service is not available., Test handling when persistence layer is not available., Test admin action logging. (+37 more)

### Community 349 - ".async_persistence"
Cohesion: 0.10
...
### Community 351 - "test_connection_cleanup_methods.py"
Cohesion: 0.08
...
### Community 352 - "mapPageRenderer.tsx"
Cohesion: 0.16
Nodes (17): RoomMapViewerProps, MapPage(), AuthenticatedMapProps, MapViewResolvedProps, renderAuthenticatedMapView(), renderMapPageState(), renderStatusGate(), resolveMapViewProps() (+9 more)
...
### Community 354 - "test_communication_commands_channels.py"
Cohesion: 0.10
...
### Community 356 - "admin_teleport_commands.py"
Cohesion: 0.11
Nodes (44): Admin teleport command handlers for MythosMUD. This module provides handlers…, execute_confirm_goto(), execute_goto_teleport(), log_goto_failure(), Any, Exception, Helper functions for goto command operations., Log failed goto action. (+36 more)

### Community 357 - "test_connection_manager_methods.py"
Cohesion: 0.06
Nodes (34): convert_uuids_to_strings_impl(), get_connection_health_stats_impl(), get_error_statistics_impl(), get_memory_alerts_impl(), get_memory_stats_impl(), get_next_sequence_impl(), get_npcs_batch_impl(), get_online_players_impl() (+26 more)

### Community 358 - "test_container_websocket_events.py"
Cohesion: 0.08
Nodes (45): emit_container_closed(), emit_container_decayed(), emit_container_opened(), emit_container_opened_to_room(), emit_container_updated(), Any, datetime, UUID (+37 more)

### Community 359 - "passive_corruption_flux/service.py"
Cohesion: 0.10
Nodes (29): CorruptionFluxServiceConfig, CorruptionOverride, Configuration for passive corruption flux (#815 PR-5). `period_label`,…, A zone/subzone-level override for corruption flux rate and/or target (#815)., Optional configuration for PassiveCorruptionFluxService. All fields have…, PassiveCorruptionFluxContext, Data models for passive corruption flux (#815 PR-5)., Resolved environmental context for one player's passive corruption flux… (+21 more)
...
### Community 364 - "passive_lucidity_flux/service.py"
Cohesion: 0.10
Nodes (23): normalize_environment_config(), period_label(), Any, datetime, Configuration and normalization for passive lucidity flux., Return a coarse period label used for environment profiles., Normalize environment config to validated structure., Passive lucidity flux service package. (+15 more)

### Community 365 - "test_room_utils.py"
Cohesion: 0.13
Nodes (20): Unit tests for room_utils. Tests utility functions for room operations., Test get_plane_from_room_id() extracts plane., Test get_plane_from_room_id() extracts different plane., Test get_plane_from_room_id() returns None for invalid format., Test is_valid_room_id_format() validates room ID format., Test get_local_channel_subject() generates subject (deprecated)., Test get_local_channel_subject() returns None for invalid room ID., test_get_local_channel_subject() (+12 more)

### Community 366 - "test_connection_statistics.py"
Cohesion: 0.06
Nodes (42): get_online_player_by_display_name_method(), get_player_presence_info_method(), Get detailed presence information for a player., Get online player information by display name., Get session management statistics., Get presence tracking statistics., get_online_player_by_display_name_impl(), get_player_presence_info_impl() (+34 more)
...
### Community 368 - "test_connection_cleaner.py"
Cohesion: 0.08
...
Cohesion: 0.09
Nodes (29): CHANNEL_TO_TYPE_MAP, handleChatMessage(), handleCommandResponse(), handleRoomMessage(), handleSystem(), resolveChatTypeFromChannel(), createMockAppendMessage(), createMockContext() (+21 more)

### Community 373 - "mythos_e2e_ddl.sql"
Cohesion: 0.08
Nodes (44): mythos_e2e.aliases, mythos_e2e.calendar_holidays, mythos_e2e.calendar_npc_schedules, mythos_e2e.container_contents, mythos_e2e.containers, mythos_e2e.corruption_adjustment_log, mythos_e2e.corruption_cooldowns, mythos_e2e.dialogue_definitions (+36 more)

### Community 374 - "mythos_unit_ddl.sql"
Cohesion: 0.08
Nodes (44): mythos_unit.aliases, mythos_unit.calendar_holidays, mythos_unit.calendar_npc_schedules, mythos_unit.container_contents, mythos_unit.containers, mythos_unit.corruption_adjustment_log, mythos_unit.corruption_cooldowns, mythos_unit.dialogue_definitions (+36 more)
...
### Community 379 - "PerformanceTracker"
Cohesion: 0.08
Nodes (24): PerformanceTracker, Any, Record a session switch event. Args: duration_ms: Duration in milliseconds, Record a health check event. Args: duration_ms: Duration in milliseconds, Trim samples to prevent unbounded memory growth. Args: metric_key: Key in…, Get comprehensive performance statistics with calculated averages. Returns:…, Tracks performance metrics for connection management operations. This class…, Initialize the performance tracker. Args: max_samples: Maximum number of… (+16 more)

### Community 380 - "Invite"
Cohesion: 0.08
...
### Community 384 - "test_connection_disconnection_websockets.py"
Cohesion: 0.09
...
### Community 390 - "test_player_related_models.py"
Cohesion: 0.07
Nodes (31): PlayerChannelPreferences, Player channel preferences model for Advanced Chat Channels. Stores player…, Unit tests for Player-related SQLAlchemy models. Tests…, Test PlayerInventory has correct table name., Test PlayerInventory __repr__ method., Test PlayerExploration can be instantiated with required fields., Test PlayerExploration has correct table name., Test PlayerExploration __repr__ method. (+23 more)

### Community 391 - "asyncio"
Cohesion: 0.06
Nodes (32): check_all_connections_health_impl(), disconnect_websocket_connection_impl(), get_room_occupants_impl(), handle_player_entered_room_impl(), periodic_health_check_impl(), Disconnect a specific WebSocket connection for a player., Check health of all connections and clean up stale/dead ones., Periodic health check task that runs continuously. (+24 more)

### Community 392 - ".publish_with_pool"
Cohesion: 0.07
Nodes (17): Client, Task, Get connection from pool. Raises: NATSPublishError: If no connection is…, Return connection to pool., Validate subject when subject manager and validation are enabled., Publish message using connection pool for high-throughput scenarios. Args:…, Add message to batch for efficient bulk publishing. Args: subject: NATS subject…, Handle batch timeout for low-traffic scenarios. (+9 more)
...
### Community 394 - "RoomSubscriptionManager"
Cohesion: 0.06
...
### Community 395 - "container_query_helpers.py"
Cohesion: 0.09
...
Cohesion: 0.12
Nodes (12): asyncio, Test process_command_unified handles special command routing., Test process_command_unified processes normal commands., Test legacy compatibility functions., Test process_command() legacy function., Test get_help_content() delegates to help system., Test get_help_content() with None command., Test process_command_unified function. (+4 more)

### Community 401 - "SpellLearningService"
Cohesion: 0.10
Nodes (31): Any, UUID, Spell learning service for handling spell acquisition. This module provides…, Learn a spell for a player., Validate prerequisites for learning a spell. Args: player_id: Player ID spell:…, Service for handling spell learning from various sources. Manages spell…, Learn a spell from a spellbook item. Args: player_id: Player ID…, # TODO: Integrate with item system to get spellbook data # pylint:… (+23 more)

### Community 402 - "test_disconnect_catchup.py"
Cohesion: 0.10
...
### Community 403 - "emit_posture_change"
Cohesion: 0.07
Nodes (39): _broadcast_room_posture_change(), emit_posture_change(), format_room_posture_message(), normalize_posture(), _PostureConnectionManager, Protocol, UUID, Unified posture change notifications for GameInfo and room observers. (+31 more)

### Community 404 - "NPCCombatDataProvider"
Cohesion: 0.05
Nodes (47): Combat messaging integration with real-time messaging system. Re-exports from…, NPCCombatDataProvider, Any, UUID, NPC Combat Data Provider. This module provides data retrieval and preparation…, Get player name for messaging. Args: player_id: ID of the player Returns:…, Get the current room ID for a player. Args: player_id: ID of the player (must…, Get player combat participant data from persistence. Args: player_id: ID of the… (+39 more)

### Community 405 - "websocket_handler.py"
Cohesion: 0.03
Nodes (90): AttributeError, async_work(), correct_async_logging(), correct_request_context(), Demonstrate correct request context binding., Demonstrate correct async logging patterns., add_request_context(), process_websocket_message() (+82 more)

### Community 406 - "PlayerDeathService"
Cohesion: 0.11
...
Cohesion: 0.08
Nodes (34): get_global_tracked_manager(), memory_leak_prevention_channel_start_session(), patch_asyncio_create_task_with_tracking(), Any, Task, Global TrackedTaskManager for Memory Leak Prevention Infrastructure. This…, Create a task with enhanced supervision for legacy cleanup scenarios. Args:…, Audit and reclaim orphaned task candidates across the system. Returns: Number… (+26 more)

### Community 410 - "resolve_weapon_attack_from_equipped"
Cohesion: 0.08
...
### Community 411 - "UUID"
Cohesion: 0.03
Nodes (32): UUID, Get the first WebSocket connection ID for a player (backward compatibility)., Check if a player has any WebSocket connections., Get the number of connections for a player by type., Subscribe a player to a room (compatibility method)., Unsubscribe a player from a room (compatibility method)., Sync online_players[...]['current_room_id'] with an actual room move…, Remove a player from all room subscriptions and occupant lists (compatibility… (+24 more)

### Community 412 - "RealTimeEventHandler"
Cohesion: 0.03
Nodes (60): UUID, Initialize specialized handler modules., Get the next sequence number for events., Subscribe to relevant game events., Delegate player entered event to specialized handler., Delegate player left event to specialized handler., Delegate NPC entered event to specialized handler., Delegate NPC left event to specialized handler. (+52 more)

### Community 413 - "._build_player_attacked_event"
Cohesion: 0.08
Nodes (15): UUID, Resolve the player and UUID needed for DP update events., Compute old_dp, new_dp, and max_dp values for PlayerDPUpdated., Publish the PlayerDPUpdated event to the event bus., Publish NPC-on-player attack as player_attacked to NATS so the client receives…, Resolve the combat event publisher used to send PlayerAttacked events to NATS., Resolve target UUID, player object, and stats needed for NATS attack event., Construct the PlayerAttackedEvent payload for NATS publication. (+7 more)
...
### Community 416 - "test_command_parser_helpers.py"
Cohesion: 0.05
...
### Community 422 - "connection_initialization.py"
Cohesion: 0.05
Nodes (58): initialize_connection_cleaner(), initialize_connection_state(), initialize_core_components(), initialize_error_handler(), initialize_game_state_provider(), initialize_health_monitor(), initialize_messaging(), initialize_room_event_handler() (+50 more)

### Community 423 - "PassiveCorruptionFluxService"
Cohesion: 0.13
Nodes (17): _as_str_attr(), FluxRoom, PassiveCorruptionFluxService, datetime, Player, Protocol, UUID, Fractional-to-integer bank, identical in shape to passive_lucidity_flux's -- a… (+9 more)

### Community 424 - "quest_commands.py"
Cohesion: 0.08
Nodes (47): _active_npc_ids_in_room(), _emit_npc_lines_for_results(), _format_goal_line(), _format_one_quest_entry(), _format_quest_action_results(), _format_quest_log(), _get_container_and_persistence(), _get_quest_service() (+39 more)

### Community 425 - "follow_movement.py"
Cohesion: 0.11
Nodes (33): drop_follower(), ensure_follower_standing(), follower_already_in_room(), follower_needs_stand(), _FollowMovementHost, on_npc_entered_room(), on_player_entered_room(), propagate_follower_move() (+25 more)
...
### Community 428 - "CombatCommandFactory"
Cohesion: 0.07
Nodes (33): Unit tests for combat command factories. Tests the CombatCommandFactory class…, Test create_attack_command() creates AttackCommand., Test create_attack_command() allows None target (validation happens later)., Test create_punch_command() creates PunchCommand., Test create_punch_command() allows None target (validation happens later)., Test create_kick_command() creates KickCommand., Test create_kick_command() allows None target (validation happens later)., Test create_strike_command() creates StrikeCommand. (+25 more)

### Community 429 - "test_skill_service.py"
Cohesion: 0.10
...
Cohesion: 0.09
Nodes (29): useMapEditing(), MapEditToolbar(), MapEditToolbarProps, buildModalCreateEdgeHandler(), buildModalPreviewHandler(), buildModalUpdateEdgeHandler(), buildModalUpdateRoomHandler(), buildPreviewEdge() (+21 more)
...
### Community 435 - "temp_dir"
Cohesion: 0.08
Nodes (18): fixture, Create a temporary directory for testing., temp_dir(), Test file structure validation with unusual filenames., Integration tests for the main validator CLI. Tests the complete validation…, Test path validator integration., Test reporter integration., Test the full validation pipeline. (+10 more)
...
Cohesion: 0.10
Nodes (35): _broadcast_shutdown_cancellation(), broadcast_shutdown_notification(), _cancel_countdown_task(), _cancel_existing_shutdown_task(), _clear_shutdown_state(), countdown_loop(), _create_countdown_task(), _get_shutdown_services() (+27 more)

### Community 450 - "test_admin_commands_helpers.py"
Cohesion: 0.09
...
Cohesion: 0.13
Nodes (26): cleanup_decayed_corpses(), _create_corpse_lifecycle_service(), _log_cleanup_results(), FastAPI, Create CorpseLifecycleService or None if persistence is unavailable., Log the results of corpse cleanup., Cleanup decayed corpse containers (every 60 ticks = 1 minute)., Run process_one(str(player_id)) for each online player and log successes. (+18 more)

### Community 457 - "test_game_tick_processing.py"
Cohesion: 0.08
Nodes (36): _cleanup_single_decayed_corpse(), _CorpseLike, Protocol, Cleanup a single decayed corpse. Returns True on success., asyncio, Unit tests for game tick processing functions. Tests the game tick processing…, Test _validate_app_state_for_status_effects returns False when no…, Test _validate_app_state_for_status_effects returns True when all required… (+28 more)
...
Cohesion: 0.14
Nodes (22): ExperienceRepository, Player, UUID, Run update_player_stat_field stored procedure and log success., Update a specific numeric field in player stats atomically. Args: player_id:…, Repository for player experience and stats persistence operations. Handles XP…, Initialize the experience repository. Args: event_bus: Optional EventBus for…, Award experience points to a player atomically. Args: player: Player to award… (+14 more)

### Community 460 - "test_connection_error_methods.py"
Cohesion: 0.13
Nodes (27): delegate_error_handler(), Generic delegate for error handler methods. Args: error_handler: Error handler…, detect_and_handle_error_state_impl(), handle_authentication_error_impl(), handle_security_violation_impl(), handle_websocket_error_impl(), Any, UUID (+19 more)
...
### Community 462 - "server/persistence/__init__.py"
Cohesion: 0.13
...
Cohesion: 0.11
Nodes (22): forceLogoutPlayer(), assertNoRestDisconnectPollution(), executeCommandTrusted(), logoutPlayer(), captureOccupantsSnapshot(), capturePresenceEvents(), EnsureMultiplayerCoLocatedOptions, ensureMultiplayerReadyForCoLocate() (+14 more)
...
### Community 469 - "useStatsRollingActions.ts"
Cohesion: 0.11
Nodes (16): STAT_ROWS, StatsRollingScreen(), mockFetch, RolledStatsInput, useStatsRolling(), applyFailedStatsRoll(), applyOkStatsRoll(), handleNetworkError() (+8 more)
...
### Community 471 - "test_shutdown_process_termination.py"
Cohesion: 0.08
...
### Community 475 - "PrototypeRegistryError"
Cohesion: 0.10
Nodes (27): ItemInstance, Item system package. This module exposes the prototype schema and registry…, ItemFactory, ItemFactoryError, Any, Exception, PrototypeRegistry, Item factory for creating item instances from prototypes. This module provides… (+19 more)
...
### Community 477 - "test_lifespan_event_subscriptions.py"
Cohesion: 0.16
...
### Community 479 - "schemas/shared/__init__.py"
Cohesion: 0.13
...
Cohesion: 0.09
Nodes (27): GameMechanicsService, Gain occult knowledge (with lucidity loss)., Heal a player's health., Damage a player's health., Award experience points to a player. CRITICAL FIX: This method prevents XP…, Service class for game mechanics operations., Initialize the game mechanics service with a persistence layer., Apply lucidity loss to a player. (+19 more)

### Community 481 - "MovementMonitor"
Cohesion: 0.13
Nodes (12): MovementMonitor, Any, Record concurrent movement count., Record an integrity check result., Validate players are not in multiple rooms., Get comprehensive movement metrics., Get current alerts based on thresholds., Comprehensive monitoring system for the movement system. This class provides: -… (+4 more)
...
### Community 484 - "test_event_publisher.py"
Cohesion: 0.08
...
Cohesion: 0.17
Nodes (16): AsciiNoise(), AsciiNoiseProps, prefersReducedMotion(), MinimapInlineBody(), ageBucket(), decayMessageText(), isProtectedToken(), MYTHOS_GLYPHS (+8 more)
...
### Community 488 - "test_connection_establishment.py"
Cohesion: 0.08
Nodes (38): _cleanup_dead_connections(), Remove a single dead connection from tracking structures. Args: conn_id: The…, Clean up dead connections under lock. Args: dead_connection_ids: List of dead…, Get player and setup room subscription. Args: player_id: The player's ID…, Track player presence and broadcast connection message. Args: player_id: The…, _remove_dead_connection(), _setup_player_and_room(), _track_player_presence() (+30 more)

### Community 489 - "convert_uuids_to_strings"
Cohesion: 0.11
...
### Community 500 - "test_combat_persistence_handler.py"
Cohesion: 0.09
...
### Community 501 - "ItemPrototypeModel"
Cohesion: 0.10
...
### Community 503 - "_as_mgr"
Cohesion: 0.07
Nodes (66): establish_websocket_connection(), Establish a new WebSocket connection. Args: websocket: The WebSocket connection…, _as_mgr(), _as_ws(), _FakeWebSocket, _make_manager(), _meta(), ConnectionMetadata (+58 more)
...
### Community 514 - "game_tick_death.py"
Cohesion: 0.13
...
### Community 518 - "test_container_query_helpers_async.py"
Cohesion: 0.16
...
### Community 521 - "test_room_occupant_manager.py"
Cohesion: 0.09
...
### Community 527 - "CombatPersistenceHandler"
Cohesion: 0.13
...
### Community 530 - "PlayerPreferencesService"
Cohesion: 0.14
...
### Community 531 - "TestLogoutCommand"
Cohesion: 0.11
...
### Community 537 - "fixtures/integration/__init__.py"
Cohesion: 0.13
...
### Community 538 - "logging_file_categories.py"
Cohesion: 0.18
...
### Community 543 - "connection_establishment.py"
Cohesion: 0.08
Nodes (39): _bind_accepted_websocket(), _cleanup_failed_connection(), _EstablishmentConnectionManager, _find_dead_connections(), Player, Protocol, UUID, WebSocket (+31 more)
...
### Community 549 - "test_lucidity_trigger_handlers.py"
Cohesion: 0.21
...
### Community 550 - "test_websocket_room_updates_fanout.py"
Cohesion: 0.13
Nodes (21): _has_phantom_for(), _make_mock_room(), mock_connection_manager(), _phantom_data_for(), asyncio, fixture, Unit tests for the per-viewer room-payload fan-out (#714). Split out of…, The fast path: an empty phantom roster still broadcasts once, unchanged (#714). (+13 more)
...
### Community 560 - "game_tick_processing.py"
Cohesion: 0.10
Nodes (33): get_current_tick(), Shared game tick counter. Kept in a leaf module so combat services can read the…, Get the current game tick., Set the current game tick (game tick loop)., Reset the current tick for testing., reset_current_tick(), set_current_tick(), process_dp_decay_and_death() (+25 more)

### Community 561 - "game_tick_protocols.py"
Cohesion: 0.12
...
### Community 565 - "PlayerOccupantProcessor"
Cohesion: 0.09
Nodes (24): PlayerOccupantProcessor, Any, UUID, Player occupant processing utilities. This module handles querying and…, Process players and convert to occupant information. Args: room_id: The room ID…, Processes player occupants for rooms., Initialize player occupant processor. Args: connection_manager:…, Ensure a player is included in the player ID strings list if specified. Args:… (+16 more)
...
Cohesion: 0.07
Nodes (33): health_monitor(), mock_cleanup_dead_websocket(), mock_is_websocket_open(), mock_performance_tracker(), mock_validate_token(), asyncio, fixture, Unit tests for health monitor. Tests the HealthMonitor class. (+25 more)
...
### Community 575 - "test_alias_expansion.py"
Cohesion: 0.18
...
### Community 579 - "PrototypeRegistry"
Cohesion: 0.15
...
### Community 580 - "reset_database"
Cohesion: 0.11
Nodes (19): Reset the database connection state (for testing). This resets the…, reset_database(), fixture, Reset database state before each test., Test reset_database resets module-level _database_url., reset_db_state(), test_reset_database_resets_module_url(), fixture (+11 more)
...
### Community 602 - "SpellTargetingService"
Cohesion: 0.20
...
### Community 603 - "test_statistics_aggregator.py"
Cohesion: 0.10
...
### Community 609 - "test_metadata.py"
Cohesion: 0.11
...
### Community 619 - "AsciiMapRenderer"
Cohesion: 0.15
...
### Community 621 - "test_inventory_mutation_guard_error_handling.py"
Cohesion: 0.13
...
### Community 625 - "test_inventory_mutation_guard_internal.py"
Cohesion: 0.09
...
### Community 643 - "get_async_session"
Cohesion: 0.08
Nodes (29): add_flavor_text_column(), Add flavor_text column if missing., load_seed_data(), Load all seed data files., close_db(), get_async_session(), init_db(), Get an async database session as an async context manager. Usage: async for… (+21 more)
...
### Community 648 - "handle_unequip_command"
Cohesion: 0.29
...
### Community 649 - "._handle_exception"
Cohesion: 0.16
...
### Community 650 - "websocket_handler_validation.py"
Cohesion: 0.18
Nodes (16): check_websocket_message_rate_limit(), extract_csrf_token_from_raw(), get_connection_csrf_context(), WebSocket, WebSocket message validation, CSRF/JWT resolution, and rate limiting. Extracted…, Persist a validated message JWT on connection metadata after reconnect edge…, Validate csrfToken from the message body and optionally heal connection…, Resolve the CSRF/JWT token used for message validation. Prefer connection… (+8 more)
...
### Community 653 - "test_connection_state_machine.py"
Cohesion: 0.08
...
### Community 659 - "test_player_spell_repository.py"
Cohesion: 0.27
...
### Community 661 - "Room"
Cohesion: 0.06
Nodes (38): currentDpOf(), getCurrentLucidity(), markPlayerDead(), PlayerStatusSetters, skipDeadInRespawnRoom(), syncDeathState(), syncDeliriumState(), usePlayerStatusEffects() (+30 more)
...
Cohesion: 0.11
Nodes (32): assert_auth_rate_limit_paths_registered(), _auth_bucket(), auth_client_key(), auth_rate_limit_response(), _collect_post_paths(), _HasPrefix, _HasRoutes, _IncludedRouterLike (+24 more)

### Community 671 - "Color & Contrast"
Cohesion: 0.12
...
Cohesion: 0.19
Nodes (10): MPRegenerationService, Any, UUID, Get MP regeneration multiplier based on player state. Args: stats: Player stats…, Restore MP from resting (accelerated regeneration). Args: player_id: Player ID…, Restore MP from meditation (highly accelerated regeneration). Args: player_id:…, Restore MP from consuming an item. Args: player_id: Player ID amount: Amount of…, Service for managing MP regeneration. Handles passive regeneration over time… (+2 more)
...
Cohesion: 0.04
Nodes (78): asyncio, Unit tests for wearable container service. Tests the WearableContainerService…, Test handle_unequip_wearable_container returns None when no item_instance_id., Test handle_unequip_wearable_container preserves container., Test handle_unequip_wearable_container returns None when container not found., Test get_wearable_containers_for_player returns containers., Test get_wearable_containers_for_player returns empty list when no containers., Test get_wearable_containers_for_player handles errors gracefully. (+70 more)
...
### Community 698 - "test_logging_file_categories.py"
Cohesion: 0.15
...
### Community 700 - "validate_player_presence_impl"
Cohesion: 0.13
Nodes (15): Validate player presence and clean up any inconsistencies., validate_player_presence_method(), Validate player presence and clean up any inconsistencies., validate_player_presence_impl(), test_validate_player_presence_method(), Test validate_player_presence_impl() detects player with connections but not…, Test validate_player_presence_impl() fixes connection count mismatch., Test validate_player_presence_impl() handles errors. (+7 more)
...
### Community 723 - "CleanupContext"
Cohesion: 0.14
Nodes (15): CleanupContext, Context for periodic cleanup checks. Groups parameters to stay under param-…, asyncio, Test cleanup_orphaned_data() cleans up orphaned data., Test cleanup_dead_connections() cleans up dead websocket connections., Test cleanup_orphaned_data() closes stale active connections., Test check_and_cleanup() no-ops when memory monitor does not request cleanup., Test force_cleanup() performs forced cleanup. (+7 more)

### Community 724 - "CatatoniaObserverProtocol"
Cohesion: 0.17
Nodes (10): CatatoniaObserverProtocol, datetime, Protocol, UUID, Handle a player returning from catatonia., Handle a player requiring sanitarium failover., Return False to suppress failover (debounce); True allows failover handling., Protocol for observers interested in catatonia state changes. (+2 more)

### Community 725 - "WearableContainerServiceError"
Cohesion: 0.07
...
### Community 728 - "test_ascii_map_renderer_exits.py"
Cohesion: 0.13
...
### Community 733 - "test_system_monitoring_endpoints.py"
Cohesion: 0.23
Nodes (13): Any, Resolve the container-owned MemoryLeakMetricsCollector (#679: no per-request…, _resolve_memory_leak_collector_from_request(), asyncio, Unit tests for server.api.system_monitoring resolvers. #679:…, The collector resolved from the container is reused, not rebuilt per request., No collector on the container -- endpoint still succeeds, just without leak…, _request_with_container() (+5 more)

### Community 734 - "_should_include_npc"
Cohesion: 0.14
...
Cohesion: 0.10
Nodes (20): Conclusion, Configuration, Context Management, 🚨 CRITICAL ANTI-PATTERNS - DO NOT USE, Custom Log Analysis, Data Sanitization, Enhanced Logging Best Practices for MythosMUD, ❌ FORBIDDEN IMPORT PATTERNS (+12 more)
...
Cohesion: 0.21
Nodes (14): generate_unique_bogus_email(), AsyncSession, Generate a unique bogus email address for a user. This function creates a bogus…, Validate that a bogus email follows our expected format. Args: email: The email…, validate_bogus_email_format(), asyncio, Unit tests for auth email utilities., test_generate_unique_bogus_email_adds_suffix_when_taken() (+6 more)

### Community 743 - "_validate_command_basics"
Cohesion: 0.08
Nodes (19): Validate basic command requirements. Returns result dict if invalid, None if…, _validate_command_basics(), Test _validate_command_basics returns result for empty command., Test _validate_command_basics returns result for command too long., Test _validate_command_basics returns result for invalid content., Test _validate_command_basics returns None for valid command., Test _validate_command_basics function., Test _validate_command_basics returns result for empty command. (+11 more)

### Community 744 - "get_player_service_for_testing"
Cohesion: 0.16
Nodes (10): get_player_service_for_testing(), Get a PlayerService instance for testing purposes. This function allows tests…, Tests for get_player_service_for_testing helper function., Test get_player_service_for_testing returns provided service., Test get_player_service_for_testing creates PlayerService when None provided., TestGetPlayerServiceForTesting, Test get_player_service_for_testing() function., Test get_player_service_for_testing() with injected service. (+2 more)

### Community 745 - "test_zone_config_loader.py"
Cohesion: 0.20
...
### Community 746 - "HealthRepository"
Cohesion: 0.08
...
### Community 747 - "StatisticsAggregator"
Cohesion: 0.06
...
### Community 748 - "safe_close_websocket_impl"
Cohesion: 0.18
Nodes (11): _CloseableWebSocketManager, is_websocket_open_impl(), Protocol, WebSocket, Safe WebSocket close helpers for connection management. Leaf module: no imports…, Return True if this WebSocket id was already marked closed., Record that this WebSocket id has been closed., Check if a WebSocket is open. (+3 more)

### Community 749 - "TestPrepareCommandForProcessing"
Cohesion: 0.14
Nodes (8): Unit tests for command preparation. Tests command preparation and processing…, Test _prepare_command_for_processing function., Test _prepare_command_for_processing returns rate limit result when rate…, Test _prepare_command_for_processing returns validation result when validation…, Test _prepare_command_for_processing handles empty command after cleaning., Test _prepare_command_for_processing handles empty command after normalization., Test _prepare_command_for_processing successfully prepares command., TestPrepareCommandForProcessing
...
### Community 754 - "test_mp_regeneration_service.py"
Cohesion: 0.14
...
### Community 755 - "MotdInterstitialScreen.tsx"
Cohesion: 0.19
...
### Community 775 - "mock_connection_manager"
Cohesion: 0.29
...
### Community 796 - "game_tick_status_effects.py"
Cohesion: 0.14
...
### Community 797 - "_seed_players_cache"
Cohesion: 0.39
Nodes (3): Keep players cache truthy; empty LRUCache is bool-false via __len__., _seed_players_cache(), TestCachedDecorator

### Community 798 - "test_cache_service.py"
Cohesion: 0.18
...
### Community 802 - "stats_generator_summary.py"
Cohesion: 0.14
Nodes (15): Get a summary of the character's stats including modifiers and totals. Args:…, _build_attribute_summary(), build_stat_summary(), _core_stat_values_by_field(), Stats, Stat summary helpers for StatsGenerator (keeps stats_generator under module…, Coerce optional stat ints for summary totals (matches legacy ``or 50``…, Map core stat field names to current values (typed access, no getattr). (+7 more)
...
### Community 804 - "handle_system_command"
Cohesion: 0.24
...
### Community 805 - ".connection_manager"
Cohesion: 0.12
Nodes (18): Check connection state before publishing combat ended event., setter, Return the connection manager, resolving it from the application container if…, Explicitly set the connection manager (primarily used in tests)., _connection_manager_from_config_app(), is_npc_attack_on_player_blocked_by_login_grace_period(), is_player_attack_blocked_by_login_grace_period(), ConnectionManager (+10 more)

### Community 806 - "user_manager.py"
Cohesion: 0.11
Nodes (12): datetime, User management service for MythosMUD chat system. This module provides…, Get active global mutes applied by a player., Get all mutes applied by a player. Args: player_id: Player ID Returns:…, Get system-wide user management statistics. Returns: Dictionary with system…, Clean up expired player mutes., Clean up expired channel mutes., Clean up expired global mutes. (+4 more)

### Community 807 - "RateLimiter"
Cohesion: 0.13
...
### Community 808 - "test_player_repository.py"
Cohesion: 0.02
Nodes (115): F, _coerce_row_stats(), _defaulted_numerics(), _defaulted_strings(), _parse_equipped_safely(), Any, Player, Extract and coerce stats from row. Returns empty dict if not a dict. (+107 more)
...
Cohesion: 0.11
Nodes (19): mythos_dev.corruption_adjustment_log, mythos_dev.corruption_cooldowns, mythos_dev.lucidity_adjustment_log, mythos_dev.lucidity_cooldowns, mythos_dev.lucidity_exposure_state, mythos_dev.player_channel_preferences, mythos_dev.player_effects, mythos_dev.player_inventories (+11 more)
...
### Community 838 - ".broadcast_connection_message"
Cohesion: 0.17
Nodes (7): Player, Get a player from the persistence layer (async version)., Get a player from the persistence layer (public API)., Get multiple players from the persistence layer in a single batch operation., Track when a player connects., Broadcast a connection message for a player who is already tracked as online., Send initial game_state event to a newly connected player.

### Community 839 - "test_follow_flow.py"
Cohesion: 0.17
...
Cohesion: 0.09
Nodes (25): mock_app(), asyncio, fixture, Unit tests for security headers middleware. Tests the SecurityHeadersMiddleware…, Test middleware error handling., Test _add_security_headers_to_response adds headers to Response., Test _add_security_headers_to_response includes subdomains in HSTS., Test _add_security_headers_to_response without subdomains in HSTS. (+17 more)

### Community 842 - ".check_and_cleanup"
Cohesion: 0.17
Nodes (8): Return connection IDs that exceed max_connection_age., Clean up orphaned data that might accumulate over time. Args:…, Stale-prune threshold (seconds). Higher in e2e/local to avoid mid-run drops., Force immediate cleanup of all orphaned data. Args: cleanup_stats: Cleanup…, Periodically check for cleanup conditions and perform cleanup if needed. Args:…, _stale_prune_max_age_seconds(), Test _stale_prune_max_age_seconds uses longer threshold in local env., test_stale_prune_max_age_local()
...
### Community 846 - "_CombatServiceDeps"
Cohesion: 0.17
...
### Community 851 - "_occupation_slots_9"
Cohesion: 0.17
...
### Community 857 - "_personal_interest_4"
Cohesion: 0.17
...
### Community 872 - "test_ascii_map_renderer_grid.py"
Cohesion: 0.17
...
### Community 878 - "MythosPanel.tsx"
Cohesion: 0.22
Nodes (7): buildPanelClasses(), CONTENT_PADDING, HEADER_PADDING, MythosPanel(), MythosPanelProps, SIZE_CLASSES, VARIANT_CLASSES

### Community 879 - "WebSocketManager"
Cohesion: 0.22
...
### Community 882 - "command_request_app_state"
Cohesion: 0.24
Nodes (10): command_request_app_state(), CommandExecutionRequest, Return app.state for HTTP Request or WebSocketRequestContext (duck-typed).…, Unit tests for unified command request app-state extraction., Returns app.state for HTTP/FastAPI-style request objects., Returns app.state for WebSocketRequestContext., Gracefully returns None when app or state is absent., test_command_request_app_state_from_http_request_like_object() (+2 more)

### Community 883 - "should_notify_quest_progress"
Cohesion: 0.22
Nodes (11): _as_int(), _goal_is_met(), _progress_has_any_value(), Any, Coerce progress/config scalars to int; non-numeric becomes default., Return True if one goal is satisfied given current progress., True if any goal slot has a non-zero / non-empty progress value., Return True when a progress personal-system line should be sent. Notifies on… (+3 more)
...
### Community 890 - "test_event_publisher_helpers.py"
Cohesion: 0.14
...
### Community 917 - "ComprehensiveLoggingMiddleware"
Cohesion: 0.23
...
### Community 918 - "test_channel_broadcasting_strategies.py"
Cohesion: 0.20
...
### Community 920 - "_get_lifecycle_manager"
Cohesion: 0.20
...
### Community 923 - "session_factory"
Cohesion: 0.24
Nodes (14): async_sessionmaker, Provide an async session factory for integration tests. CRITICAL: This fixture…, session_factory(), async_sessionmaker, asyncio, AsyncSession, Verify get_rooms_with_exits() (room cache data source) includes arena zone…, Call get_player_by_id() with non-existent UUID; verify return shape when empty. (+6 more)
...
### Community 925 - "register_error_handlers"
Cohesion: 0.33
Nodes (9): add_error_handling_middleware(), FastAPI, Add error handling middleware to FastAPI application. Args: app: FastAPI…, Register error handlers for FastAPI application. This function registers…, Setup complete error handling for FastAPI application. This function sets up…, register_error_handlers(), setup_error_handling(), Middleware package for MythosMUD server. This package contains middleware… (+1 more)
...
Cohesion: 0.02
Nodes (116): factory(), fixture, Unit tests for command factories. Tests the CommandFactory class., Test create_reply_command delegates to communication factory., Test create_channel_command delegates to communication factory., Test create_go_command delegates to exploration factory., Test create_sit_command delegates to exploration factory., Test create_stand_command delegates to exploration factory. (+108 more)

### Community 931 - "extract_zone_name"
Cohesion: 0.20
...
### Community 934 - "gameStore.ts"
Cohesion: 0.09
Nodes (22): fetchSpy, buildRoomListRequest(), FetchRoomListConfig, fetchRoomListData(), parseRoomListResponse(), UseRoomMapDataOptions, UseRoomMapDataResult, RoomDetailsPanel() (+14 more)
...
### Community 953 - "resolve_connection_manager"
Cohesion: 0.27
...
### Community 959 - "generate_openapi_spec.py"
Cohesion: 0.22
...
### Community 961 - "create_app"
Cohesion: 0.14
Nodes (17): create_app(), FastAPI, Mount all versioned API routers under /v1., Create and configure the FastAPI application. This function sets up the FastAPI…, _register_v1_routers(), _create_get_app(), main(), FastAPI (+9 more)

### Community 962 - "test_database.py"
Cohesion: 0.20
...
### Community 964 - "_get_npc_room_id"
Cohesion: 0.14
...
### Community 966 - "TestResolveExitTarget"
Cohesion: 0.20
...
Cohesion: 0.16
Nodes (14): _coerce_row_quantity(), fetch_container_items(), _item_dict_from_contents_row(), _metadata_dict_from_cell(), PsycopgConnection, PsycopgCursor, UUID, Fetch container items directly from normalized tables. Queries… (+6 more)

### Community 968 - "TestHorizontalExitCharBetween"
Cohesion: 0.20
...
### Community 969 - "DecodeLiabilitiesFn"
Cohesion: 0.22
...
### Community 971 - "test_emote_repository.py"
Cohesion: 0.27
...
### Community 973 - "test_combat_service_npc_in_combat.py"
Cohesion: 0.15
...
### Community 978 - "populate_test_npc_databases.py"
Cohesion: 0.31
...
### Community 983 - "get_player_impl"
Cohesion: 0.22
Nodes (9): get_player_impl(), get_players_batch_impl(), Player, Get a player from the persistence layer (async version)., Get multiple players from the persistence layer in a single batch operation., Send initial game_state event to a newly connected player., send_initial_game_state_impl(), test_get_player_impl() (+1 more)
...
### Community 1006 - "._get_vertical_exit_char"
Cohesion: 0.22
...
### Community 1011 - "test_message_filtering.py"
Cohesion: 0.17
...
### Community 1012 - "test_websocket_handler_validation.py"
Cohesion: 0.20
...
### Community 1013 - "generate_room_id"
Cohesion: 0.21
...
### Community 1019 - "GameClientV2.tsx"
Cohesion: 0.07
Nodes (38): IncapacitatedBanner, calculateOccupantCount(), GameClientV2(), GameClientV2Content(), GameClientV2Props, MainDockPanelId, MainDockSlotMeta, daypartAccent() (+30 more)
...
Cohesion: 0.09
Nodes (22): 1. Overview, 2. Members, 3. Boundary contract, 4. Key design decisions, 5. Constraints, 6. Developer guide, 7. Troubleshooting, 8. Related docs (+14 more)

### Community 1037 - "Middleware Package Design"
Cohesion: 0.18
Nodes (11): 1. Overview, 2. Members, 3. Boundary contract, 4. Key design decisions, 5. Constraints, 6. Developer guide, 7. Troubleshooting, 8. Related docs (+3 more)

### Community 1038 - "mp_regeneration_service"
Cohesion: 0.22
...
### Community 1046 - "player_participant"
Cohesion: 0.22
...
### Community 1049 - "spell_effects_support.py"
Cohesion: 0.11
...
### Community 1052 - "_EventPersistence"
Cohesion: 0.24
...
### Community 1058 - "._generate_invite_code"
Cohesion: 0.25
...
### Community 1061 - ".validate_tls_files"
Cohesion: 0.25
Nodes (5): Any, field_validator, Validate TLS file paths exist when TLS is enabled., Validate max payload is reasonable., Validate value is positive.
...
### Community 1088 - "Test Suite Quality Audit Report"
Cohesion: 0.18
...
### Community 1096 - "initialize_components"
Cohesion: 0.29
...
### Community 1098 - "load_zone_configurations"
Cohesion: 0.25
...
### Community 1099 - "RoomBasedChannelStrategy"
Cohesion: 0.25
...
### Community 1107 - "_FakeEstablishmentManager"
Cohesion: 0.20
...
### Community 1108 - "_FakeMessageQueue"
Cohesion: 0.20
...
### Community 1126 - "🔴 Anti-Patterns Check (Critical)"
Cohesion: 0.22
...
### Community 1138 - "TestGetContainer"
Cohesion: 0.25
...
### Community 1146 - "get_zone_from_room_id"
Cohesion: 0.25
Nodes (8): Test get_zone_from_room_id() extracts zone., Test get_zone_from_room_id() extracts different zone., Test get_zone_from_room_id() returns None for invalid format., test_get_zone_from_room_id(), test_get_zone_from_room_id_innsmouth(), test_get_zone_from_room_id_invalid(), get_zone_from_room_id(), Extract zone from room ID. Room ID format:…

### Community 1147 - "required"
Cohesion: 0.25
Nodes (8): description, exits, id, name, plane, sub_zone, zone, required

### Community 1149 - "InstanceRoomLookup"
Cohesion: 0.29
Nodes (5): Set the instance manager for instanced room lookup (instance-first)., InstanceRoomLookup, Protocol, Room, Minimal instance-manager surface used for instanced room lookup.

### Community 1150 - "reset_database"
Cohesion: 0.29
...
### Community 1151 - "command_service"
Cohesion: 0.29
...
### Community 1153 - "fixtures/unit/__init__.py"
Cohesion: 0.13
...
### Community 1156 - "generate_invites.py"
Cohesion: 0.38
...
### Community 1162 - "preferences_service"
Cohesion: 0.22
...
### Community 1174 - "local-channel-isolation.spec.ts"
Cohesion: 0.36
Nodes (8): ensureIthaquaInFoyer(), leaveEasternHallwayWest(), pageShowsEasternHallway(), prepareLocalIsolationPair(), primeBothForCoLocate(), returnAwToFoyerIfInHallway(), softCommand(), waitForLookReflected()
...
### Community 1207 - "connection_state_machine.py"
Cohesion: 0.29
...
Cohesion: 0.08
Nodes (51): CorruptionTier, StrEnum, Corruption bands over 0..100. `PURE` is reserved for exactly 0 (#815) --…, build_corruption_aware_greeting(), corruption_hostility_scale(), _pick_greeting(), NPC <-> player corruption relationship (#815 PR-4). An NPC's corruption is a…, A greeting reaction whose message depends on the entering player's corruption… (+43 more)
...
### Community 1230 - "utils/config.ts"
Cohesion: 0.10
...
### Community 1248 - "Test Quality Audit Report"
Cohesion: 0.29
...
### Community 1256 - ".get_task_lifecycle_metrics"
Cohesion: 0.33
...
### Community 1257 - "mark_player_seen_impl"
Cohesion: 0.33
...
### Community 1258 - "broadcast_global_event_impl"
Cohesion: 0.33
Nodes (6): broadcast_global_event_impl(), broadcast_global_impl(), Broadcast a message to all connected players., Broadcast a global event to all connected players., test_broadcast_global_event_impl(), test_broadcast_global_impl()

### Community 1259 - "infrastructure/conftest.py"
Cohesion: 0.33
...
### Community 1260 - "broadcast_to_room_impl"
Cohesion: 0.33
Nodes (6): broadcast_room_event_impl(), broadcast_to_room_impl(), Broadcast a message to all players in a room., Broadcast a room-specific event to all players in the room., test_broadcast_room_event_impl(), test_broadcast_to_room_impl()

### Community 1261 - "event_publisher"
Cohesion: 0.29
...
### Community 1271 - "get_active_connection_count_impl"
Cohesion: 0.27
Nodes (6): get_active_connection_count_impl(), Get the total number of active connections., Stop the periodic health check task., stop_health_checks_impl(), test_get_active_connection_count_impl(), test_stop_health_checks_impl()
...
### Community 1293 - "update_player_room_cache_impl"
Cohesion: 0.33
Nodes (6): Keep online_players[...]['current_room_id'] in sync with an actual room move.…, update_player_room_cache_impl(), #297/#610: online_players[...]['current_room_id'] is written once at connect…, A player absent from online_players (e.g. NPC-driven room event, or a race with…, test_update_player_room_cache_impl_player_not_online_is_a_noop(), test_update_player_room_cache_impl_refreshes_existing_entry()
...
### Community 1296 - "_get_enum_value"
Cohesion: 0.33
...
### Community 1297 - "TestCheckRateLimit"
Cohesion: 0.33
...
### Community 1302 - "TestGetPlayerService"
Cohesion: 0.33
...
### Community 1303 - "TestGetConnectionManager"
Cohesion: 0.33
...
### Community 1304 - "validate_shutdown_admin_permission"
Cohesion: 0.25
...
### Community 1306 - "TestGetAsyncPersistence"
Cohesion: 0.33
...
### Community 1307 - "TestGetExplorationService"
Cohesion: 0.33
...
### Community 1308 - "TestGetPlayerCombatService"
Cohesion: 0.33
...
### Community 1309 - "SystemAdminChannelStrategy"
Cohesion: 0.25
...
### Community 1311 - "test_initiate_shutdown_countdown_success"
Cohesion: 0.40
...
### Community 1312 - "test_event_publisher_init_with_initial_sequence"
Cohesion: 0.20
...
### Community 1313 - "TestGetPlayerDeathService"
Cohesion: 0.33
...
### Community 1332 - "TestGetMagicService"
Cohesion: 0.33
...
### Community 1339 - "TestGetSpellTargetingService"
Cohesion: 0.33
...
### Community 1348 - "TestGetSpellEffects"
Cohesion: 0.33
...
### Community 1352 - "snapshot_chaosium_graphify.ps1"
Cohesion: 0.70
...
### Community 1368 - "TestGetSpellLearningService"
Cohesion: 0.33
...
### Community 1369 - "test_command_player_state.py"
Cohesion: 0.06
...
### Community 1370 - "_EventBusPublishPort"
Cohesion: 0.40
...
### Community 1371 - "TestGetMPRegenerationService"
Cohesion: 0.33
...
### Community 1372 - "TestGetNPCLifecycleManager"
Cohesion: 0.33
...
### Community 1373 - "TestGetCatatoniaRegistry"
Cohesion: 0.33
...
### Community 1374 - "TestGetPassiveLucidityFluxService"
Cohesion: 0.33
...
### Community 1375 - "TestGetMythosTimeConsumer"
Cohesion: 0.33
...
### Community 1376 - "TestGetChatService"
Cohesion: 0.33
...
### Community 1377 - "TestGetNatsMessageHandler"
Cohesion: 0.33
Nodes (4): Tests for get_nats_message_handler dependency function., Test get_nats_message_handler returns handler when present., Test get_nats_message_handler returns None when handler is None (NATS disabled)., TestGetNatsMessageHandler

### Community 1378 - "message_filtering_helper"
Cohesion: 0.40
...
### Community 1382 - "wearable_service"
Cohesion: 0.40
...
### Community 1415 - "persistence_handler"
Cohesion: 0.33
...
### Community 1428 - "TestGetRoomService"
Cohesion: 0.33
...
### Community 1442 - "long_description"
Cohesion: 0.50
Nodes (4): maxLength, minLength, type, long_description

### Community 1443 - "TestGetPlayerService"
Cohesion: 0.33
...
### Community 1444 - "prototype_id"
Cohesion: 0.50
Nodes (4): prototype_id, maxLength, minLength, type

### Community 1445 - "short_description"
Cohesion: 0.50
...
### Community 1454 - "get_subzone_local_channel_subject"
Cohesion: 0.33
...
### Community 1459 - "setup_jwt_secret"
Cohesion: 0.50
...
### Community 1515 - ".validate_invite"
Cohesion: 0.30
Nodes (3): Request, Get all unused invites., Validate an invite code.
...
### Community 1539 - "weight"
Cohesion: 0.67
Nodes (3): weight, minimum, type

### Community 1548 - ".use_invite"
Cohesion: 0.40
Nodes (3): UUID, Mark an invite as used by a specific user (atomic auth-and-capture). Uses the…, Get all invites used by a user.

### Community 1549 - ".resolve_combat_target"
Cohesion: 0.40
Nodes (3): Resolve combat target using target resolution service. Public API., Validate target_result and resolve to a live NPC target_match., Resolve combat target using target resolution service.

### Community 1550 - "._check_alerts"
Cohesion: 0.40
Nodes (3): UUID, Check for alerts and log them., Record a movement attempt with metrics.

### Community 1551 - ".__call__"
Cohesion: 0.40
Nodes (4): Receive, Scope, Send, ASGI application interface. Args: scope: ASGI connection scope receive: ASGI…

### Community 1552 - "CombatEventPublisherProtocol"
Cohesion: 0.40
Nodes (4): CombatEventPublisherProtocol, Protocol, Publish a PlayerAttackedEvent to the combat event stream., Combat event publisher (avoids importing CombatEventPublisher).

### Community 1554 - "NewGameSessionResult"
Cohesion: 0.40
Nodes (4): Handle a new game session by disconnecting existing connections., NewGameSessionResult, TypedDict, Result payload from handle_new_game_session_impl.

### Community 1555 - "movement_monitor"
Cohesion: 0.40
Nodes (5): movement_monitor(), fixture, Create a MovementMonitor instance., Create a sample player ID., sample_player_id()

### Community 1556 - "mock_container"
Cohesion: 0.40
...
### Community 1738 - "1. Component Refactoring"
Cohesion: 0.50
...
### Community 1740 - "Executive Summary"
Cohesion: 0.50
...
### Community 1754 - "enabled"
Cohesion: 0.50
Nodes (4): default, description, type, enabled

### Community 1755 - "plane"
Cohesion: 0.50
Nodes (4): description, pattern, type, plane
...
### Community 1758 - "ensure_database_directory"
Cohesion: 0.50
Nodes (4): ensure_database_directory(), Ensure database directory exists (deprecated for PostgreSQL). This function is…, Test ensure_database_directory is no-op for PostgreSQL., test_ensure_database_directory_postgresql()

### Community 1761 - "get_connection_id_from_websocket_impl"
Cohesion: 0.50
Nodes (4): get_connection_id_from_websocket_impl(), WebSocket, Get connection ID from a WebSocket instance., test_get_connection_id_from_websocket_impl()

### Community 1763 - "mock_lifecycle_manager"
Cohesion: 0.50
Nodes (4): mock_lifecycle_manager(), mock_npc(), fixture, Create a mock lifecycle manager.

### Community 1764 - "TestGetStatsGenerator"
Cohesion: 0.50
...
### Community 1783 - "Common Anti-Patterns"
Cohesion: 0.67
Nodes (3): Common Anti-Patterns, ✅ Do This Instead, ❌ Don't Do This

### Community 1784 - "Testing Logging"
Cohesion: 0.67
Nodes (3): Integration Tests, Testing Logging, Unit Tests for Logging

### Community 1785 - "metadata"
Cohesion: 0.67
Nodes (3): additionalProperties, type, metadata

### Community 1787 - "get_dual_connection_stats_impl"
Cohesion: 0.67
Nodes (3): get_dual_connection_stats_impl(), Get comprehensive connection statistics., test_get_dual_connection_stats_impl()

### Community 1788 - "get_performance_stats_impl"
Cohesion: 0.67
Nodes (3): get_performance_stats_impl(), Get connection performance statistics., test_get_performance_stats_impl()

### Community 1791 - "mock_connection_manager"
Cohesion: 0.67
Nodes (3): mock_connection_manager(), fixture, Create a mock ConnectionManager.

### Community 2276 - ".get_engine"
Cohesion: 0.29
Nodes (5): _dispose_engine_safely(), AsyncEngine, Dispose database engine with Windows/asyncpg-safe cleanup., Get the database engine, initializing if necessary. Returns: AsyncEngine: The…, Close database connections.

### Community 2283 - ".__call__"
Cohesion: 0.29
...
## Knowledge Gaps
- **6005 isolated node(s):** `wsl-bashrc-codacy.sh script`, `uvx`, `jcodemunch-mcp`, `JCODEMUNCH_MAX_FOLDER_FILES`, `@codacy/codacy-mcp` (+6000 more)
...
- **Why does `get_logger()` connect `get_logger` to `User`, `container_endpoints_basic.py`, `game_tick_death.py`, `Player`, `server/dependencies.py`, `zone_config_loader.py`, `NPCCombatIntegrationService`, `inventory_equip_command.py`, `NPCBase`, `NATSService`, `TargetMatch`, `event_types.py`, `test_message_handlers.py`, `PlayerPreferencesService`, `test_npc_definitions_api.py`, `NPCDefinition`, `ContainerComponent`, `test_rescue_service.py`, `api/character_creation.py`, `spell_effects_support.py`, `time.py`, `test_combat_event_publisher.py`, `MemoryMonitor`, `PlayerService`, `EventBus`, `test_npc_admin_commands.py`, `connection_establishment.py`, `RoomEventHandler`, `test_combat_monitoring_service.py`, `server/tests/conftest.py`, `PlayerNameExtractor`, `get_session_maker`, `test_communication_commands_flows.py`, `lifespan_magic.py`, `lifespan_startup.py`, `TargetResolutionService`, `game_tick_processing.py`, `game_tick_protocols.py`, `ApplicationContainer`, `test_magic_commands.py`, `PlayerOccupantProcessor`, `extract_player_name`, `test_connection_session_management.py`, `look_container.py`, `MythosTickScheduler`, `handle_emote_command`, `test_chat_validator.py`, `Spell`, `CombatParticipantData`, `GameBundle`, `CombatConfiguration`, `magic_service.py`, `initialize_components`, `CombatInstance`, `test_websocket_initial_state.py`, `lucidity_migration.py`, `real_time.py`, `test_who_commands.py`, `connection_manager.py`, `test_chat_pose_helpers.py`, `test_look_room.py`, `test_rest_command.py`, `player_presence_tracker.py`, `look_helpers.py`, `NPCEventHandler`, `NPCOccupantProcessor`, `chat_service.py`, `WebSocketMessageValidator`, `test_chat_npc_system.py`, `CorruptionService`, `test_movement_service.py`, `RoomService`, `combat_service.py`, `handle_teach_command`, `ScheduleEntry`, `ChatMessage`, `lifespan.py`, `command_result_text`, `is_player_in_login_grace_period`, `command_service.py`, `PlayerPositionService`, `npc_database.py`, `rooms.py`, `test_look_player.py`, `PlayerCombatService`, `player_inventory_migration.py`, `test_combat_persistence_handler_events.py`, `IdleMovementHandler`, `test_connection_disconnection.py`, `disconnect_grace_period.py`, `test_aggro_threat.py`, `websocket_handler_validation.py`, `look_command.py`, `SkillRepository`, `apply_communication_dampening`, `fixtures/integration/__init__.py`, `FeatureFlagService`, `run_flee_effect`, `test_logout_commands.py`, `hallucinations.py`, `test_connection_helpers_impl.py`, `test_metrics_endpoints.py`, `test_look_npc.py`, `test_status_commands.py`, `LoggedHTTPException`, `test_teleport_helpers.py`, `admin_setstat_command.py`, `ChatModeration`, `QuestService`, `test_go_command.py`, `room_update_event_builder.py`, `TaskRegistry`, `MemoryLeakMetricsCollector`, `test_inventory_display_helpers.py`, `container_persistence.py`, `inventory_get_command.py`, `follow_service.py`, `item_instance_persistence_async.py`, `catatonia_check.py`, `connection_state_machine.py`, `test_shutdown_sequence.py`, `Test Player Disconnect Handlers`, `test_lucidity_command_disruption.py`, `PerformanceMonitor`, `test_admin_setlucidity_command.py`, `OccupantFormatter`, `test_lucidity_recovery_commands.py`, `test_hallucination_services.py`, `test_lucidity_event_dispatcher.py`, `CombatAuditLogger`, `test_alias_commands.py`, `NPCActionMessage`, `RoomRepository`, `admin_hallucinate_command.py`, `LoggingConfig`, `LogAggregator`, `inventory_command_helpers.py`, `equipment_helpers.py`, `lifespan_protocols.py`, `Room`, `PlayerEventHandler`, `HealthRepository`, `HealthStatus`, `ExceptionTracker`, `communication_commands.py`, `NATSRetryHandler`, `MonitoringDashboard`, `test_container_persistence_async_helpers.py`, `InstanceManager`, `PartyService`, `NPCCombatIntegrationBase`, `test_lifecycle_periodic.py`, `admin_commands.py`, `test_rate_overrides.py`, `MovementService`, `build_event`, `game_tick_status_effects.py`, `websocket_handler_commands.py`, `test_occupants.py`, `Stats`, `test_party_commands.py`, `user_manager.py`, `test_player_repository.py`, `websocket_helpers.py`, `middleware`, `handle_read_command`, `PlayerEnteredRoom`, `retry.py`, `LucidityService`, `command_guards.py`, `combat_attack.py`, `migrate_combat_data.py`, `DialogueService`, `CircuitBreaker`, `_str_id`, `ContainerRepository`, `.async_persistence`, `test_chat_nats_publisher.py`, `test_map_helpers.py`, `admin_teleport_commands.py`, `test_container_websocket_events.py`, `passive_corruption_flux/service.py`, `rest_countdown_task.py`, `passive_lucidity_flux/service.py`, `channel_broadcasting_strategies.py`, `optimized_security_validator.py`, `test_channel_commands.py`, `player_connection_setup.py`, `container_query_helpers.py`, `SpellLearningService`, `emit_posture_change`, `NPCCombatDataProvider`, `websocket_handler.py`, `TrackedTaskManager`, `resolve_weapon_attack_from_equipped`, `RealTimeEventHandler`, `connection_initialization.py`, `quest_commands.py`, `CombatCommandFactory`, `PayloadOptimizer`, `PeriodicOrphanAuditor`, `admin_shutdown_command.py`, `talk_command.py`, `ExperienceRepository`, `_find_item_in_equipped`, `PrototypeRegistryError`, `test_lifespan_event_subscriptions.py`, `MovementMonitor`, `Profession`, `test_lifecycle_respawn.py`, `MemoryThresholdMonitor`, `NPCCommunicationIntegration`?**
  _High betweenness centrality (0.142) - this node is a cross-community bridge._
- **Why does `ValidationError` connect `get_logger` to `Player`, `NPCCombatIntegrationBase`, `test_command_helpers.py`, `TargetMatch`, `event_types.py`, `test_command_processor.py`, `test_database_helpers.py`, `ContainerComponent`, `MovementService`, `api/character_creation.py`, `test_profession_service.py`, `PlayerService`, `LoggedHTTPException`, `test_command_parser_helpers.py`, `InventoryCommandFactory`, `DatabaseManager`, `test_go_command.py`, `container_persistence.py`, `UtilityCommandFactory`, `item_instance_persistence_async.py`, `EmoteService`, `test_command_parser.py`, `NPCCombatIntegration`, `PlayerStateCommandFactory`, `server/persistence/__init__.py`, `ExplorationCommandFactory`, `inventory_command_helpers.py`, `test_character_creation_service.py`, `CommunicationCommandFactory`, `test_command_processing.py`, `test_database_extended.py`, `GameMechanicsService`, `ContainerService`, `test_movement_service.py`, `PlayerRespawnWrapper`, `test_command_service.py`, `test_player_service.py`, `test_player_service_mutations.py`, `command_service.py`, `ModerationCommandFactory`, `npc_database.py`, `test_container_persistence_async_helpers.py`?**
  _High betweenness centrality (0.024) - this node is a cross-community bridge._
- **Why does `SchemaValidator` connect `get_logger` to `PathValidator`, `SchemaValidator`?**
  _High betweenness centrality (0.014) - this node is a cross-community bridge._
- **Are the 142 inferred relationships involving `LoggedHTTPException` (e.g. with `test_get_admin_sessions_error()` and `test_get_npc_population_stats_generic_error()`) actually correct?**
  _`LoggedHTTPException` has 142 INFERRED edges - model-reasoned connections that need verification._
