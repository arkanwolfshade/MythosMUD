# AGENTS.md agent instructions

> 24 nodes

## Key Concepts

- **PostgreSQL procedures/functions access** (6 connections) — `AGENTS.md`
- **AGENTS.md agent instructions** (6 connections) — `AGENTS.md`
- **COPPA compliance requirements** (3 connections) — `AGENTS.md`
- **Contributing guide** (3 connections) — `CONTRIBUTING.md`
- **MythosMUD project overview** (3 connections) — `README.md`
- **One server only rule** (2 connections) — `AGENTS.md`
- **Security-first contribution principle** (2 connections) — `CONTRIBUTING.md`
- **Procedures as CRUD Boundary** (2 connections) — `db/procedures/README.md`
- **Father-son Mythos MUD vision** (2 connections) — `PLANNING.md`
- **Comprehensive planning document** (2 connections) — `PLANNING.md`
- **Mandatory server startup rules** (2 connections) — `USER_RULES.md`
- **Obsidian LLM wiki permanent memory** (1 connections) — `AGENTS.md`
- **Server authority rule** (1 connections) — `AGENTS.md`
- **TDD write-tests-first workflow** (1 connections) — `CONTRIBUTING.md`
- **apply_procedures.ps1** (1 connections) — `db/procedures/README.md`
- **Container Functions Moved from DDL** (1 connections) — `db/procedures/README.md`
- **Named Schema Per Database** (1 connections) — `db/README.md`
- **Dual connection system with NATS** (1 connections) — `README.md`
- **Tech stack FastAPI React PostgreSQL** (1 connections) — `README.md`
- **no-direct-table-crud-in-python-sql rule** (1 connections) — `.semgrep.yml`
- **no-select-star-in-python-sql rule** (1 connections) — `.semgrep.yml`
- **Token efficiency over speed priority** (1 connections) — `USER_RULES.md`
- **CLAUDE.md agent router** (1 connections) — `CLAUDE.md`
- **Contributor Covenant Code of Conduct** (1 connections) — `CODE_OF_CONDUCT.md`

## Relationships

- No strong cross-community connections detected

## Source Files

- `.semgrep.yml`
- `AGENTS.md`
- `CLAUDE.md`
- `CODE_OF_CONDUCT.md`
- `CONTRIBUTING.md`
- `PLANNING.md`
- `README.md`
- `USER_RULES.md`
- `db/README.md`
- `db/procedures/README.md`

## Audit Trail

- EXTRACTED: 34 (74%)
- INFERRED: 12 (26%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*