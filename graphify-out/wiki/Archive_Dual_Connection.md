# Archive Dual Connection

> 8 nodes · cohesion 0.25

## Key Concepts

- **database** (4 connections) — `docs/examples/logging/migration_examples.py`
- **.execute()** (4 connections) — `docs/examples/logging/migration_examples.py`
- **.create_user()** (3 connections) — `docs/examples/logging/migration_examples.py`
- **migration_example_7()** (3 connections) — `docs/examples/logging/migration_examples.py`
- **Simulate database operations.** (1 connections) — `docs/examples/logging/migration_examples.py`
- **Simulate database execute.** (1 connections) — `docs/examples/logging/migration_examples.py`
- **Simulate user creation.** (1 connections) — `docs/examples/logging/migration_examples.py`
- **Example 7: Database logging migration.** (1 connections) — `docs/examples/logging/migration_examples.py`

## Relationships

- [AI Development Workflow](AI_Development_Workflow.md) (2 shared connections)
- [Argon2 Password Hashing](Argon2_Password_Hashing.md) (2 shared connections)

## Source Files

- `docs/examples/logging/migration_examples.py`

## Audit Trail

- EXTRACTED: 16 (89%)
- INFERRED: 2 (11%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*