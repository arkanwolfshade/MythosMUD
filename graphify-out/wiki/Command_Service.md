# command_service

> 7 nodes

## Key Concepts

- **command_service()** (4 connections) — `server/tests/unit/commands/test_command_service.py`
- **mock_request()** (3 connections) — `server/tests/unit/commands/test_command_service.py`
- **mock_user()** (3 connections) — `server/tests/unit/commands/test_command_service.py`
- **fixture** (3 connections)
- **Create a CommandService instance.** (1 connections) — `server/tests/unit/commands/test_command_service.py`
- **Create a mock request object.** (1 connections) — `server/tests/unit/commands/test_command_service.py`
- **Create a mock user object.** (1 connections) — `server/tests/unit/commands/test_command_service.py`

## Relationships

- [test_command_service.py](test_command_service.py.md) (3 shared connections)
- [CommandService](CommandService.md) (1 shared connections)

## Source Files

- `server/tests/unit/commands/test_command_service.py`

## Audit Trail

- EXTRACTED: 9 (90%)
- INFERRED: 1 (10%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*