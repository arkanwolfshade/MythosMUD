# Database Architecture

> 5 nodes

## Key Concepts

- **Database Architecture** (4 connections) — `docs/architecture/DATABASE_ARCHITECTURE.md`
- **ADR-005: Repository Pattern for Data Access** (1 connections) — `docs/architecture/decisions/ADR-005-repository-pattern-data-access.md`
- **ADR-006: PostgreSQL as Primary Datastore** (1 connections) — `docs/architecture/decisions/ADR-006-postgresql-primary-datastore.md`
- **ADR-007: FastAPI with Async/Await** (1 connections) — `docs/architecture/decisions/ADR-007-fastapi-async-await.md`
- **ADR-015: PostgreSQL Procedures and Functions for Data Access** (1 connections) — `docs/architecture/decisions/ADR-015-postgresql-procedures-migration.md`

## Relationships

- No strong cross-community connections detected

## Source Files

- `docs/architecture/DATABASE_ARCHITECTURE.md`
- `docs/architecture/decisions/ADR-005-repository-pattern-data-access.md`
- `docs/architecture/decisions/ADR-006-postgresql-primary-datastore.md`
- `docs/architecture/decisions/ADR-007-fastapi-async-await.md`
- `docs/architecture/decisions/ADR-015-postgresql-procedures-migration.md`

## Audit Trail

- EXTRACTED: 4 (100%)
- INFERRED: 0 (0%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*