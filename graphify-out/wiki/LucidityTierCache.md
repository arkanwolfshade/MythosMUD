...
- **Fractured (also hallucination-eligible for phantoms) is not deranged for exits.** (1 connections) — `server/tests/unit/services/test_lucidity_tier_cache.py`
- **A UUID and its string form must key into the same cache entry.** (1 connections) — `server/tests/unit/services/test_lucidity_tier_cache.py`
