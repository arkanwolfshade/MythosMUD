# Server Models (53)

> 2 nodes

## Key Concepts

- **test_room_add_player_silently()** (3 connections) — `server/tests/unit/models/test_room_class.py`
- **Test Room.add_player_silently() adds player without event.** (1 connections) — `server/tests/unit/models/test_room_class.py`

## Relationships

- [Server Models (23)](Server_Models_%2823%29.md) (1 shared connections)
- [Server Models (18)](Server_Models_%2818%29.md) (1 shared connections)

## Source Files

- `server/tests/unit/models/test_room_class.py`

## Audit Trail

- EXTRACTED: 4 (100%)
- INFERRED: 0 (0%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*