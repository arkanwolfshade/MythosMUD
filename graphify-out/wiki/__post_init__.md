# .__post_init__

> 69 nodes

## Key Concepts

- **.__post_init__()** (21 connections) — `server/events/event_types.py`
- **MythosHourTickEvent** (11 connections) — `server/events/event_types.py`
- **communication_integration.py** (11 connections) — `server/npc/communication_integration.py`
- **NPCSpoke** (10 connections) — `server/events/event_types.py`
- **NPCCommunicationIntegration** (10 connections) — `server/npc/communication_integration.py`
- **NPCListened** (9 connections) — `server/events/event_types.py`
- **QuestCompleted** (8 connections) — `server/events/event_types.py`
- **PlayerMortallyWoundedEvent** (4 connections) — `server/events/event_types.py`
- **.handle_player_message()** (4 connections) — `server/npc/communication_integration.py`
- **.__init__()** (4 connections) — `server/npc/communication_integration.py`
- **._process_message_for_response()** (4 connections) — `server/npc/communication_integration.py`
- **.send_message_to_room()** (4 connections) — `server/npc/communication_integration.py`
- **.__post_init__()** (3 connections) — `server/events/event_types.py`
- **.__post_init__()** (3 connections) — `server/events/event_types.py`
- **.__post_init__()** (3 connections) — `server/events/event_types.py`
- **.__post_init__()** (3 connections) — `server/events/event_types.py`
- **.__post_init__()** (3 connections) — `server/events/event_types.py`
- **.__post_init__()** (3 connections) — `server/events/event_types.py`
- **.__post_init__()** (3 connections) — `server/events/event_types.py`
- **.__post_init__()** (3 connections) — `server/events/event_types.py`
- **.__post_init__()** (3 connections) — `server/events/event_types.py`
- **.__post_init__()** (3 connections) — `server/events/event_types.py`
- **.__post_init__()** (3 connections) — `server/events/event_types.py`
- **.__post_init__()** (3 connections) — `server/events/event_types.py`
- **.__post_init__()** (3 connections) — `server/events/event_types.py`
- *... and 44 more nodes in this community*

## Relationships

- [get_npc_instance_service](get_npc_instance_service.md) (16 shared connections)
- [EventBus](EventBus.md) (10 shared connections)
- [BaseEvent](BaseEvent.md) (6 shared connections)
- [ScheduleService](ScheduleService.md) (4 shared connections)
- [QuestService](QuestService.md) (3 shared connections)
- [get_logger](get_logger.md) (3 shared connections)
- [GameBundle](GameBundle.md) (2 shared connections)
- [threading.py](threading.py.md) (2 shared connections)
- [ChatService](ChatService.md) (2 shared connections)
- [PartyService](PartyService.md) (1 shared connections)
- [PlayerEnteredRoom](PlayerEnteredRoom.md) (1 shared connections)
- [ConnectionManager](ConnectionManager.md) (1 shared connections)

## Source Files

- `server/events/event_types.py`
- `server/npc/communication_integration.py`
- `server/npc/npc_base.py`

## Audit Trail

- EXTRACTED: 124 (98%)
- INFERRED: 3 (2%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*