...
- **_validate_hasher_params()** (3 connections) — `server/auth/argon2_utils.py`
- **_warn_if_hasher_params_below_recommended()** (3 connections) — `server/auth/argon2_utils.py`
...
- **Reject out-of-range Argon2 parameters (same ranges as the module-level…** (1 connections) — `server/auth/argon2_utils.py`
- **Log a warning when parameters are valid but below the recommended-security…** (1 connections) — `server/auth/argon2_utils.py`
...
- [test_auth_utils.py](test_auth_utils.py.md) (11 shared connections)
