...
- **_generate_narrative_recap()** (5 connections) — `server/commands/debrief_command.py`
...
- **Debrief command for MythosMUD. After sanitarium failover (LCD -100), players…** (1 connections) — `server/commands/debrief_command.py`
- **Complete debrief by clearing cooldown and finalizing message.** (1 connections) — `server/commands/debrief_command.py`
- **Handle the debrief command after sanitarium failover. Provides narrative recap…** (1 connections) — `server/commands/debrief_command.py`
