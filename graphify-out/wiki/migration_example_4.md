# migration_example_4

> 4 nodes

## Key Concepts

- **migration_example_4()** (4 connections) — `docs/examples/logging/migration_examples.py`
- **expensive_operation()** (3 connections) — `docs/examples/logging/migration_examples.py`
- **Simulate an expensive operation.** (1 connections) — `docs/examples/logging/migration_examples.py`
- **Example 4: Performance logging migration.** (1 connections) — `docs/examples/logging/migration_examples.py`

## Relationships

- [migration_examples.py](migration_examples.py.md) (2 shared connections)
- [fastapi_integration.py](fastapi_integration.py.md) (1 shared connections)

## Source Files

- `docs/examples/logging/migration_examples.py`

## Audit Trail

- EXTRACTED: 5 (83%)
- INFERRED: 1 (17%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*