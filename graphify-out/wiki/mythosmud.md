# Mythosmud

> 17 nodes

## Key Concepts

- **Geography Overview.md** (6 connections) — `data/MythosMUD-Obsidian/wiki/world/Geography Overview.md`
- **MythosMUD.md** (6 connections) — `data/MythosMUD-Obsidian/wiki/world/MythosMUD.md`
- **Graphify Code Graph.md** (4 connections) — `data/MythosMUD-Obsidian/wiki/concepts/Graphify Code Graph.md`
- **MythosMUD** (4 connections) — `data/MythosMUD-Obsidian/wiki/world/MythosMUD.md`
- **Geography and Major Locations.md** (3 connections) — `data/MythosMUD-Obsidian/wiki/sources/Geography and Major Locations.md`
- **Code Graph Entry** (3 connections) — `data/MythosMUD-Obsidian/wiki/code/Code Graph Entry.md`
- **Graphify Code Graph** (3 connections) — `data/MythosMUD-Obsidian/wiki/concepts/Graphify Code Graph.md`
- **Code Graph Entry.md** (2 connections) — `data/MythosMUD-Obsidian/wiki/code/Code Graph Entry.md`
- **Live exploration (preferred for "how does X work?")** (1 connections) — `data/MythosMUD-Obsidian/wiki/code/Code Graph Entry.md`
- **Synced community wiki (read-only dump)** (1 connections) — `data/MythosMUD-Obsidian/wiki/code/Code Graph Entry.md`
- **Chaosium pack graphs (external)** (1 connections) — `data/MythosMUD-Obsidian/wiki/concepts/Graphify Code Graph.md`
- **Relationship to this vault** (1 connections) — `data/MythosMUD-Obsidian/wiki/concepts/Graphify Code Graph.md`
- **Geography and Major Locations (source summary)** (1 connections) — `data/MythosMUD-Obsidian/wiki/sources/Geography and Major Locations.md`
- **Geography Overview** (1 connections) — `data/MythosMUD-Obsidian/wiki/world/Geography Overview.md`
- **Engineering memory** (1 connections) — `data/MythosMUD-Obsidian/wiki/world/MythosMUD.md`
- **Sources** (1 connections) — `data/MythosMUD-Obsidian/wiki/world/MythosMUD.md`
- **World** (1 connections) — `data/MythosMUD-Obsidian/wiki/world/MythosMUD.md`

## Relationships

- [Nameless horrors   2nd edition](Nameless_horrors___2nd_edition.md) (2 shared connections)
- [Does love forgive](Does_love_forgive.md) (1 shared connections)
- [Chaosium coc catalog](Chaosium_coc_catalog.md) (1 shared connections)
- [Reign of terror](Reign_of_terror.md) (1 shared connections)
- [Lucidity](Lucidity.md) (1 shared connections)

## Source Files

- `data/MythosMUD-Obsidian/wiki/code/Code Graph Entry.md`
- `data/MythosMUD-Obsidian/wiki/concepts/Graphify Code Graph.md`
- `data/MythosMUD-Obsidian/wiki/sources/Geography and Major Locations.md`
- `data/MythosMUD-Obsidian/wiki/world/Geography Overview.md`
- `data/MythosMUD-Obsidian/wiki/world/MythosMUD.md`

## Audit Trail

- EXTRACTED: 23 (100%)
- INFERRED: 0 (0%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*