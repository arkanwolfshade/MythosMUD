# server tests unit services test

> 3 nodes

## Key Concepts

- **test_update_default_channel_invalid_channel()** (3 connections) — `server/tests/unit/services/test_player_preferences_service.py`
- **test_update_default_channel_invalid_id()** (3 connections) — `server/tests/unit/services/test_player_preferences_service.py`
- **Test updating default channel with invalid ID.** (2 connections) — `server/tests/unit/services/test_player_preferences_service.py`

## Relationships

- [server tests unit services test](server_tests_unit_services_test.md) (4 shared connections)

## Source Files

- `server/tests/unit/services/test_player_preferences_service.py`

## Audit Trail

- EXTRACTED: 6 (100%)
- INFERRED: 0 (0%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*