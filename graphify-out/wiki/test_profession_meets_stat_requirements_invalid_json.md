# test_profession_meets_stat_requirements_invalid_json

> 2 nodes

## Key Concepts

- **test_profession_meets_stat_requirements_invalid_json()** (3 connections) — `server/tests/unit/models/test_profession.py`
- **Test meets_stat_requirements returns True when stat_requirements is invalid…** (1 connections) — `server/tests/unit/models/test_profession.py`

## Relationships

- [Profession](Profession.md) (1 shared connections)
- [test_profession.py](test_profession.py.md) (1 shared connections)

## Source Files

- `server/tests/unit/models/test_profession.py`

## Audit Trail

- EXTRACTED: 4 (100%)
- INFERRED: 0 (0%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*