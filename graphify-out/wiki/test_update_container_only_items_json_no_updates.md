# test_update_container_only_items_json_no_updates

> 2 nodes

## Key Concepts

- **test_update_container_only_items_json_no_updates()** (3 connections) — `server/tests/unit/container_persistence/test_container_persistence.py`
- **Test update_container with only items_json (no lock_state or metadata_json).** (1 connections) — `server/tests/unit/container_persistence/test_container_persistence.py`

## Relationships

- [container_persistence/container_persistence.py](container_persistence-container_persistence.py.md) (1 shared connections)
- [test_container_persistence.py](test_container_persistence.py.md) (1 shared connections)

## Source Files

- `server/tests/unit/container_persistence/test_container_persistence.py`

## Audit Trail

- EXTRACTED: 4 (100%)
- INFERRED: 0 (0%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*