# useProfessions.ts

> 18 nodes

## Key Concepts

- **useProfessions.ts** (21 connections) — `client/src/hooks/useProfessions.ts`
- **ProfessionSelectionScreen.tsx** (14 connections) — `client/src/components/ProfessionSelectionScreen.tsx`
- **loadProfessions()** (5 connections) — `client/src/hooks/useProfessions.ts`
- **parseProfessionsErrorResponse()** (5 connections) — `client/src/hooks/useProfessions.ts`
- **useProfessions()** (4 connections) — `client/src/hooks/useProfessions.ts`
- **ProfessionSelectionScreen.test.tsx** (4 connections) — `client/src/components/ProfessionSelectionScreen.test.tsx`
- **ProfessionSelectionScreen()** (3 connections) — `client/src/components/ProfessionSelectionScreen.tsx`
- **parseProfessionsBody()** (3 connections) — `client/src/hooks/useProfessions.ts`
- **ProfessionSelectionContentProps** (2 connections) — `client/src/components/ProfessionSelectionScreen.tsx`
- **ProfessionSelectionScreenProps** (2 connections) — `client/src/components/ProfessionSelectionScreen.tsx`
- **UseProfessionsResult** (2 connections) — `client/src/hooks/useProfessions.ts`
- **handleProfessionsFetchError()** (2 connections) — `client/src/hooks/useProfessions.ts`
- **parseDetailMessage()** (2 connections) — `client/src/hooks/useProfessions.ts`
- **ProfessionSelectionScreen** (2 connections) — `client/src/mythosApp/appLazyScreens.tsx`
- **UseProfessionsOptions** (1 connections) — `client/src/hooks/useProfessions.ts`
- **ProfessionSelectionContent()** (1 connections) — `client/src/components/ProfessionSelectionScreen.tsx`
- **fetchSpy** (1 connections) — `client/src/components/ProfessionSelectionScreen.test.tsx`
- **SERVER_UNAVAILABLE_PATTERNS** (1 connections) — `client/src/hooks/useProfessions.ts`

## Relationships

- [useMythosAppState.ts](useMythosAppState.ts.md) (8 shared connections)
- [useMythosAppActions.ts](useMythosAppActions.ts.md) (5 shared connections)
- [logger.ts](logger.ts.md) (4 shared connections)
- [apiTypeGuards.ts](apiTypeGuards.ts.md) (3 shared connections)
- [SkillAssignmentScreen.tsx](SkillAssignmentScreen.tsx.md) (2 shared connections)
- [security.ts](security.ts.md) (2 shared connections)
- [GameLogPanel.tsx](GameLogPanel.tsx.md) (1 shared connections)

## Source Files

- `client/src/components/ProfessionSelectionScreen.test.tsx`
- `client/src/components/ProfessionSelectionScreen.tsx`
- `client/src/hooks/useProfessions.ts`
- `client/src/mythosApp/appLazyScreens.tsx`

## Audit Trail

- EXTRACTED: 50 (100%)
- INFERRED: 0 (0%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*