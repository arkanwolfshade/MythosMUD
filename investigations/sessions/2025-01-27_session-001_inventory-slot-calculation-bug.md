...
When logged in as "ArkanWolfshade/Cthulhu1" and running the `/inventory` command, the display initially showed "1
 of 20 slots in use" even though the item (Phosphor Charm) was in an equipped backpack. The expected behavior is:
...
**Approach**: When items are picked up and have `slot_type='backpack'`, automatically place them into the equipped
 backpack container if one exists.
