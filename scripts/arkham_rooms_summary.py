...
def generate_dot_file(rooms: Dict, intersections: Dict, connections: Set, output_path: str = "arkham_city_graph.dot"):
    ...
    with open(output_path, 'w') as f:
        ...
        for intersection_id, intersection_data in intersections.items():
            ...
            f.write(f'  "{intersection_id}" [label="{intersection_data["name"]}", fillcolor="{color}", shape=diamond];\n')
...
def main():
    ...
    print(f"across {len(set(room['subzone'] for room in rooms.values()) | set(intersection['subzone'] for intersection in intersections.values()))} subzones")
