...
<body>
    <div class="container">
        ...
        <div class="stats">
            ...
            <div class="stat-card">
                <div class="stat-number">{len(set(room['subzone'] for room in rooms.values()) | set(intersection['subzone'] for intersection in intersections.values()))}</div>
            ...
            <div class="controls">
                <div class="control-group">
                    ...
                    <select id="subzone-filter">
                        ...
                        {chr(10).join(f'<option value="{subzone}">{subzone.title()}</option>' for subzone in sorted(set(room['subzone'] for room in rooms.values()) | set(intersection['subzone'] for intersection in intersections.values())))}
                    </select>
                </div>
                <div class="control-group">
        ...
        <div class="room-list">
            ...
            <div class="subzone-section">
                ...
                <div class="room-item">
                    ...
                    <div class="room-exits">Exits: {', '.join(f"{dir}: {target}" for dir, target in room_data['exits'].items() if target) if any(target for target in room_data['exits'].values()) else 'None'}</div>
                </div>''' for room_id, room_data in sorted([(rid, rdata) for rid, rdata in rooms.items() if rdata['subzone'] == subzone])))}
                {chr(10).join(f'''
                <div class="intersection-item">
                    <div class="room-name">{intersection_data['name']}</div>
                    <div class="room-id">{intersection_id}</div>
                    <div class="room-exits">Exits: {', '.join(f"{dir}: {target}" for dir, target in intersection_data['exits'].items() if target) if any(target for target in intersection_data['exits'].values()) else 'None'}</div>
                </div>''' for intersection_id, intersection_data in sorted([(iid, idata) for iid, idata in intersections.items() if idata['subzone'] == subzone]))}
            </div>''' for subzone in sorted(set(room['subzone'] for room in rooms.values()) | set(intersection['subzone'] for intersection in intersections.values())))}
        </div>
    </div>
...
def main():
    ...
    print(f"\n=== Summary ===")
    print(f"Generated interactive HTML visualization with:")
    print(f"- {len(rooms)} rooms")
    print(f"- {len(intersections)} intersections") 
    print(f"- {len(connections)} connections")
    print(f"- {len(set(room['subzone'] for room in rooms.values()) | set(intersection['subzone'] for intersection in intersections.values()))} subzones")
