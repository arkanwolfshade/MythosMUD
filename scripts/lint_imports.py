...
def _print_failure(output: str, returncode: int) -> None:
    ...
    print("See .importlinter and docs/architecture/decisions/ADR-001-layered-architecture-event-driven.md", file=sys.stderr)
    print(BANNER, file=sys.stderr)
    if output.strip():
...
def main() -> int:
    ...
    probe = safe_run_static(*cmd, "--version", cwd=PROJECT_ROOT, capture_output=True, text=True)
    ...
    result = safe_run_static(*cmd, cwd=PROJECT_ROOT, capture_output=True, text=True)
