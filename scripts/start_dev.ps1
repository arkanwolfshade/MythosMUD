...
if ($Help) {
    ...
    Write-Host ""
    Write-Host "Usage: .\start_dev.ps1 [-ServerPort <port>] [-ClientPort <port>] [-ServerOnly] [-ClientOnly] [-Help]"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "Examples:"
    Write-Host "    .\start_dev.ps1                           # Start both server and client"
    Write-Host "    .\start_dev.ps1 -ServerOnly              # Start only the server"
    Write-Host "    .\start_dev.ps1 -ClientOnly              # Start only the client"
    Write-Host "    .\start_dev.ps1 -ServerPort 4001         # Start server on port 4001"
    Write-Host "    .\start_dev.ps1 -ClientPort 3001         # Start client on port 3001"
    Write-Host "    .\start_dev.ps1 -Help                    # Show help"
    exit 0
}
...
function Start-Server {
    ...
    Start-Process powershell -ArgumentList "-NoExit", "-Command", "cd '$PWD'; .\scripts\start_server.ps1 -Port $ServerPort" -WindowStyle Normal
}
