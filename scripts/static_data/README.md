...
This directory contains scripts for generating SQL seed data files from JSON source data.

## Purpose

The `generate_sql.mjs` script reads JSON data files and generates PostgreSQL INSERT statements for:
...
The script uses AJV (Another JSON Schema Validator) to validate all input data before generating SQL. Validation errors will cause the script to exit with an error.
...
- Zone and subzone configurations are updated with `ON CONFLICT DO UPDATE` to allow config changes
