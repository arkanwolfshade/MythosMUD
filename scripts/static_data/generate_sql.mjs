...
async function generateRooms() {
  ...
  for (const [zoneStable, zoneName] of insertZones.entries()) {
    ...
    const weatherPatterns = zoneConfig?.weather_patterns
      ? JSON.stringify(zoneConfig.weather_patterns).replaceAll("'", "''")
      : "[]";
    const specialRules = zoneConfig?.special_rules
      ? JSON.stringify(zoneConfig.special_rules).replaceAll("'", "''")
      : "{}";
  ...
  for (const s of insertSubzones) {
    ...
    if (!subzoneIdByStable.has(key)) {
      ...
      const specialRules = subzoneConfig?.special_rules
        ? JSON.stringify(subzoneConfig.special_rules).replaceAll("'", "''")
        : "{}";
    ...
    if (base.endsWith("subzone_config.json")) {
      ...
      if (roomsIndex >= 0 && planeIndex > 0 && planeIndex < parts.length - 3) {
        ...
        if (plane && zone && subzone) {
          ...
          console.warn(
            `Warning: Missing plane/zone/subzone in path: ${file} (plane=${plane}, zone=${zone}, subzone=${subzone})`,
          );
        }
      } else {
        console.warn(
          `Warning: Could not parse subzone config path: ${file} (roomsIndex=${roomsIndex}, planeIndex=${planeIndex}, parts.length=${parts.length})`,
        );
      }
