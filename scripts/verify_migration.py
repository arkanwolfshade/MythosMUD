...
async def _check_table_exists(conn: asyncpg.Connection, db_name: str) -> bool:
...
async def _validate_table_schema(conn: asyncpg.Connection) -> tuple[bool, dict[str, str]]:
    ...
    return schema_ok, found_columns


async def _validate_json_data(conn: asyncpg.Connection) -> tuple[list[str], list[str]]:
    """Validate JSON data integrity in players table."""
    json_errors = []
    stats_errors = []
    ...
    return json_errors, stats_errors


async def _check_null_values(conn: asyncpg.Connection) -> list[str]:
...
async def _validate_database_data(conn: asyncpg.Connection, row_count: int) -> list[str]:
    """Validate database data (JSON, NULLs, sample)."""
    if row_count == 0:
    ...
    json_errors, stats_errors = await _validate_json_data(conn)

    _print_json_validation_results(json_errors, stats_errors)

    # Check for NULL values in required fields
    null_issues = await _check_null_values(conn)
    if null_issues:
    ...
    # Sample data check
    await _print_sample_data(conn)

    return json_errors


def _print_json_validation_results(json_errors: list[str], stats_errors: list[str]) -> None:
...
async def _print_sample_data(conn: asyncpg.Connection) -> None:
...
async def _check_foreign_keys(conn: asyncpg.Connection) -> None:
...
def _print_verification_summary(db_name: str, schema_ok: bool, json_errors: list[str]) -> bool:
    """Print verification summary and return result."""
    print(f"\n{'=' * 60}")
...
async def verify_database(db_name: str, host: str, port: int, user: str, password: str) -> bool:
    ...
    try:
        ...
        try:
            ...
            if not await _check_table_exists(conn, db_name):
                return False

            # 2. Check table schema
            schema_ok, found_columns = await _validate_table_schema(conn)

            # 3. Check row count and validate data
            row_count = await conn.fetchval("SELECT COUNT(*) FROM players")
            print(f"\n[INFO] Total players: {row_count}")

            json_errors = await _validate_database_data(conn, row_count)

            # 7. Check foreign key constraints
            await _check_foreign_keys(conn)

            # Summary
            return _print_verification_summary(db_name, schema_ok, json_errors)
