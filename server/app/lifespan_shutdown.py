...
from .lifespan_protocols import (
    lifespan_connection_manager,
    lifespan_event_bus,
    lifespan_memory_monitor,
    lifespan_nats_handler,
    lifespan_task_registry,
    lifespan_tick_scheduler,
)
...
async def _shutdown_nats_handler(app: FastAPI) -> None:
    ...
    nats_message_handler = lifespan_nats_handler(app)
    if not nats_message_handler:
        return

    logger.info("Stopping NATS message handler")
    try:
        _ = await nats_message_handler.stop()
...
async def _shutdown_connection_manager(app: FastAPI) -> None:
    ...
    connection_manager = lifespan_connection_manager(app)
    if not connection_manager:
        return

    memory_monitor = lifespan_memory_monitor(app)
    if memory_monitor is not None:
        try:
            await memory_monitor.stop_idle_sampler()
        except (AttributeError, TypeError, RuntimeError) as e:
            logger.error("Error stopping idle memory sampler", error=str(e))
...
async def _shutdown_mythos_tick_scheduler(app: FastAPI) -> None:
    ...
    mythos_tick_scheduler = lifespan_tick_scheduler(app)
...
async def _shutdown_task_registry(container: ApplicationContainer) -> None:
    ...
    task_registry = lifespan_task_registry(container)
    if not task_registry:
        return

    logger.info("Executing TaskRegistry graceful shutdown coordination")
    try:
        shutdown_success = await task_registry.shutdown_all(timeout=5.0)
        if shutdown_success:
...
async def _shutdown_event_bus(container: ApplicationContainer) -> None:
    ...
    event_bus = lifespan_event_bus(container)
    if not event_bus:
        return
