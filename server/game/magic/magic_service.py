...
# Runtime cycle is broken via TYPE_CHECKING / injected combat; checker still sees mixin edges.
# pylint: disable=too-many-return-statements,too-many-lines,missing-class-docstring,missing-function-docstring,too-few-public-methods  # Reason: Magic service size; Protocol stubs (PEP 544)
...
class _StatsPlayer(Protocol):
    def get_stats(self) -> dict[str, object]: ...


class _PlayerPersistence(Protocol):
    async def get_player_by_id(self, player_id: uuid.UUID) -> _StatsPlayer | None: ...


class _CombatTickState(Protocol):
    next_turn_tick: int | None
    turn_interval_ticks: int


def _stat_int(stats: dict[str, object], key: str, default: int) -> int:
...
class _MagicServiceCore(MagicServiceCompletionMixin, MagicServiceHealingMixin):  # pylint: disable=too-many-instance-attributes  # Reason: Magic service requires many configuration and state tracking attributes
    ...
    def _player_persistence(self) -> _PlayerPersistence:
        return cast(_PlayerPersistence, cast(object, self.player_service.persistence))

    async def _get_player_and_normalized_stats(
        self, player_id: uuid.UUID
    ) -> tuple[_StatsPlayer | None, dict[str, object] | None]:
        """Load player and return normalized stats (MP/max_MP). Returns (player, stats) or (None, None)."""
        player = await self._player_persistence().get_player_by_id(player_id)
        if not player:
            return None, None
        stats = player.get_stats()
        pow_val = _stat_int(stats, "power", 50)
        if "max_magic_points" not in stats:
            stats["max_magic_points"] = math.ceil(pow_val * 0.2)
        if not _stat_int(stats, "magic_points", 0) and _stat_int(stats, "max_magic_points", 0) > 0:
            stats["magic_points"] = stats["max_magic_points"]
        return player, stats

    def _check_mp_sufficient(self, stats: dict[str, object], spell: Spell) -> tuple[bool, str]:
        """Return (False, message) if not enough MP, else (True, '')."""
        current_mp = _stat_int(stats, "magic_points", 0)
        if current_mp < spell.mp_cost:
            return False, f"Not enough magic points. You need {spell.mp_cost} MP, but only have {current_mp}."
        return True, ""

    def _check_lucidity_sufficient(self, stats: dict[str, object], spell: Spell) -> tuple[bool, str]:
        """Return (False, message) if Mythos spell and not enough lucidity, else (True, '')."""
        if not (spell.is_mythos() and spell.requires_lucidity()):
            return True, ""
        current_lucidity = _stat_int(stats, "lucidity", 100)
    ...
    def _check_already_casting(self, player_id: uuid.UUID) -> JsonMap | None:
    ...
    def _get_spell_from_registry(self, spell_id: str) -> Spell | None:
    ...
    async def _validate_spell_casting(
        self, player_id: uuid.UUID, spell: Spell, target_name: str | None
    ) -> tuple[TargetMatch | None, str | None]:
    ...
    async def _handle_instant_cast(
        self, player_id: uuid.UUID, spell: Spell, target: TargetMatch, mastery: int
    ) -> JsonMap:
        ...
        _ = await self.player_spell_repository.record_spell_cast(player_id, spell.spell_id)

        if self.spell_learning_service:
            await self.spell_learning_service.increase_mastery_on_cast(player_id, spell.spell_id, True)

        default_msg = f"{spell.name} cast successfully."
        return {
            "success": True,
            "message": str(effect_result.get("message", default_msg)),
    ...
    def _calculate_initiative_tick(self, combat: _CombatTickState, current_tick: int) -> int | None:
        """
    ...
    async def _start_delayed_cast(  # pylint: disable=too-many-arguments,too-many-positional-arguments  # Reason: Delayed cast requires many parameters for spell context and timing
        self, player_id: uuid.UUID, spell: Spell, target: TargetMatch, mastery: int, current_tick: int
    ) -> JsonMap:
        ...
        if self.combat_service:
            ...
            if combat:
                ...
                next_initiative_tick = self._calculate_initiative_tick(
                    cast(_CombatTickState, cast(object, combat)), current_tick
                )

        target_type_str = str(target.target_type.value)

        try:
            _ = self.casting_state_manager.start_casting(
                player_id,
                spell,
                current_tick,
                StartCastingTarget(
    ...
    ) -> tuple[Spell | None, TargetMatch | None, JsonMap | None]:
    ...
    async def _consume_materials_if_required(self, player_id: uuid.UUID, spell: Spell) -> JsonMap | None:
        """Consume materials when spell requires them. Returns error dict to return, or None to continue."""
        if not spell.materials:
            return None
        material_result = cast(
            JsonMap, cast(object, await self.spell_materials_service.consume_materials(player_id, spell))
        )
        if not material_result.get("success"):
            return {
                "success": False,
                "message": str(material_result.get("message", "Failed to consume materials.")),
            }
        return None

    async def _casting_roll_or_fail_result(self, player_id: uuid.UUID, spell: Spell, mastery: int) -> JsonMap | None:
    ...
    async def cast_spell(self, player_id: uuid.UUID, spell_id: str, target_name: str | None = None) -> JsonMap:
        """
        Cast a spell.
    ...
    async def _casting_roll(self, player_id: uuid.UUID, spell: Spell, mastery: int) -> bool:
        ...
        stats = player.get_stats()
        intelligence = _stat_int(stats, "intelligence", 50)
    ...
    async def restore_mp(self, player_id: uuid.UUID, amount: int) -> JsonMap:
        """
        ...
        Returns:
            dict: Result message
        """
        return cast(JsonMap, cast(object, await self.spell_costs_service.restore_mp(player_id, amount)))
    ...
    async def send_spell_execution_notifications(
        self, player_id: uuid.UUID, spell_id: str, effect_result: JsonMap, room_id: str
    ) -> None:
        """
    ...
    @override
    async def _send_spell_completion_message(self, player_id: uuid.UUID, spell_id: str, effect_result: JsonMap) -> None:
        ...
        try:
            ...
            if effect_result.get("success") and effect_result.get("message"):
                ...
                failure_message = str(effect_result.get("message", "The spell failed."))
                await send_game_event(
                    player_id,
                    "command_response",
                    {
    ...
    async def interrupt_casting(self, player_id: uuid.UUID) -> JsonMap:
        """
        ...
        _ = self.casting_state_manager.interrupt_casting(player_id)
        ...
        interrupted_spell = self.spell_registry.get_spell(casting_state.spell_id)
        if interrupted_spell is not None:
            await self.spell_costs_service.apply_costs(player_id, interrupted_spell)
        return {
            "success": True,
