...
def _npc_has_full_combat_data(stats: dict[str, Any], config: dict[str, Any]) -> bool:
...
def _record_npc_error(results: dict[str, Any], error_key: str, npc: NPCDefinition, exc: Exception) -> None:
...
async def _migrate_one_npc(npc: NPCDefinition, session: AsyncSession, dry_run: bool, results: dict[str, Any]) -> None:
...
def _validate_one_npc(npc: NPCDefinition, results: dict[str, Any]) -> None:
    validate_npc_combat_data(npc)
...
def _npc_has_combat_data(stats: dict[str, Any], config: dict[str, Any]) -> bool:
    return any(key in stats for key in _COMBAT_STATS_KEYS) or any(key in config for key in _COMBAT_CONFIG_KEYS)


async def _rollback_one_npc(npc: NPCDefinition, session: AsyncSession, results: dict[str, Any]) -> None:
    current_stats = npc.get_base_stats()
    current_config = npc.get_behavior_config()
