...
class CombatService:  # pylint: disable=too-many-instance-attributes  # Reason: Combat service requires many state tracking and service attributes
    ...
    def _get_combat_for_flee_failed(self, combat_id: UUID) -> CombatInstance | None:
        """Return active combat for flee-failed free hits, or None if not found/inactive."""
        combat = self._active_combats.get(combat_id)
        if not combat or combat.status != CombatStatus.ACTIVE:
            logger.warning(
                "execute_flee_failed_free_hits: combat not found or not active",
                combat_id=combat_id,
            )
            return None
        return combat

    def _get_fleeing_and_opponents(
        self, combat: CombatInstance, fleeing_participant_id: UUID
    ) -> tuple[CombatParticipant, list[CombatParticipant]] | None:
        """Return (fleeing participant, opponents list) or None if fleeing missing/dead."""
        fleeing = combat.participants.get(fleeing_participant_id)
        if not fleeing or fleeing.is_dead():
            return None
        opponents = [
            p
            for p in combat.participants.values()
        ...
        return (fleeing, opponents)

    def _get_flee_failed_free_hits_context(
        self, combat_id: UUID, fleeing_participant_id: UUID
    ) -> tuple[CombatInstance, CombatParticipant, list[CombatParticipant]] | None:
        """Resolve combat, fleeing participant, and opponents list. Returns None if combat invalid or fleeing dead."""
        combat = self._get_combat_for_flee_failed(combat_id)
        if not combat:
            return None
        result = self._get_fleeing_and_opponents(combat, fleeing_participant_id)
        if not result:
            return None
        fleeing, opponents = result
        return (combat, fleeing, opponents)

    def _get_free_hit_damage(self) -> int:
        """Default damage for flee-failed free hits; fallback 10 on config error or non-int."""
        try:
            damage = get_config().game.basic_unarmed_damage
            return damage if isinstance(damage, int) else 10
        except (ImportError, AttributeError, ValueError, TypeError):
            return 10

    async def _apply_one_opponent_free_hit(
        self,
        combat_id: UUID,
        ...
        """Apply one opponent's free hit; return True if combat ended (caller should stop loop)."""
        ...
        if not fleeing or fleeing.is_dead():
            return True
        result = await self.process_attack(
            attacker_id=opponent.participant_id,
            target_id=fleeing_participant_id,
            damage=damage,
            is_initial_attack=False,
        )
        return result.combat_ended

    async def execute_flee_failed_free_hits(self, combat_id: UUID, fleeing_participant_id: UUID) -> None:
        """
        Apply one free attack from each opponent against the fleeing participant.

        Used when voluntary flee roll fails. Each alive, able opponent attacks
        the fleeing participant once. If the fleeing participant dies, combat
        ends and remaining free hits are not applied.

        Args:
            combat_id: ID of the combat
            fleeing_participant_id: Participant who failed to flee
        """
        context = self._get_flee_failed_free_hits_context(combat_id, fleeing_participant_id)
        if not context:
            return
        _combat, _fleeing, opponents = context
        damage = self._get_free_hit_damage()
        for opponent in opponents:
            if await self._apply_one_opponent_free_hit(combat_id, fleeing_participant_id, opponent, damage):
                break
