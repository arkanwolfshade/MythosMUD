...
class EnvironmentalContainerLoader:
    ...
    def _validate_container_capacity(self, capacity_slots: int, room_id: str) -> None:
        if 1 <= capacity_slots <= 20:
            return
        log_and_raise(
            ValidationError,
    ...
    def _parse_lock_state(self, lock_state_str: str, room_id: str) -> ContainerLockState:
        try:
            return ContainerLockState(lock_state_str)
        except ValueError:
            log_and_raise(
                ValidationError,
    ...
    def load_container_from_room_json(self, room_json: dict[str, Any], room_id: str) -> ContainerComponent | None:
        """Load environmental container definition from room JSON."""
        container_block = room_json.get("container")
        if not container_block or not container_block.get("enabled", True):
            return None
        if room_json.get("id") != room_id:
            logger.warning("Room ID mismatch", json_room_id=room_json.get("id"), provided_room_id=room_id)
        capacity_slots = container_block.get("capacity_slots", 20)
        self._validate_container_capacity(capacity_slots, room_id)
        lock_state = self._parse_lock_state(container_block.get("lock_state", "unlocked"), room_id)
        allowed_roles = container_block.get("allowed_roles", [])
        key_item_id = container_block.get("key_item_id")
        metadata = {"key_item_id": key_item_id} if key_item_id else {}
    ...
    async def migrate_room_container_to_postgresql(self, room_json: dict[str, Any], room_id: str) -> UUID | None:
        """migrate_room_container_to_postgresql."""
