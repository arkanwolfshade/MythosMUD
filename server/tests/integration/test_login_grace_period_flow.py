...
class FakeGraceManager:
    """Typed stand-in for ConnectionManager grace-period fields."""

    login_grace_period_players: dict[uuid.UUID, GraceEntry]
    login_grace_period_start_times: dict[uuid.UUID, float]
    player_websockets: dict[uuid.UUID, object]
    async_persistence: object | None
    app: object | None

    def __init__(self) -> None:
        self.login_grace_period_players = {}
        self.login_grace_period_start_times = {}
        self.player_websockets = {}
        self.async_persistence = None
        self.app = None

    async def get_player(self, _player_id: uuid.UUID) -> None:
        return None


@pytest.fixture
def mock_connection_manager() -> FakeGraceManager:
    """Create a mock ConnectionManager for integration testing."""
    return FakeGraceManager()
...
@pytest.mark.asyncio
async def test_start_grace_period_removes_from_combat(
...
@pytest.mark.asyncio
async def test_grace_period_blocks_combat_initiation(
    mock_connection_manager: FakeGraceManager,
...
@pytest.mark.asyncio
async def test_grace_period_remaining_time_decreases(
    mock_connection_manager: FakeGraceManager,
) -> None:  # pylint: disable=redefined-outer-name  # Reason: Fixture parameter name matches fixture function name, pytest standard pattern
...
@pytest.mark.asyncio
async def test_grace_period_expires_after_duration(
    mock_connection_manager: FakeGraceManager,
) -> None:  # pylint: disable=redefined-outer-name  # Reason: Fixture parameter name matches fixture function name, pytest standard pattern
...
@pytest.mark.asyncio
async def test_multiple_players_independent_grace_periods(
    mock_connection_manager: FakeGraceManager,
) -> None:  # pylint: disable=redefined-outer-name  # Reason: Fixture parameter name matches fixture function name, pytest standard pattern
...
@pytest.mark.asyncio
async def test_grace_period_can_be_cancelled(
    mock_connection_manager: FakeGraceManager,
) -> None:  # pylint: disable=redefined-outer-name  # Reason: Fixture parameter name matches fixture function name, pytest standard pattern
...
@pytest.mark.asyncio
async def test_grace_period_start_time_tracking(
    mock_connection_manager: FakeGraceManager,
) -> None:  # pylint: disable=redefined-outer-name  # Reason: Fixture parameter name matches fixture function name, pytest standard pattern
...
async def test_effect_based_grace_start_then_tick_expiration_clears_in_memory(
    ...
    await handle_login_grace_period_expiration(player_id, mock_connection_manager)
