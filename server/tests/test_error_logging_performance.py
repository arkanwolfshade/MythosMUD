...
class TestLogAndRaisePerformance:
    ...
    def test_log_and_raise_bulk_performance(self, perf_mixin):
        ...
        with patch("server.utils.error_logging.get_logger") as mock_get_logger, \
            ...
            for batch_size in [100, 500, 1000]:
                ...
                assert ops_per_second > 500, f"log_and_raise rate too low: {ops_per_second:.0f} ops/second"
    ...
    def test_log_and_raise_concurrent_performance(self, perf_mixin):
        ...
        with patch("server.utils.error_logging.get_logger") as mock_get_logger, \
            ...
            assert avg_time < 0.05, f"Average log_and_raise too slow: {avg_time:.6f}s"
            assert max_time < 0.2, f"Maximum log_and_raise too slow: {max_time:.6f}s"
