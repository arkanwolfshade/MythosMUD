...
class TestRunningEventLoop:
    ...
    async def test_complete_connection_disconnection_flow(self):
        ...
        mock_connection_manager.broadcast_to_room.assert_called()
        call_args = mock_connection_manager.broadcast_to_room.call_args
        message = call_args[0][1]
        assert message["event_type"] == "player_entered"
        ...
        # Verify player left event was broadcast
        mock_connection_manager.broadcast_to_room.assert_called()
        call_args = mock_connection_manager.broadcast_to_room.call_args
        message = call_args[0][1]
    ...
    async def test_multiple_events_processing(self):
        ...
        # Verify that broadcast_to_room was called for each event
        assert mock_connection_manager.broadcast_to_room.call_count == 3

        # Verify the message types
        calls = mock_connection_manager.broadcast_to_room.call_args_list
        assert calls[0][0][1]["event_type"] == "player_entered"
        assert calls[1][0][1]["event_type"] == "player_entered"
        assert calls[2][0][1]["event_type"] == "player_left"
