...
class TestTemporaryPermanentMutes:
    ...
    async def test_temporary_mute_with_long_duration(self, mock_user_manager, mock_rate_limiter, mock_nats_service):
        ...
        # Apply temporary mute (1 hour)
        mute_result = self.chat_service.mute_player(muter_id=self.muter_id, target_player_name=self.target_name)
        assert mute_result is True

        # Test emote is blocked when muted
        mock_user_manager.is_player_muted.return_value = True
        emote_result = await self.chat_service.send_emote_message(self.target_id, "dances")
        assert emote_result["success"] is False
        assert "muted" in emote_result["error"].lower()

        # Test multiple emotes are blocked during mute period
        emote_result = await self.chat_service.send_emote_message(self.target_id, "laughs")
        assert emote_result["success"] is False

        emote_result = await self.chat_service.send_emote_message(self.target_id, "cries")
        assert emote_result["success"] is False
    ...
    async def test_permanent_mute_behavior(self, mock_user_manager, mock_rate_limiter, mock_nats_service):
        ...
        # Apply permanent mute (no duration)
        mute_result = self.chat_service.mute_player(muter_id=self.muter_id, target_player_name=self.target_name)
        assert mute_result is True

        # Test emote is blocked when permanently muted
        mock_user_manager.is_player_muted.return_value = True
        emote_result = await self.chat_service.send_emote_message(self.target_id, "waves")
        assert emote_result["success"] is False
        assert "muted" in emote_result["error"].lower()
    ...
    async def test_mute_type_priority_with_emotes(self, mock_user_manager, mock_rate_limiter, mock_nats_service):
        ...
        # Apply personal mute
        personal_mute_result = self.chat_service.mute_player(
