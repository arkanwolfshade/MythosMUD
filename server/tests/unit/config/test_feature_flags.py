...
class TestFeatureFlagSystem:
    ...
    def test_combat_feature_flag_can_be_disabled(self):
        ...
        config = GameConfig(aliases_dir=os.getenv("GAME_ALIASES_DIR"), combat_enabled=False)
    ...
    def test_combat_configuration_settings(self):
        ...
        config = GameConfig(
            aliases_dir=os.getenv("GAME_ALIASES_DIR"),
            combat_enabled=True,
    ...
    def test_combat_configuration_invalid_tick_interval(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            GameConfig(aliases_dir=os.getenv("GAME_ALIASES_DIR"), combat_tick_interval=0)
    ...
    def test_combat_configuration_invalid_timeout(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            GameConfig(aliases_dir=os.getenv("GAME_ALIASES_DIR"), combat_timeout_seconds=30)
    ...
    def test_combat_configuration_invalid_xp_multiplier(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            GameConfig(aliases_dir=os.getenv("GAME_ALIASES_DIR"), combat_xp_multiplier=0.5)
...
class TestCombatMonitoringConfiguration:
    ...
    def test_combat_monitoring_invalid_thresholds(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            GameConfig(aliases_dir=os.getenv("GAME_ALIASES_DIR"), combat_alert_threshold=0)
        assert "Alert threshold must be between 1 and 100" in str(exc_info.value)

        # Performance threshold too high
        with pytest.raises(ValidationError) as exc_info:
            GameConfig(aliases_dir=os.getenv("GAME_ALIASES_DIR"), combat_performance_threshold=10000)
        assert "Performance threshold must be between 100 and 5000 milliseconds" in str(exc_info.value)

        # Error threshold too high
        with pytest.raises(ValidationError) as exc_info:
            GameConfig(aliases_dir=os.getenv("GAME_ALIASES_DIR"), combat_error_threshold=100)
...
class TestCombatConfigurationEdgeCases:
    ...
    def test_configuration_precedence(self, monkeypatch):
        ...
        config = GameConfig(aliases_dir="data/aliases", combat_tick_interval=10)
        assert config.combat_tick_interval == 10
        ...
        reset_config()
        config = get_config()
        assert config.game.combat_tick_interval == 6  # Default value
