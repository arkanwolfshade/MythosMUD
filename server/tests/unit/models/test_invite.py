...
def test_invite_is_expired_with_future_expiry():
...
def test_invite_is_expired_with_past_expiry():
...
def test_invite_is_expired_with_aware_datetime():
...
def test_invite_is_valid_with_active_and_not_expired():
...
def test_invite_is_valid_with_inactive():
...
def test_invite_is_valid_with_expired():
...
def test_invite_is_valid_with_inactive_and_expired():
...
def test_invite_create_invite_defaults():
    ...
    with patch("server.models.invite.Invite._generate_invite_code", return_value="TESTCODE123"):
        ...
        # is_active defaults to True in the column definition, but may not be set until saved
        # Check that it's either True or None (will be True when saved to DB)
        assert invite.is_active is True or invite.is_active is None
...
def test_invite_create_invite_with_creator():
...
def test_invite_create_invite_with_custom_expiry():
...
def test_invite_generate_invite_code_format():
...
def test_invite_generate_invite_code_uniqueness():
