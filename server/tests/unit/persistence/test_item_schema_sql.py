...
def test_item_schema_and_seed_scripts_create_expected_artifacts(tmp_path: Path):
    ...
    try:
        ...
        indexes = {
            row["name"]: row for row in connection.execute("PRAGMA index_list('item_instances')")
        }
        ...
        ordered_ids = [
            ...
            for row in connection.execute(
                "SELECT prototype_id FROM item_prototypes ORDER BY prototype_id"
            )
        ]
