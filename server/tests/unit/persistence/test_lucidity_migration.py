"""Tests for the lucidity system database migration."""
...
from server.scripts.lucidity_migration import migrate_lucidity_system
...
CREATE TABLE players (
    ...
    current_room_id TEXT NOT NULL DEFAULT 'earth_arkhamcity_LCDitarium_room_foyer_001',
    respawn_room_id TEXT DEFAULT 'earth_arkhamcity_LCDitarium_room_foyer_001',
...
def legacy_database(tmp_path: Path) -> Path:
    """Provision a legacy database missing lucidity tables for migration tests."""
    db_path = tmp_path / "legacy_lucidity.db"
...
def test_migration_creates_tables_and_backfills_defaults(legacy_database: Path) -> None:
    """Ensure new lucidity tables exist and baseline data is backfilled."""
    ...
    with sqlite3.connect(legacy_database) as conn:
        ...
        lucidity_index_names = {row[1] for row in conn.execute("PRAGMA index_list('player_lucidity')")}
        assert "idx_player_lucidity_tier" in lucidity_index_names

        adjustment_index_names = {row[1] for row in conn.execute("PRAGMA index_list('lucidity_adjustment_log')")}
        assert "idx_lucidity_adjustment_player_created" in adjustment_index_names
...
def test_migration_enforces_constraints_and_foreign_keys(legacy_database: Path) -> None:
    ...
    with sqlite3.connect(legacy_database) as conn:
        ...
        with pytest.raises(sqlite3.IntegrityError):
            conn.execute(
                "UPDATE player_lucidity SET current_tier = ? WHERE player_id = ?",
        ...
        conn.execute(
            "INSERT INTO lucidity_exposure_state (player_id, entity_archetype, encounter_count) VALUES (?, ?, ?)",
        ...
        conn.execute(
            "INSERT INTO lucidity_cooldowns (player_id, action_code, cooldown_expires_at) VALUES (?, ?, ?)",
...
def test_migration_is_idempotent_and_preserves_updates(legacy_database: Path) -> None:
    ...
    with sqlite3.connect(legacy_database) as conn:
        ...
        lucidity_row_count = conn.execute("SELECT COUNT(*) FROM player_lucidity").fetchone()[0]
        assert lucidity_row_count == 2
