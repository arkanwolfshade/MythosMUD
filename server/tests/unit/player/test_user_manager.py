...
# Test player IDs - use consistent UUIDs for testing
TEST_PLAYER_001 = uuid.uuid4()
TEST_PLAYER_002 = uuid.uuid4()
TEST_PLAYER_003 = uuid.uuid4()
...
class TestUserManager:
    ...
    def test_add_admin_success(self, mock_get_persistence):
        ...
        result = self.user_manager.add_admin(TEST_PLAYER_001, "TestPlayer")

        assert result is True
        assert TEST_PLAYER_001 in self.user_manager._admin_players
    ...
    def test_add_admin_player_not_found(self, mock_get_persistence):
        ...
        assert TEST_PLAYER_001 not in self.user_manager._admin_players
    ...
    def test_remove_admin_success(self, mock_get_persistence):
        ...
        result = self.user_manager.remove_admin(TEST_PLAYER_001, "TestPlayer")
    ...
    def test_is_admin_in_memory_cache(self):
        ...
        result = self.user_manager.is_admin(TEST_PLAYER_001)
    ...
    def test_mute_player_success(self):
        ...
        assert result is True
        assert TEST_PLAYER_001 in self.user_manager._player_mutes
        assert TEST_PLAYER_002 in self.user_manager._player_mutes[TEST_PLAYER_001]

        mute_info = self.user_manager._player_mutes[TEST_PLAYER_001][TEST_PLAYER_002]
        assert mute_info["target_id"] == TEST_PLAYER_002
        assert mute_info["target_name"] == "TargetPlayer"
        assert mute_info["muted_by"] == TEST_PLAYER_001
    ...
    def test_mute_player_admin_immune(self):
        ...
        # Add target as admin
        self.user_manager._admin_players.add(TEST_PLAYER_002)
        ...
        assert result is False
        assert TEST_PLAYER_001 not in self.user_manager._player_mutes
    ...
    def test_unmute_player_success(self):
        ...
        self.user_manager.mute_player(
            muter_id=TEST_PLAYER_001, muter_name="MuterPlayer", target_id=TEST_PLAYER_002, target_name="TargetPlayer"
        )

        # Then unmute
        result = self.user_manager.unmute_player(
            unmuter_id=TEST_PLAYER_001,
            unmuter_name="MuterPlayer",
            target_id=TEST_PLAYER_002,
            target_name="TargetPlayer",
        )
    ...
    def test_mute_channel_success(self):
        ...
        assert result is True
        assert TEST_PLAYER_001 in self.user_manager._channel_mutes
        assert "global" in self.user_manager._channel_mutes[TEST_PLAYER_001]

        mute_info = self.user_manager._channel_mutes[TEST_PLAYER_001]["global"]
    ...
    def test_mute_channel_permanent(self):
        ...
        result = self.user_manager.mute_channel(
            player_id=TEST_PLAYER_001, player_name="TestPlayer", channel="say", reason="Annoying"
        )

        assert result is True
        mute_info = self.user_manager._channel_mutes[TEST_PLAYER_001]["say"]
        assert mute_info["is_permanent"] is True
        assert mute_info["expires_at"] is None
    ...
    def test_unmute_channel_success(self):
        ...
        # Then unmute
        result = self.user_manager.unmute_channel(player_id=TEST_PLAYER_001, player_name="TestPlayer", channel="global")

        assert result is True
        assert TEST_PLAYER_001 not in self.user_manager._channel_mutes
    ...
    def test_mute_global_success(self):
        ...
        assert result is True
        assert TEST_PLAYER_002 in self.user_manager._global_mutes

        mute_info = self.user_manager._global_mutes[TEST_PLAYER_002]
        assert mute_info["target_id"] == TEST_PLAYER_002
        assert mute_info["muted_by"] == TEST_PLAYER_001
    ...
    def test_mute_global_admin_immune(self):
        ...
        result = self.user_manager.mute_global(
            muter_id=TEST_PLAYER_001, muter_name="MuterPlayer", target_id=TEST_PLAYER_002, target_name="AdminPlayer"
        )

        assert result is False
        assert TEST_PLAYER_002 not in self.user_manager._global_mutes
    ...
    def test_is_player_muted_active(self):
        ...
        result = self.user_manager.is_player_muted(TEST_PLAYER_001, TEST_PLAYER_002)
    ...
    def test_is_channel_muted_active(self):
        ...
        result = self.user_manager.is_channel_muted(TEST_PLAYER_001, "global")
    ...
    def test_is_channel_muted_expired(self):
        ...
        self.user_manager.mute_channel(
            player_id=TEST_PLAYER_001, player_name="TestPlayer", channel="global", duration_minutes=1
        )
    ...
    def test_is_globally_muted_active(self):
        ...
        result = self.user_manager.is_globally_muted(TEST_PLAYER_002)
    ...
    def test_can_send_message_admin(self):
        ...
        result = self.user_manager.can_send_message(TEST_PLAYER_001)
    ...
    def test_can_send_message_globally_muted(self):
        ...
        result = self.user_manager.can_send_message(TEST_PLAYER_002)
    ...
    def test_can_send_message_channel_muted(self):
        ...
        result = self.user_manager.can_send_message(TEST_PLAYER_001, channel="global")
    ...
    def test_get_player_mutes(self):
        ...
        self.user_manager.mute_global(
            muter_id=TEST_PLAYER_001, muter_name="MuterPlayer", target_id=TEST_PLAYER_003, target_name="GlobalTarget"
        )

        mutes = self.user_manager.get_player_mutes(TEST_PLAYER_001)

        assert len(mutes["player_mutes"]) == 1
        assert len(mutes["channel_mutes"]) == 1
        assert len(mutes["global_mutes"]) == 1
        assert TEST_PLAYER_002 in mutes["player_mutes"]
        assert "global" in mutes["channel_mutes"]
        assert TEST_PLAYER_003 in mutes["global_mutes"]
    ...
    def test_is_player_muted_by_others_true(self):
        ...
        result = self.user_manager.is_player_muted_by_others(TEST_PLAYER_002)

        assert result is True

    def test_is_player_muted_by_others_false(self):
        """Test checking if player is not muted by others."""
        result = self.user_manager.is_player_muted_by_others(TEST_PLAYER_001)
    ...
    def test_get_who_muted_player(self):
        ...
        # Apply personal mute
        self.user_manager.mute_player(
            muter_id=TEST_PLAYER_003, muter_name="AnotherMuter", target_id=TEST_PLAYER_002, target_name="TargetPlayer"
        )

        muted_by = self.user_manager.get_who_muted_player(TEST_PLAYER_002)
    ...
    def test_cleanup_expired_mutes(self):
        ...
        # Add expired mutes
        self.user_manager._player_mutes[TEST_PLAYER_001] = {
            TEST_PLAYER_002: {"target_id": TEST_PLAYER_002, "expires_at": datetime.now(UTC) - timedelta(minutes=1)}
        }

        self.user_manager._channel_mutes[TEST_PLAYER_001] = {
            "global": {"channel": "global", "expires_at": datetime.now(UTC) - timedelta(minutes=1)}
        }

        self.user_manager._global_mutes[TEST_PLAYER_003] = {
            "target_id": TEST_PLAYER_003,
        ...
        assert TEST_PLAYER_001 not in self.user_manager._player_mutes
        assert TEST_PLAYER_001 not in self.user_manager._channel_mutes
        assert TEST_PLAYER_003 not in self.user_manager._global_mutes
    ...
    def test_load_player_mutes_file_not_exists(self):
        ...
        result = self.user_manager.load_player_mutes(TEST_PLAYER_001)
    ...
    def test_save_player_mutes_success(self):
        ...
        # Save mutes
        result = self.user_manager.save_player_mutes(TEST_PLAYER_001)
    ...
    def test_cleanup_player_mutes_success(self):
        ...
        # Cleanup
        result = self.user_manager.cleanup_player_mutes(TEST_PLAYER_001)
    ...
    def test_cleanup_player_mutes_delete_file(self):
        ...
        self.user_manager.save_player_mutes(TEST_PLAYER_001)

        mute_file = self.user_manager._get_player_mute_file(TEST_PLAYER_001)
        assert mute_file.exists()

        result = self.user_manager.cleanup_player_mutes(TEST_PLAYER_001, delete_file=True)
...
class TestUserManagerIntegration:
    ...
    def test_full_mute_lifecycle(self):
        ...
        # 2. Check if muted
        is_muted = self.user_manager.is_player_muted(TEST_PLAYER_001, TEST_PLAYER_002)
    ...
    def test_persistence_lifecycle(self):
        ...
        result = new_user_manager.load_player_mutes(TEST_PLAYER_001)
        assert result is True

        # 4. Verify data was loaded correctly
        assert TEST_PLAYER_001 in new_user_manager._player_mutes
