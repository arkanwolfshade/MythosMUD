"""Tests ensuring NPC combat integration applies lucidity effects."""
...
from server.services.active_lucidity_service import UnknownEncounterCategoryError
...
@pytest.mark.asyncio
async def test_apply_encounter_lucidity_effect_invokes_active_service(session_factory, monkeypatch):
    calls: list[SimpleNamespace] = []

    class FakeActivelucidityService:
        def __init__(self, session):
            self.session = session

        async def apply_encounter_lucidity_loss(self, player_id, entity_archetype, category, location_id):
    ...
    monkeypatch.setattr(
        "server.services.npc_combat_integration_service.ActivelucidityService",
        FakeActivelucidityService,
    )

    service = object.__new__(NPCCombatIntegrationService)
    await service._apply_encounter_lucidity_effect(
        "player-1",
        "npc-nightgaunt-001",
...
@pytest.mark.asyncio
async def test_apply_encounter_lucidity_effect_falls_back_to_disturbing(session_factory, monkeypatch):
    categories: list[str] = []

    class FlakyActivelucidityService:
        def __init__(self, session):
            self.session = session
            self._attempts = 0
    ...
    await service._apply_encounter_lucidity_effect(
        "player-1",
        "npc-hound-001",
        DummyNPCDefinition(name="Hound of Tindalos", behavior={"lucidity_category": "cosmic"}),
