...
from .test_npc_combat_integration_service import (
    integration_service as _integration_service_fixture,  # noqa: F401  # pylint: disable=unused-import  # Reason: pytest discovers and uses this fixture by name; alias keeps import while avoiding name collision
)


@pytest.mark.asyncio
async def test_handle_npc_attack_on_player_happy_path(integration_service):
    ...
    integration_service._uuid_mapping.convert_to_uuid = MagicMock(
        side_effect=lambda value: npc_uuid if value == npc_id else player_uuid
    )
    ...
    integration_service._combat_service.process_attack.assert_awaited_once()


@pytest.mark.asyncio
async def test_handle_npc_attack_on_player_grace_period_blocked(integration_service):
...
@pytest.mark.asyncio
async def test_handle_npc_attack_on_player_npc_not_found(integration_service):
...
@pytest.mark.asyncio
async def test_handle_npc_attack_on_player_npc_dead(integration_service):
...
@pytest.mark.asyncio
async def test_handle_npc_attack_on_player_invalid_location(integration_service):
    ...
    assert result is False
    integration_service._combat_service.start_combat.assert_not_awaited()


@pytest.mark.asyncio
async def test_handle_npc_attack_on_player_no_combat_service(integration_service):
    ...
    integration_service._combat_service = None
...
@pytest.mark.asyncio
async def test_handle_npc_attack_on_player_existing_combat_with_same_npc(integration_service):
...
@pytest.mark.asyncio
async def test_handle_npc_attack_on_player_existing_combat_with_other_npc(integration_service):
...
def test_setup_combat_uuids_npc_attacker_valid(integration_service):
    ...
    integration_service._uuid_mapping.store_string_id_mapping.assert_called_once_with(attacker_uuid, npc_id)


def test_setup_combat_uuids_npc_attacker_value_error(integration_service):
