"""Tests for passive LCD flux service."""
...
from server.services.passive_lucidity_flux_service import (
    PassiveFluxContext,
...
async def _create_player(session: AsyncSession, *, room_id: str, lucidity: int = 100, tier: str = "lucid") -> Player:
...
async def test_positive_flux_accumulates_and_applies(session_factory):
    async with session_factory() as session:
        ...
        for p in player_list:
            # Delete related lucidity records first (foreign key constraint)
            await session.execute(delete(PlayerLucidity).where(PlayerLucidity.player_id == p.player_id))
        # Delete all players
        if player_list:
        ...
        player = await _create_player(session, room_id="earth_sunlit_garden", lucidity=90)
        ...
        assert refreshed is not None
        assert refreshed.current_lcd == 91
...
async def test_negative_flux_respects_adaptive_resistance(session_factory):
    async with session_factory() as session:
        ...
        # Without adaptive resistance we'd lose 6 LCD; ensure we lost less.
        assert refreshed.current_lcd > 94
...
async def test_companion_bonus_accumulates(session_factory):
    async with session_factory() as session:
        player = await _create_player(session, room_id="earth_campus_quad", lucidity=60, tier="uneasy")
        await _create_player(session, room_id="earth_campus_quad", lucidity=100, tier="lucid")
        ...
        assert refreshed.current_lcd >= 61  # companion support created net gain


def test_resolve_context_prefers_lucidity_override():
    class _StubRoom:
        def __init__(self):
            self.id = "earth_arkhamcity_LCDitarium_room_foyer_001"
            self.environment = "indoors"
            self.plane = "earth"
            self.zone = "arkhamcity"
            self.sub_zone = "LCDitarium"

    class _StubPersistence:
        def get_room(self, room_id: str):
            return _StubRoom()

    overrides = {"earth|arkhamcity|LCDitarium": 1.25}
    flux_service = PassiveLucidityFluxService(
        persistence=_StubPersistence(),
        lucidity_rate_overrides=overrides,
    )
    player_stub = type(
        "PlayerStub",
        (),
        {"player_id": "player-override", "current_room_id": "earth_arkhamcity_LCDitarium_room_foyer_001"},
    )

    context = flux_service._resolve_context(player_stub, datetime.now(UTC))
    assert math.isclose(context.base_flux, 1.25)
    assert context.source.startswith("lucidity_rule:")
    assert context.metadata.get("lucidity_rate_override") is True
