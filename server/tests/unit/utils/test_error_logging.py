...
This module tests the error logging utilities from server.utils.error_logging
and server.exceptions, ensuring proper error handling and logging functionality.
...
- All tests in this file MUST test server code (server.utils.error_logging, server.exceptions)
- Tests MUST call actual server functions, not just raise exceptions
- Tests MUST verify server behavior, not just that exceptions can be raised
- DO NOT test test utilities (ErrorLoggingTestMixin) - those are helpers, not application code
- DO NOT write tests that only verify Python built-in exceptions work

This file was moved from server/tests/fixtures/test_error_logging.py to ensure
tests are in proper test directories, not in the fixtures directory.
"""
