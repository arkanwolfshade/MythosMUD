...
def test_load_motd_file_exists():
    ...
    with patch("server.utils.motd_loader.get_config") as mock_config, patch("os.path.exists", return_value=True), patch("builtins.open", mock_open(read_data=mock_content)):
...
def test_load_motd_file_read_error():
    ...
    with patch("server.utils.motd_loader.get_config") as mock_config, patch("os.path.exists", return_value=True), patch("builtins.open", side_effect=OSError("Read error")):
        mock_config.return_value.get.return_value = "./data/motd.txt"
        result = load_motd()
...
def test_load_motd_empty_file():
    ...
    with patch("server.utils.motd_loader.get_config") as mock_config, patch("os.path.exists", return_value=True), patch("builtins.open", mock_open(read_data="")):
        mock_config.return_value.get.return_value = "./data/motd.txt"
        result = load_motd()
        assert isinstance(result, str)
