...
class ModerationCommandFactory:
    ...
    def create_mute_command(args: list[str]) -> MuteCommand:
        ...
        if not args:
            log_and_raise_enhanced(
                MythosValidationError, "Mute command requires a player name", args=args, logger_name=__name__
            )
    ...
    def create_unmute_command(args: list[str]) -> UnmuteCommand:
        ...
        if not args:
            log_and_raise_enhanced(
                MythosValidationError, "Unmute command requires a player name", args=args, logger_name=__name__
            )
        if len(args) > 1:
            log_and_raise_enhanced(
                MythosValidationError,
                "Unmute command takes only one argument",
                args=args,
                arg_count=len(args),
    ...
    def create_add_admin_command(args: list[str]) -> AddAdminCommand:
        ...
        if not args:
            log_and_raise_enhanced(
                MythosValidationError, "Add_admin command requires a player name", args=args, logger_name=__name__
            )
        if len(args) > 1:
            log_and_raise_enhanced(
                MythosValidationError,
    ...
    def create_admin_command(args: list[str]) -> AdminCommand:
        ...
        if not args:
            log_and_raise_enhanced(
                MythosValidationError, "Admin command requires a subcommand", args=args, logger_name=__name__
            )
    ...
    def create_mutes_command(args: list[str]) -> MutesCommand:
        ...
        if args:
            log_and_raise_enhanced(
                MythosValidationError, "Mutes command takes no arguments", args=args, logger_name=__name__
            )
        return MutesCommand()
